package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.GoldPriceQueryRequest;
import com.icbc.api.request.GoldPriceQueryRequest.GoldPriceQueryRequestBiz;
import com.icbc.api.response.GoldPriceQueryResponse;

public class GoldPriceQueryTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002166";


	public void test_cop() throws IcbcApiException {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		GoldPriceQueryRequest request = new GoldPriceQueryRequest();
		request.setServiceUrl("http://ip:port/api/preciousmetal/gold/price/V1/query");

		GoldPriceQueryRequestBiz bizContent = new GoldPriceQueryRequestBiz();
		bizContent.setQuotePriceReqTs("2016-10-28 17:15:12.762126");

		request.setBizContent(bizContent);

		GoldPriceQueryResponse response = client.execute(request, "msgId");
		System.out.println(JSONObject.toJSONString(response));
		if (response.isSuccess() && response.getSubFlag() == 1) {
			// 业务成功处理
			response.getExchangePrice();//
		} else {
			// 失败
		}

	}

}
