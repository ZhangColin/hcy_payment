package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.GyjrIssueInvoiceOnlineRequestV1;
import com.icbc.api.response.GyjrIssueInvoiceOnlineResponseV1;

import java.util.List;

public class GyjrIssueInvoiceOnlineRequestV1Test {
    public static void main(String[] args) throws IcbcApiException {
        String apigwPublicKey = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----"; //api网关公钥
        String privateKey = "-----请使用正确的APP私钥-----";  //私钥
        String appId = "11000000000000001006";
        String url = "http://ip:port/api/gyjr/b2b/invoice/onlineCancel/V1";
        DefaultIcbcClient client = new DefaultIcbcClient(appId, IcbcConstants.SIGN_TYPE_RSA2, privateKey, apigwPublicKey);

        GyjrIssueInvoiceOnlineRequestV1 request = new GyjrIssueInvoiceOnlineRequestV1();
        GyjrIssueInvoiceOnlineRequestV1.TransInfo transInfo = new GyjrIssueInvoiceOnlineRequestV1.TransInfo();
        GyjrIssueInvoiceOnlineRequestV1.RequestV1Biz bizContent = new GyjrIssueInvoiceOnlineRequestV1.RequestV1Biz();

        bizContent.setTransInfo(transInfo);
        request.setBizContent(bizContent);

        request.setServiceUrl(url);
        transInfo.setProtocolCode("聚融协议号");
        transInfo.setAccNo("特征值");
        transInfo.setAppid(appId);
        transInfo.setEvent_no("1334454555");

        transInfo.setMerge(true);
        transInfo.setSplit(false);
        transInfo.setInvoiceClass("1");
        transInfo.setGetWay("1");
        transInfo.setGetDate("2023-02-12");
        transInfo.setGetBrno("");
        transInfo.setGetZoneno("");
        transInfo.setReceiveType("1");
        /**
         * set各个字段
         * 。。。
         */

        String jsonstr = "[{\"MONTH\":\"12\",\"ZONENO\":\"100\",\"DTLSERNO\":\"rfhjhjff324324\",\"WORKDATE\":\"2022-11-11\",\"CNLTRXCD\":\"yhjfff\",\"TELLERNO\":\"20\",\"EVENTSEQ\":\"sdfasdf\",\"INVNO\":\"asfasdf\",\"TZONENO\":\"235\",\"TBRNO\":\"123\",\"BAKDEC6\":\"0\",\"NREALAMT\":\"100000\",\"NRCVAMT\":\"100000\",\"BAKDEC1\":\"403\",\"BAKDEC2\":\"itemcode2\",\"TAXRATE\":\"17\",\"TAXFLAG\":\"1\",\"NDCTAMT\":\"0\",\"FEEITEM\":\"布洛芬\",\"TPFEATTYPE\":\"1\",\"TPFEAT\":\"www\",\"TPID\":\"www\",\"BUSIDATE\":\"2022-10-10\",\"TAXBASE\":\"2\",\"RCVTRNOVR\":\"100000\",\"REALTRNOVR\":\"100000\",\"TAXAMT\":\"1700\"}],\"get_brno\":\"01222\",\"get_zoneno\":\"04000\",\"get_date\":\"2023-01-01\",\"receive_type\":\"1\"}]";
        List list = JSON.parseArray(jsonstr);
        transInfo.setItemList(list);
        GyjrIssueInvoiceOnlineResponseV1 response = client.execute(request);
        System.out.println(response.isSuccess());
        System.out.println(response.getReturnCode());
        System.out.println(response.getEventNo());
    }
}
