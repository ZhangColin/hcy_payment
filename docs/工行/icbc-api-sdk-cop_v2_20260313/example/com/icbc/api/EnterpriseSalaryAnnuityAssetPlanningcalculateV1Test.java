package com.icbc.api;

import com.icbc.api.request.EnterpriseSalaryAnnuityPlanningcalculateRequestV1;
import com.icbc.api.response.EnterpriseSalaryAnnuityPlanningcalculateResponseV1;
import org.junit.Assert;
import org.junit.Test;

/**
 * @Author: kfzx-libl
 * @Description: 个人养老规划测算测试案例
 * @Date: 2021/1/7
 */
public class EnterpriseSalaryAnnuityAssetPlanningcalculateV1Test {

    
    public void test_cop() {
        // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值

        //1、合作方在工行API平台的APP的编号,应用在API开放平台注册时生成
        String APP_ID = "10000000000004097509";
        //2、RSA公私钥，API平台的公钥，生产公钥与测试公钥是不一样的
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
        //3、RSA公私钥，合作方应用的私钥，应用公钥需要维护在API平台的APP管理中
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //4、AES加密秘钥
        String AES_KEY = "NTF9Fjy668DxUulsCd2Jqw==";

        //5、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8",
                "json", APIGW_PUBLIC_KEY, IcbcConstants.ENCRYPT_TYPE_AES, AES_KEY, (String) null, (String) null);

        //6、设置请求对象
        EnterpriseSalaryAnnuityPlanningcalculateRequestV1 request = new EnterpriseSalaryAnnuityPlanningcalculateRequestV1();
        //7、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("https://ip:port/api/enterprise/salary/annuity/planningcalculate/V1");
        //8、设置业务参数
        EnterpriseSalaryAnnuityPlanningcalculateRequestV1.EnterpriseSalaryAnnuityPlanningcalculateRequestV1Biz bizContent = new EnterpriseSalaryAnnuityPlanningcalculateRequestV1.EnterpriseSalaryAnnuityPlanningcalculateRequestV1Biz();

        //9、组装请求参数
        bizContent.setSerialNo("8303020097321146625"); //流水号
        bizContent.setCustName("张露露"); //人员姓名
        bizContent.setCertType("1"); //身份证号
        bizContent.setCertNo("320321198608110443"); //证件号码
        bizContent.setMobileNo("12322132119"); //移动电话号码
        bizContent.setSex("0");    //性别
        bizContent.setBirthDate("1981-05"); //出生年月 yyyy-MM
        bizContent.setBeginWorkDate("2002-07"); //开始工作时间 yyyy-MM
        bizContent.setZoneNo("110000");    //参加社保地区
        bizContent.setMonthWages("12500.65"); //每月工资（最多14 位整数，2位小数）
        bizContent.setRetirementAge("60"); //退休年龄
        bizContent.setAnnuityFlag("0");    //是否参加年金
        bizContent.setExcpectMoney("9200.71"); //退休后希望领取金额（最多14 位整数，2位小数）
        request.setBizContent(bizContent);

        EnterpriseSalaryAnnuityPlanningcalculateResponseV1 response = null;
        try {
            response = client.execute(request);
        } catch (IcbcApiException e) {
            // 测试环境案例经常调不通 给个默认值
            response = new EnterpriseSalaryAnnuityPlanningcalculateResponseV1();
        }

        if (response.isSuccess()) {
            // 业务成功处理
        } else {
            // 失败
        }

        Assert.assertNotNull(response);
    }
}
