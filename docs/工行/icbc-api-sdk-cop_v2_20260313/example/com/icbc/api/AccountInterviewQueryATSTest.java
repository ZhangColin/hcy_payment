package com.icbc.api;

import org.junit.Test;
import com.icbc.api.request.AccountInterviewQueryATSRequestV1;
import com.icbc.api.request.AccountInterviewQueryATSRequestV1.AccountInterviewQueryATSRequestV1Biz;
import com.icbc.api.response.AccountInterviewQueryATSResponseV1;

public class AccountInterviewQueryATSTest{
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		AccountInterviewQueryATSRequestV1 request = new AccountInterviewQueryATSRequestV1();
		AccountInterviewQueryATSRequestV1Biz bizContent = new AccountInterviewQueryATSRequestV1Biz();
		request.setServiceUrl("ip:port/api/mybank/account/eaccount/eaccinterviewquery/V1");
		bizContent.setCorpNo(""); //合作方机构编号
		bizContent.setCorpSerno(""); //合作方交易单号
		bizContent.setCorpDate(""); //合作方工作日期
		bizContent.setCorpTime(""); //合作方工作时间
		bizContent.setMediumId(""); //卡号
		bizContent.setSecretKey(""); //sm4对称密钥(Base64编码) 

    //request.setSecretKey("h8ZujhDntpKRohwFmGXcnXygNjJHRQbyuVG3183uOdXI2fRgCXeDED9zOw5dO2JucVy7vZ19od7CSUxI2/sVOn6dPklVoaquVElgKRUPlr6D/cqLu25K7hJ3Pt/uOnx4qF/ykmO/IB2qS0rs/sp0Zw=="); 
		request.setBizContent(bizContent);
		AccountInterviewQueryATSResponseV1 response = client.execute(request, "msgId");
		if (response.isSuccess() ) {
			//成功
			response.getInterviewFlag();
		} else {
			//失败
		}
	}
}
