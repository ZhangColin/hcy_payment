package com.icbc.api;

import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.UiIcbcClient;

import java.util.Random;

public class PersloanMeasurementRequestV1Test {

    public static void main(String[] args) {
        String url ="http://ip:port/ui/mybank/loan/inclufin/persloan/measurement/V1";
        String appid="10000000000004094638";
        String priKey="-----请使用正确的APP私钥-----";

        Random random=new Random();
        int uuidRandom=random.nextInt();
        String msgId="msgId"+uuidRandom;

        UiIcbcClient client=new UiIcbcClient(appid, IcbcConstants.SIGN_TYPE_RSA2, priKey,  IcbcConstants.CHARSET_UTF8);

        PersloanMeasurementRequestV1 request = new PersloanMeasurementRequestV1();
        request.setServiceUrl(url);
        PersloanMeasurementRequestV1.PersloanMeasurementRequestV1Biz biz = new PersloanMeasurementRequestV1.PersloanMeasurementRequestV1Biz();
        biz.setBizType("measureWithHouse");
        biz.setCooperUnit("WS3602073");
        request.setBizContent(biz);
        try {
            String form = client.buildPostForm(request, msgId);
            System.out.println(form);
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
