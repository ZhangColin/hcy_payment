package com.icbc.api;

import org.junit.Test;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.B2cLicenseplatePaymentPayRequestV2;
import com.icbc.api.request.B2cLicenseplatePaymentPayRequestV2.B2cLicenseplatePaymentPayRequestV2Biz;
import com.icbc.api.response.B2cLicenseplatePaymentPayResponseV2;

public class B2cLicenseplatePaymentPayV2Test {
	
	//appid
	protected static final String APP_ID = "10000000000000002156";
	
	//合作方私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	//网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		B2cLicenseplatePaymentPayRequestV2 request = new B2cLicenseplatePaymentPayRequestV2();
		
		//根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/b2c/licenseplate/payment/pay/V2");

		B2cLicenseplatePaymentPayRequestV2Biz bizContent = new B2cLicenseplatePaymentPayRequestV2Biz();
		bizContent.setConsumerId("xxx");
		bizContent.setOrderDate("totalDiscAmt");
		bizContent.setOrderId("13131414141414");
		bizContent.setAmount("10000");
		bizContent.setMerchantId("1234567890");
		bizContent.setMerchantAcct("6222020200088440918");
		bizContent.setMerchantType("1");
		bizContent.setGoodsId("1213");
		bizContent.setGoodsName("dfsfdfs");
		bizContent.setCarriageAmt("1");
		bizContent.setGoodsNum("10000");
		bizContent.setTimeout("");
		bizContent.setExtendParams("");
		bizContent.setPlateno("京A23456");
		bizContent.setPlatecolor("0");
		bizContent.setPayType("1");
		bizContent.setAreacode("0200");
		bizContent.setSecorderflag("");

		request.setBizContent(bizContent);

		B2cLicenseplatePaymentPayResponseV2 response;
		try {
			response = client.execute(request, "msgId");
			if (response.isSuccess()) {
				// 业务成功处理
				String orderId = response.getOrderId();
				String totalAmt = response.getTotalAmt();
				String merDiscAmt = response.getMerDiscAmt();
				String bankDiscAmt = response.getBankDiscAmt();
				String totalDiscAmt = response.getTotalDiscAmt();
			} else {
				//失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
}
