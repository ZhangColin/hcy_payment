package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiVacctNovelAcctBalanceQueryRequestV1;
import com.icbc.api.request.JftApiVacctNovelAcctBalanceQueryRequestV1.JftApiVacctNovelAcctBalanceQueryRequestV1Biz;
import com.icbc.api.response.JftApiVacctNovelAcctBalanceQueryResponseV1;
import org.junit.Test;

import java.util.ArrayList;
import java.util.List;

/**
 * 新型登记簿余额查询
 *
 * @author Jxg
 * @date 2021/5/26 10:52
 */
public class JftApiVacctNovelAcctBalanceQueryTest {
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    protected static final String APP_ID = "10000000000000335017";

    @SuppressWarnings("unchecked")

    public void test_cop() {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");
        JftApiVacctNovelAcctBalanceQueryRequestV1 request = new JftApiVacctNovelAcctBalanceQueryRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/vacct/novel/acct/balance/query/V1");
        JftApiVacctNovelAcctBalanceQueryRequestV1Biz bizContent = new JftApiVacctNovelAcctBalanceQueryRequestV1Biz();
        bizContent.setAppId("10000000000000335038");
        List<String> acctNoList = new ArrayList<>();
        acctNoList.add("2021063016430000001515");
        acctNoList.add("2021063013490000001509");
        bizContent.setAcctNoList(acctNoList);
        bizContent.setVersion("1.0.0");//设置接口访问版本号
        request.setBizContent(bizContent);
        JftApiVacctNovelAcctBalanceQueryResponseV1 response;
        try {
            response = (JftApiVacctNovelAcctBalanceQueryResponseV1) client.execute(request, System.currentTimeMillis()+"");
            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
