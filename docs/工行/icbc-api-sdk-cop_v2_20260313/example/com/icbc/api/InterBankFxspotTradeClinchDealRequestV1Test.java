package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.InterBankFxspotTradeClinchDealRequestV1;
import com.icbc.api.request.InterBankFxspotTradeClinchDealRequestV1.InterBankFxspotTradeClinchDealRequestV1Biz;
import com.icbc.api.response.InterBankFxspotTradeClinchDealResponseV1;
/*
 * @author kfzx-guxq
 * @date 2023/03/22 19:39
 */
public class InterBankFxspotTradeClinchDealRequestV1Test {
	public static void main(String[] args) {
        // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值

        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "10000000000004096790";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //4、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
        //5、设置请求对象
        InterBankFxspotTradeClinchDealRequestV1 request = new InterBankFxspotTradeClinchDealRequestV1();

        //6、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("https://ip:port/api/interbank/fxspot/trade/clinchDeal/V1");
        //7、设置业务参数
        InterBankFxspotTradeClinchDealRequestV1Biz bizContent = new InterBankFxspotTradeClinchDealRequestV1Biz();
        bizContent.setAccno1("125464653");
        bizContent.setAccno2("545616551");
        bizContent.setAmount("7558.55");
        bizContent.setBackGround("1");
        bizContent.setBfjFlag1("1");
        bizContent.setBfjFlag2("1");
        bizContent.setBranchId("215454566");
        bizContent.setCino("122335545");
        bizContent.setCurrPair("USD.CNY");
        bizContent.setDealCcy("USD");
        bizContent.setDirection("1");
        bizContent.setExtTradeId("121355445");
        bizContent.setMarkId("002");
        bizContent.setOpenBank1("工行xxx");
        bizContent.setOpenBank2("工行xxx");
        bizContent.setProdType("203");
        bizContent.setReqId("123456789");
        bizContent.setSubBackGround("201");
        bizContent.setTenor("0");
        bizContent.setTradeDate("20230328");
        bizContent.setValueDate("20230328");
        bizContent.setDealUser("0000500008");
        request.setBizContent(bizContent);

        InterBankFxspotTradeClinchDealResponseV1 response;
        try {
            System.out.println("request:"+ JSON.toJSONString(request));
            response = client.execute(request);
            System.out.println("response:" + JSON.toJSONString(response));
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
