package com.icbc.api.request;


import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcConstants;
import com.icbc.api.request.GyjrInvoiceQryListRequestV1;
import com.icbc.api.response.GyjrInvoiceQryListResponseV1;

/**
 * @ClassName GyjrInvoiceQryListRequestV1Test
 * @Description TODO
 * @author: kfzx-zhangy19
 * @create: 2022/12/29 16:52
 **/
public class GyjrInvoiceQryListRequestV1Test {
    protected static String APP_ID = "11000000000000001051";
//    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    // API网关公钥(变量值需替换)-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----
//    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
//    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
//    protected static String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----";
    protected static String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    public static void main(String[] args) {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        try {
            GyjrInvoiceQryListRequestV1 request = new GyjrInvoiceQryListRequestV1();
            request.setServiceUrl("http://ip:port/api/gyjr/b2b/invoice/qryInvoiceList/V1");
            GyjrInvoiceQryListRequestV1.GyjrInvoiceQryListRequestV1Biz bizContent = new GyjrInvoiceQryListRequestV1.GyjrInvoiceQryListRequestV1Biz();
            GyjrInvoiceQryListRequestV1.GyjrInvoiceQryListRequestV1Biz.TransInfo transInfo = new  GyjrInvoiceQryListRequestV1.GyjrInvoiceQryListRequestV1Biz.TransInfo();
            transInfo.setAccNo("0200062014212394556");
            transInfo.setAppid(APP_ID);
            transInfo.setProtocolCode("GYJR20220415100404714699148");
            transInfo.setStartDate("2022-10-10");
            transInfo.setEndDate("2022-12-10");
            transInfo.setEventNo("aweilfj39104710uej13412");
            transInfo.setInvoiceId("1");
            transInfo.setInvoiceNo("000483");
            transInfo.setInvStatus("1");
            transInfo.setPageNum("1");
            transInfo.setPageSize("20");
            bizContent.setTransInfo(transInfo);
            request.setBizContent(bizContent);
            System.out.println("入参：" + JSON.toJSONString(request));
            GyjrInvoiceQryListResponseV1 response =  client.execute(request);
            System.out.println("出参：" + JSON.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(JSON.toJSONString(response.getReturn_content()));
            } else {
                // 失败
                System.out.println(response.getReturnMsg());
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
