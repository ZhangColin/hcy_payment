...
import com.icbc.api.request.CvLetterReceptRequestV1;
import com.icbc.api.request.CvLetterReceptRequestV1.CvLetterReceptRequestV1Biz;
import com.icbc.api.response.CvLetterReceptResponseV1;

public class CvLetterReceptV1Test {
	// 1、网关公钥
	...
	public static void main(String[] args) throws Exception {
		...
		CvLetterReceptRequestV1 request = new CvLetterReceptRequestV1();

		// 4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://ip:port/api/saes/servmer/ordersubmit/V1");
		// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		CvLetterReceptRequestV1Biz bizContent = new CvLetterReceptRequestV1Biz();
		bizContent.setSaTrxId("0000000001");
		bizContent.setCvAccount("622203xxxxxxxxxxx");
		bizContent.setCvCurrenceCode("001");
		bizContent.setCvAccountBalance(10000);
		bizContent.setCvAccountStatus("0");
		bizContent.setCvStatus(1);
		bizContent.setCvLetterId("0000000001");
		bizContent.setCvLetterName("验资函通知书");
		bizContent.setCvLetterFile("sfdaiSSsESFD");

		request.setBizContent(bizContent);

		CvLetterReceptResponseV1 response;
		try {
