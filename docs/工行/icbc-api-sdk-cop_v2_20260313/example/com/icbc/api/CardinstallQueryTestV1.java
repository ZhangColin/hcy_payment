...
import com.icbc.api.request.CardinstallQueryRequestV1;
import com.icbc.api.request.CardinstallQueryRequestV1.CardinstallQueryRequestV1Biz;
import com.icbc.api.response.CardinstallQueryResponseV1;

/**
 * 信用卡办卡分期订单查询接口
 * 
 * 请商户配置SDK中的lib文件夹中的jar包后，根据备注1-7提示进行数据替换
 */
public class CardinstallQueryTestV1{
	//1、网关公钥
	...
	public static void main(String[] args) throws Exception {
		...
		CardinstallQueryRequestV1 request = new CardinstallQueryRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/cardinstall/query/V1");

		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		CardinstallQueryRequestV1Biz bizContent = new CardinstallQueryRequestV1Biz();
		bizContent.setShopId("020002040095");
		bizContent.setOutTradeNo("ZHL2021093014100022531");
		request.setBizContent(bizContent);

		CardinstallQueryResponseV1 response;
		try {
			response = client.execute(request, "msgId");//6、msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
