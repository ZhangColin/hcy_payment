package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BcssMemberCardQueryRequestV1;
import com.icbc.api.response.BcssMemberCardQueryResponseV1;
import com.icbc.api.response.ResultDataResponseV1;
import com.icbc.api.utils.IcbcEncrypt;


public class BcssMemberCardQueryTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static final String AES_KEY = "645r6z1azEItiR+ghwF4+g==";

    // APIGW_PUBLIC_KEY
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----\n";

    protected static final String APP_ID = "10000000000004097413";


    /**
     * APIGW（行外网关） 2020年11月 api地址
     */
    protected static final String API_GW_OUT = "http://ip:port/api";


    public static void main(String[] args) {
        try {
            testQuery();
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }


    public static void testQuery() throws IcbcApiException {
        BcssMemberCardQueryRequestV1.BcssMemberCardQueryRequestBizV1 bizV1 = new BcssMemberCardQueryRequestV1.BcssMemberCardQueryRequestBizV1();
        bizV1.setCorpId("2000000067");
        bizV1.setQueryType("1");
        bizV1.setUserNo("2021071200028");

        BcssMemberCardQueryRequestV1 requestV1 = new BcssMemberCardQueryRequestV1();
        requestV1.setServiceUrl(API_GW_OUT + "/bcss/member/card/query/V1");
        requestV1.setBizContent(bizV1);
        DefaultIcbcClient client = getIcbcClient();
        BcssMemberCardQueryResponseV1 response = client.execute(requestV1);
        System.out.println(JSONObject.toJSONString(response));
        String resultData = response.getResultData();
        String aes = IcbcEncrypt.decryptContent(resultData, "AES", "645r6z1azEItiR+ghwF4+g==", "UTF-8");
        System.out.println(aes);
    }

    private static DefaultIcbcClient getIcbcClient() {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        return client;
    }

//    private static DefaultIcbcClient getIcbcClient() {
//        DefaultIcbcClient client = null;//nullnew DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
//        client=new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY,
//                "AES", "645r6z1azEItiR+ghwF4+g==", null, null, null, null, null);
//        return client;
//    }


    private static UiIcbcClient getUiClinet() throws IcbcApiException {
        UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
        return client;
    }

}
