package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.response.EiopCarbonCreditsAddResponseV1;
import com.icbc.api.IcbcConstants;
import com.icbc.api.request.EiopCarbonCreditsAddRequestV1;
import com.icbc.api.request.EiopCarbonCreditsAddRequestV1.EiopCarbonCreditsAddRequestV1Biz;
public class EiopCarbonCreditsAddTest {
	//1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	//2、appid 通行证第三方测试1 (行外)
	protected static final String APP_ID = "10000000000000014035";

	//3、密钥对认证方式，公钥在API平台登记，此处为私钥 行外访问
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		//签名类型为SHA256WithRSA时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);		
		
		EiopCarbonCreditsAddRequestV1 request = new EiopCarbonCreditsAddRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		//行外流程
		request.setServiceUrl("http://ip:port/api/eiop/carbonCredits/add/V1");
		
		//5、请对照接口文档用bizContent.setEiopCouponResultQuery()方法对业务上送数据进行赋值			
		EiopCarbonCreditsAddRequestV1Biz bizContent = new  EiopCarbonCreditsAddRequestV1Biz();

		bizContent.setOutRequestNo("702100");
		bizContent.setMobile("152XXXXXXXX");
		bizContent.setRuleId("88890");
		bizContent.setMerchantId("");
		bizContent.setSceneId("");

		request.setBizContent(bizContent);

		EiopCarbonCreditsAddResponseV1 response = new EiopCarbonCreditsAddResponseV1();
		try {
			response = client.execute(request, "msgId");
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println("ruleDesc:" + response.getRuleDesc());
				System.out.println("carbonNum:" + response.getCarbonNum());
			} else {
				// 失败
				int returnCode = response.getReturnCode();
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}

	}
}
