...
import com.icbc.api.request.MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1;
import com.icbc.api.request.MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1.MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1Biz;
 ...
 *
 */
public class MybankAccountWbscustomerPubaccountSMSBCustInfoTestV1 {
		...
		private static Logger logger = Logger.getLogger(MybankAccountWbscustomerPubaccountSMSBCustInfoTestV1.class.getName());
		...
		public static void main(String[] args) {
			...
			MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1 request = new MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1();

			// 4、根据测试环境和生产环境替换相应ip和端口
			// 行外流程
			request.setServiceUrl("https://ip:port/api/mybank/account/wbscustomer/pubcustomer/bindcustqry/V1");

			// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
			MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1Biz bizContent = new MybankAccountWbscustomerPubaccountSMSBCustInfoRequestV1Biz();

			bizContent.setAppid("xxxxxxxxxxxxxxxxxxxx");
			bizContent.setParabcode("QDB");
			bizContent.setOpenid("oITMP1C64i488MWhhaOoNTaAYMVQ");
			bizContent.setCode("001C6E0w3wTJpV21E41w3LQHpn4C6E0v");
			bizContent.setIsOpenidOnly("1");

			request.setBizContent(bizContent);
			// 建立返回项
			MybankAccountWbscustomerPubaccountSMSBCustInfoResponseV1 response = new MybankAccountWbscustomerPubaccountSMSBCustInfoResponseV1();
			try {
				response = client.execute(request);
				logger.info("return_code:" + response.getReturnCode());
				logger.info("return_msg:" + response.getReturnMsg());
				logger.info("msg_id"+response.getMsgId());
				logger.info("tranerrorcode:" + response.getTranerrorcode());
				logger.info("tranerrormsg:" + response.getTranerrormsg());
				logger.info("firstBindcardTime:" + response.getFirstBindcardTime());
				logger.info("firstBindcardChannel:" + response.getFirstBindcardChannel());
				logger.info("subscrbFlag:" + response.getSubscrbFlag());
				logger.info("isBindCard:" + response.getIsBindCard());
				...
				System.out.println("firstBindcardTime:" + response.getFirstBindcardTime());
				System.out.println("firstBindcardChannel:" + response.getFirstBindcardChannel());
				System.out.println("subscrbFlag:" + response.getSubscrbFlag());
				System.out.println("isBindCard:" + response.getIsBindCard());*/
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
