package com.icbc.api;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.CardbusinessEtcAgreementunbindRequestV1;
import com.icbc.api.request.CardbusinessEtcAgreementunbindRequestV1.CardbusinessEtcAgreementunbindRequestV1Biz;
import com.icbc.api.response.CardbusinessEtcAgreementunbindResponseV1;

public class CardbusinessEtcAgreementunbindV1Test {
	protected static String APP_ID = "0200EH0013035"; 
	protected static 	String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	protected static 	String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	public static void main(String[] args) {
		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2",MY_PRIVATE_KEY, APIGW_PUBLIC_KEY); 
		CardbusinessEtcAgreementunbindRequestV1 request = new CardbusinessEtcAgreementunbindRequestV1();
		CardbusinessEtcAgreementunbindRequestV1Biz bizContent = new CardbusinessEtcAgreementunbindRequestV1Biz();
    
		bizContent.setMsgId ("152687444"); 
		bizContent.setAgreementNum("440521573114032"); 
		bizContent.setOrgCode("00003"); 
		bizContent.setVehicleId ("00003");
		bizContent.setPlateNum ("粤A88888");
		bizContent.setApplyTime("2012-01-01");
		bizContent.setEffectiveTime("2012-01-01");
		bizContent.setIssuerId ("370101"); 
		bizContent.setPlateColor(0);
		
		request.setServiceUrl("http://ip:port/api/cardbusiness/etc/agreementunbind/V1");
		request.setBizContent(bizContent);
		request.isNeedEncrypt();

		String msg_id = String.valueOf(System.currentTimeMillis());
		try {
			CardbusinessEtcAgreementunbindResponseV1 response = client.execute(request, msg_id);
			if (response.isSuccess() && response.getReturnCode() == 0) {
				
			}else{
				
			}
//			System.out.println("sucessflg=>" + response.getReturnCode() + "|" + response.getReturnMsg());
//			System.out.println("getMsgId:" + response.getMsgId());
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}}
