package com.icbc.api;

import com.icbc.api.request.GyjrB2bBillApplyguaranteeRequestV1;

import java.util.ArrayList;
import java.util.List;

public class GyjrB2bBillApplyguaranteeTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16" +
            "bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL" +
            "T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP" +
            "3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000004097878";
    private static final String CA_PRIVATE_STR="-----请使用正确的APP私钥-----";

    public static void main(String[] args) {

        UiIcbcClient client = new UiIcbcClient(APP_ID, CA_PRIVATE_STR, IcbcConstants.CHARSET_UTF8);

        GyjrB2bBillApplyguaranteeRequestV1 request = new GyjrB2bBillApplyguaranteeRequestV1();

        request.setServiceUrl("http://ip:port/ui/gyjr/b2b/bill/applyguarantee/V1");

        GyjrB2bBillApplyguaranteeRequestV1.GyjrB2bBillApplyguaranteeRequestV1Biz bizContent = new GyjrB2bBillApplyguaranteeRequestV1.GyjrB2bBillApplyguaranteeRequestV1Biz();

        bizContent.setTradeName("APPLYGUARANTEE");
        bizContent.setTradeVersion("V1");
        bizContent.setProtocolCode("4515515");
        bizContent.setPlatVendorId("89798488");
        bizContent.setPlatBatSerialNo("31525481515");
        bizContent.setPageIndex("1");
        bizContent.setPageSize("20");
        bizContent.setTotalAmt("200");
        bizContent.setTotalCount("2");
        bizContent.setCallBackUrl("http://ip:port/");
        bizContent.setApplicantName("付海军");
        bizContent.setApplicantAccount("888888");

        List packList = new ArrayList();

        // 票据1
        GyjrB2bBillApplyguaranteeRequestV1.PackListInfo packInfo1 = new GyjrB2bBillApplyguaranteeRequestV1.PackListInfo();
        packInfo1.setAccptrName("付海军");
        packInfo1.setBusiSeq("948151233545");
        packInfo1.setCdTp("AC01");
        packInfo1.setDrwrName("fuhaijun");
        packInfo1.setDueDate("20231231");
        packInfo1.setGuaranteeAcctId("032134545");
        packInfo1.setGuaranteeAcctsvcr("591505945851851");
        packInfo1.setGuaranteeAmt("101");
        packInfo1.setGuaranteeBranch("8481818");
        packInfo1.setGuaranteeFullName("中国工商银行");
        packInfo1.setGuaranteeIsIcbc("0");
        packInfo1.setHolderName("fuhaijun");
        packInfo1.setIssueDate("20231231");
        packInfo1.setPackNo("548105");
        packInfo1.setRangeAmt("100000");
        packInfo1.setRangeBegin("0");
        packInfo1.setRangeEnd("50");
        packInfo1.setWarnteeAcctId("8748151");
        packInfo1.setWarnteeRemark("付海军");

        // 票据2
        GyjrB2bBillApplyguaranteeRequestV1.PackListInfo packInfo2 = new GyjrB2bBillApplyguaranteeRequestV1.PackListInfo();
        packInfo2.setAccptrName("付海军");
        packInfo2.setBusiSeq("033566032652");
        packInfo2.setCdTp("AC01");
        packInfo2.setDrwrName("fuhaijun");
        packInfo2.setDueDate("20231231");
        packInfo2.setGuaranteeAcctId("032134545");
        packInfo2.setGuaranteeAcctsvcr("591505945851851");
        packInfo2.setGuaranteeAmt("101");
        packInfo2.setGuaranteeBranch("8481818");
        packInfo2.setGuaranteeFullName("中国工商银行");
        packInfo2.setGuaranteeIsIcbc("0");
        packInfo2.setHolderName("fuhaijun");
        packInfo2.setIssueDate("20231231");
        packInfo2.setPackNo("548105");
        packInfo2.setRangeAmt("100000");
        packInfo2.setRangeBegin("0");
        packInfo2.setRangeEnd("50");
        packInfo2.setWarnteeAcctId("8748151");
        packInfo2.setWarnteeRemark("付海军");

        packList.add(packInfo1);
        packList.add(packInfo2);

        bizContent.setPackList(packList);
        request.setBizContent(bizContent);

        try{
            String s = client.buildPostForm(request);
            System.out.println(s);
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }

    }

}
