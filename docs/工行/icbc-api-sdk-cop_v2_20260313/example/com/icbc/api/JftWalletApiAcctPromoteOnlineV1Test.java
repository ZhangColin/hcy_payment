package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftWalletApiAcctPromoteOnlineRequestV1;
import com.icbc.api.request.JftWalletApiAcctPromoteOnlineRequestV1.JftWalletApiAcctPromoteOnlineRequestV1Biz;
import com.icbc.api.response.JftWalletApiAcctPromoteOnlineReponseV1;


/**
 * 三升二线上升级测试类
 *
 * @author kfzx-changcx
 *
 */
public class JftWalletApiAcctPromoteOnlineV1Test {

	// 公钥（根据环境变化）
String MY_PRIVATE_KEY2 = "-----请使用正确的APP私钥-----";
//私钥（根据环境变化）
String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";


	protected static final String APP_ID = "10000000000000008062";// (根据实际情况确定appid)



	// 绑定发送短信

	public void test_cop() {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA",
				MY_PRIVATE_KEY2, "UTF-8", "json", APIGW_PUBLIC_KEY, "AES",
				"i5N/TQHG9hq00OpdNzpUUw==", "", "");
		JftWalletApiAcctPromoteOnlineRequestV1 request = new JftWalletApiAcctPromoteOnlineRequestV1();

		request.setServiceUrl("http://ip:port/api/jft/wallet/api/acctpromoteonline/V1");//根据实际情况确定url和端口

		JftWalletApiAcctPromoteOnlineRequestV1Biz biz = new JftWalletApiAcctPromoteOnlineRequestV1Biz();

		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				biz.setAppId("10000000000000012090");
				biz.setTrxAccDate("2018-10-09");
				biz.setTrxAccTime("15:47:00");
				biz.setCorpDate("2017-03-15");
				biz.setCorpSerno("321654987");
				biz.setOutServiceCode("accountleveladjust");
				biz.setCorpMediumId("");//合作方介质号
				biz.setMediumId("120180712000538311");
				biz.setCertType(0);
				biz.setCertNo("110105198101021118");
				biz.setCustName("武则天");
				biz.setNotifyAddr("");
				// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				System.out.println(biz);
				request.setBizContent(biz);

				JftWalletApiAcctPromoteOnlineReponseV1 response;

				try {

					response = (JftWalletApiAcctPromoteOnlineReponseV1) client.execute(request, System.currentTimeMillis() + "");
					System.out.println("~~~~~~~~~");
					System.out.println(JSONObject.toJSONString(response));
					System.out.println("~~~~~~~~~");
					if (response.isSuccess()) {
						// 业务成功处理
						System.out.println(response.getReturnCode());
					} else {
						// 失败
					}

				} catch (IcbcApiException e) {

					e.printStackTrace();

				}
			}

			public static void main(String[] args) {
				new JftWalletApiAcctPromoteOnlineV1Test().test_cop();
			}
		}
