package com.icbc.api;

import static org.junit.Assert.assertNotNull;

import java.text.SimpleDateFormat;
import java.util.Date;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import com.icbc.api.crypt.AES;
import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BcssMemGetMemCardInfoRequestV1;
import com.icbc.api.request.BcssMemGetMemCardInfoRequestV1.BcssMemGetMemCardInfoRequesBizV1;
 
import com.icbc.api.response.BcssMemGetMemCardInfoResponseV1;

public class BcssMemberGetMemCardInfoTestV1 {

    	private static final Log logger = LogFactory.getLog(BcssMemberGetMemCardInfoTestV1.class);
	// MY_PRIVATE_KEY
		protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

		// APIGW_PUBLIC_KEY
		protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

		// APP_ID
		protected static final String APP_ID = ""; //10000000000004096876

		// 10000000000004096876
		public static void main(String[] args) throws IcbcApiException {
			
			//http://ip:port:8005/api-member/bcssGetExchangeNoteService/query?corpId=2000000067
			//&personId=d9xqFJMpYN1/7QVR6Ae1UqN94QsIlUPCcSiBtgALJE3/4TMHu6u/NDllWuBVyhnr
			//&timeStamp=2020-02-02 12:00:00&trxSrc=1
				
			BcssMemGetMemCardInfoRequestV1 request = new BcssMemGetMemCardInfoRequestV1();
			//http://ip:port/api/cardbusiness/aggregatepay/b2c/online/refundqry/V1
			request.setServiceUrl("http://ip:port/api/bcss/member/getMemCardInfo/V1");
			BcssMemGetMemCardInfoRequesBizV1 hrb = new BcssMemGetMemCardInfoRequesBizV1();
            String  userInfo  = "d9xqFJMpYN1/7QVR6Ae1UqN94QsIlUPCcSiBtgALJE3/4TMHu6u/NDllWuBVyhnr";
			hrb.setCorpId("2000000067");//2000000067
	 
			hrb.setPayMedium("34434343"); 
			hrb.setCustSort(userInfo);
			hrb.setCustCode("1.222");
			hrb.setCustName("323232");
			//System.out.println(AES.aesDecrypt(userInfo, "645r6z1azEItiR+ghwF4+g=="));
			
			//String timeStmp = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date());
			 
			request.setBizContent(hrb);

			DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
			BcssMemGetMemCardInfoResponseV1 response = client.execute(request);
			 
			logger.info(JSON.toJSONString(response ));
		}
}
