package com.icbc.api;

import javax.servlet.http.HttpServlet;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.request.EpmPartyfeeBillDeleteRequestV1;
import com.icbc.api.response.EpmPartyfeeBillDeleteResponseV1;

public class EpmPartyfeeBillDeleteApiServiceImplTest extends HttpServlet {

	/**
	 * Comment for <code>serialVersionUID</code>
	 */
	private static final long serialVersionUID = 1588786734333856129L;
	
	public static void main(String[] args) {

		String appId ="10000000000000469001";
		String MY_PRIVATE_KEY ="-----请使用正确的APP私钥-----";
		String APIGW_PUBLIC_KEY ="-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16"+
		"bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL"+
		"T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP"+
		"3CiNgg0W6O3AGqwIDAQAB";

		DefaultIcbcClient client = new DefaultIcbcClient(appId, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		EpmPartyfeeBillDeleteRequestV1 request = new EpmPartyfeeBillDeleteRequestV1();		
		request.setServiceUrl("http://ip:port/api/epm/partyfee/bill/delete/V1");
		EpmPartyfeeBillDeleteRequestV1.EpmPartyfeeBillDeleteRequestV1Biz bizContent = new EpmPartyfeeBillDeleteRequestV1.EpmPartyfeeBillDeleteRequestV1Biz();
		request.setBizContent(bizContent);
		bizContent.setProtocolNo("PT20181213100000108002");
		bizContent.setBillNo("1291218");

		EpmPartyfeeBillDeleteResponseV1 response;
		try {
			response = client.execute(request, "msgId");
			// 业务成功处理
			System.out.println(response.getReturnCode());
			System.out.println(response.getReturnMsg());
			System.out.println(response.getMsgId());
		
		} catch (Exception e) {
			e.printStackTrace();
		}
		
	}
}
