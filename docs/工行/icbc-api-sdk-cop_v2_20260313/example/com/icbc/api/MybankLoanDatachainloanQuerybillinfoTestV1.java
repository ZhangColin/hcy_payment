package com.icbc.api;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import com.icbc.api.request.MybankLoanDatachainloanQuerybillinfoRequestV1;
import com.icbc.api.request.MybankLoanDatachainloanQuerybillinfoRequestV1Biz;
import com.icbc.api.response.MybankLoanDatachainloanQuerybillinfoResponseV1;

public class MybankLoanDatachainloanQuerybillinfoTestV1 {

	private static Log log = LogFactory.getLog(MybankLoanDatachainloanQuerybillinfoTestV1.class);

    public static void main(String[] args) {
        try {
        	String apigwPublicKey = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
            String privateKey = "-----请使用正确的APP私钥-----";
            String appId = "XXXX";
		   	String url = "http://ip:port/api/mybank/loan/datachainloan/querybillinfo/V1";
		
		   	DefaultIcbcClient client = new DefaultIcbcClient(appId, privateKey,apigwPublicKey);
		   	MybankLoanDatachainloanQuerybillinfoRequestV1 request = new  MybankLoanDatachainloanQuerybillinfoRequestV1();
		   	request.setServiceUrl(url);
		   	MybankLoanDatachainloanQuerybillinfoRequestV1Biz bizContent = new  MybankLoanDatachainloanQuerybillinfoRequestV1Biz();		   				
		   	bizContent.setSerialNo("201805131234567");
		   	bizContent.setAppNo("F-APIP");
		   	bizContent.setAreaCode("");
		   	bizContent.setEmployeeCode("");
		   	bizContent.setLanguage("zh_CN");
		   	bizContent.setTransNo("IDataChainLoanCocoaSerivce");
		   	bizContent.setVer("1.0");
		   	bizContent.setCircleMode("1");
		   	bizContent.setBorrowerName( "交通银行");
		   	bizContent.setBorrowerCerType("3");
		   	bizContent.setBorrowerCerId("020099990039242");
		   	bizContent.setCustName("3");
		   	bizContent.setCustCerType("3");
		   	bizContent.setCustCerId("020199990039246");
		   	bizContent.setEntrustProNo("2026000000364");
		   	bizContent.setCredenceNo("1026000000365");
		   	request.setBizContent(bizContent);
		   	MybankLoanDatachainloanQuerybillinfoResponseV1 response;		
		   	response = client.execute(request);
            if (response.isSuccess()) {
                String a = response.getReturnContent();
                log.info(a);
            } else {
                String a = response.getReturnContent();
                log.error(a);
            }
        } catch (IcbcApiException e1) {
            e1.printStackTrace();
        }
    }

}
