...
public class MybankEnterpriseExchangeFesasdTest {
    ...
    public static void main(String[] args) throws Exception {
        String serviceUri = BASE_URL + "mybank/enterprise/exchange/fesasd/V1";
        IcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
        MybankEnterpriseExchangeFesasdRequestBizV1 bizContent =
                new MybankEnterpriseExchangeFesasdRequestBizV1();
        MybankEnterpriseExchangeFesasdRequestV1 request =
                new MybankEnterpriseExchangeFesasdRequestV1();
        // 请对照接口文档用bizContent.setXxx()方法对业务上送数据进行赋值
        bizContent.setTransCode("FESASD");
        bizContent.setTranDate("20190101");
        bizContent.setTranTime("103231001");
        bizContent.setLanguage("zh_CN");
        bizContent.setfSeqNo("FESASD2070301278");
        bizContent.setAgmtNo("null");
        request.setServiceUrl(serviceUri);
        request.setBizContent(bizContent);
        String validate = ExceptionUtils.validate(bizContent);
        if (null == validate) {
            MybankEnterpriseExchangeFesasdResponseV1 response = client.execute(request);
