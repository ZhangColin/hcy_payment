package com.icbc.api;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiVacctNovelAcctTranStatusQueryRequestV1;
import com.icbc.api.request.JftApiVacctNovelAcctTranStatusQueryRequestV1.JftApiVacctNovelAcctTranStatusQueryRequestV1Biz;
import com.icbc.api.response.JftApiVacctNovelAcctTranStatusQueryResponseV1;
import org.junit.Test;

public class JftApiVacctNovelAcctTranStatusQueryTestV1 {
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000205520";

    @SuppressWarnings("unchecked")

    public void test_cop() {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");
        JftApiVacctNovelAcctTranStatusQueryRequestV1 request = new JftApiVacctNovelAcctTranStatusQueryRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/vacct/novel/acct/tran/status/query/V1");
        JftApiVacctNovelAcctTranStatusQueryRequestV1Biz bizContent = new JftApiVacctNovelAcctTranStatusQueryRequestV1Biz();
        bizContent.setAppId("10000000000000286524");
        bizContent.setBusiNo("0JFT2021070100011363324");
        bizContent.setSerialNo("30902000146002000095449690");
        bizContent.setSceneType("01");
        bizContent.setVersion("1.0.0");//设置接口访问版本号
        request.setBizContent(bizContent);
        JftApiVacctNovelAcctTranStatusQueryResponseV1 response;
        try {
            response = (JftApiVacctNovelAcctTranStatusQueryResponseV1) client.execute(request, System.currentTimeMillis()+"");

            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
