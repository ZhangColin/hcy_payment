...
import com.icbc.api.request.SettlementAccountBalanceQueryRequestV1.SettlementAccountBalanceQueryRequestV1Biz;
import com.icbc.api.response.SettlementAccountBalanceQueryResponseV1;

public class SettlementAccountBalanceQueryTest{
	...
	public void test_cop() throws IcbcApiException{
		...
		request.setServiceUrl("https://ip:port/api/settlement/account/balance/V1/query");
		...
		SettlementAccountBalanceQueryResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess() && response.getReturnCode() == 0) {
			// 业务成功处理
			System.out.println(response.getAccountBalance()); // 可用余额
			System.out.println(response.getHoldBalance()); // 保留金额
		} else {
			// 失败
