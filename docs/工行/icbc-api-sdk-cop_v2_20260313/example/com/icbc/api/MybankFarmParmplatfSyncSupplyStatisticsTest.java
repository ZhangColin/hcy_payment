package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MybankFarmFarmplatfSyncSupplyStatisticsRequestV1;
import com.icbc.api.response.MybankFarmFarmplatfSyncSupplyStatisticsResponseV1;

/**
 * 供销统计数据同步接口
 */
public class MybankFarmParmplatfSyncSupplyStatisticsTest {

    protected static String APP_ID = "10000000000004096993";

    private static String BASE_URL = "http://apip-gateway-nrouter-saas-gn-F.month.sdc.cs.icbc/api";

    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";


    public static void main(String[] args) throws Exception {
        String URI = "mybank/farm/farmplatf/syncSupplyStatistics/V1";

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
        try {
            MybankFarmFarmplatfSyncSupplyStatisticsRequestV1 request = new MybankFarmFarmplatfSyncSupplyStatisticsRequestV1();
            MybankFarmFarmplatfSyncSupplyStatisticsRequestV1.MybankFarmFarmplatfSyncSupplyStatisticsRequestBizV1 requestBizV1 = new MybankFarmFarmplatfSyncSupplyStatisticsRequestV1.MybankFarmFarmplatfSyncSupplyStatisticsRequestBizV1();

            requestBizV1.setfSeqNo("1234567890");
            requestBizV1.setCorpCode("icbc");
            requestBizV1.setTrxCode("62304");
            requestBizV1.setDetail("{\"batchDate\": \"20241217\",\"allTenantCount\": \"1111\",\"yearTenantCount\": \"10\",\"allProvinceCount\": \"1\",\"yearProvinceCount\": \"1\",\"allCountyCount\": \"15\",\"yearCountyCount\": \"1\",\"allUserCount\": \"1000\",\"yearUserCount\": \"2000\",\"featureUsageCountList\": [{\"featureName\": \"xxx1\",\"usageCount\": \"2\"},{\"featureName\": \"xxx2\",\"usageCount\": \"3\"}],\"mauList\": [{\"mauMonth\": \"1\",\"mauMonthCount\": \"100\"},{\"mauMonth\": \"2\",\"mauMonthCount\": \"150\"}]}");

            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(requestBizV1);

            System.out.println(JSON.toJSONString(request));

            MybankFarmFarmplatfSyncSupplyStatisticsResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                System.out.println("success");
                System.out.println(JSONObject.toJSONString(response));
            } else {
                System.out.println("error");
                System.out.println(JSONObject.toJSONString(response));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }

    }

}


