package com.icbc.api;

import com.alibaba.fastjson.JSON;
import com.icbc.api.request.MybankAccountFundsarrivalmateFamnotifymapinfoQueryRequestV1;
import com.icbc.api.response.MybankAccountFundsarrivalmateFamnotifymapinfoQueryResponseV1;

public interface MybankAccountFundsarrivalmateFamnotifymapinfoQueryTestV1 {

    public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //4、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
        MybankAccountFundsarrivalmateFamnotifymapinfoQueryRequestV1 request = new MybankAccountFundsarrivalmateFamnotifymapinfoQueryRequestV1();
        request.setServiceUrl("http://ip:port/api/mybank/account/fundsarrivalmate/famnotifymapinfoqry/V1");

        MybankAccountFundsarrivalmateFamnotifymapinfoQueryRequestV1.FamNotifyMapInfoQueryRequestV1Biz bizContent = new MybankAccountFundsarrivalmateFamnotifymapinfoQueryRequestV1.FamNotifyMapInfoQueryRequestV1Biz();
        bizContent.setAgreeno("1");
        bizContent.setNotifyid("guq_clerk_002");
        bizContent.setField("guq_dealer_002");
        bizContent.setType("4");
        bizContent.setPage("1");
        bizContent.setPagesize("10");

        request.setBizContent(bizContent);
        MybankAccountFundsarrivalmateFamnotifymapinfoQueryResponseV1 response;
        try {
            //测试时，避免apip识别成重放，请修改msgId,每次跑，递加1比如
            response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
                //Assert.assertEquals(response.getReturnMsg(), "success");
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }

}
