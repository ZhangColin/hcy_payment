package com.icbc.api;

import com.icbc.api.request.JftUiVendorOpenAccountMobileRequestV1;
import com.icbc.api.request.JftUiVendorOpenAccountMobileRequestV1.JftUiVendorOpenAccountMobileRequestV1Biz;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.io.PrintWriter;

/**
  * @Author dccb-wblit
  * @Description 移动端往来户预约开户UI接口V1-测试类
  * @Date 2021/1/18 16:36
  * @Version 1.0.0
  */
public class JftUiVendorOpenAccountMobileTestV1 extends HttpServlet {

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		doPost(req, resp);
	}

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {

		try {
			String APP_ID = "10000000000000004473";
			String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

			UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);

			JftUiVendorOpenAccountMobileRequestV1 request = new JftUiVendorOpenAccountMobileRequestV1();
			request.setServiceUrl("https://ip:port/ui/jft/ui/vendor/open/account/mobile/V1");

			JftUiVendorOpenAccountMobileRequestV1Biz bizContent = new JftUiVendorOpenAccountMobileRequestV1Biz();
			bizContent.setAppId(APP_ID);
			bizContent.setApi_name("MOBILEOPENACC");
			bizContent.setApi_version("001.001.001.001");
			bizContent.setChl_type("10");
			bizContent.setFlow_id("H00000X");
			bizContent.setNotify_url("https://xxxx/do.jsp");
			request.setBizContent(bizContent);

			resp.setHeader("Content-Type", "text/html;charset="
					+ IcbcConstants.CHARSET_UTF8);
			PrintWriter out = resp.getWriter();

			out.write("<html>");
			out.write("<head>");
			out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset="
					+ IcbcConstants.CHARSET_UTF8 + "\">");
			out.write("</head>");
			out.write("<body>");
			out.write(client.buildPostForm(request));
			out.write("</body>");
			out.write("</html>");

		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
}

