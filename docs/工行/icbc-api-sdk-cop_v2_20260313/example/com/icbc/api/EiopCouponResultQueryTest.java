package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.response.EiopCouponResultQueryResponseV1;
import com.icbc.api.IcbcConstants;
import com.icbc.api.request.EiopCouponResultQueryRequestV1;
import com.icbc.api.request.EiopCouponResultQueryRequestV1.EiopCouponResultQueryRequestV1Biz;
public class EiopCouponResultQueryTest {
	//1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	//2、appid 通行证第三方测试1 (行外)
	protected static final String APP_ID = "10000000000000014035";

	//3、密钥对认证方式，公钥在API平台登记，此处为私钥 行外访问
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		//签名类型为SHA256WithRSA时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);		
		
		EiopCouponResultQueryRequestV1 request = new EiopCouponResultQueryRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		//行外流程
		request.setServiceUrl("http://ip:port/api/eiop/coupon/result/query/V1");
		
		//5、请对照接口文档用bizContent.setEiopCouponResultQuery()方法对业务上送数据进行赋值			
		EiopCouponResultQueryRequestV1Biz bizContent = new  EiopCouponResultQueryRequestV1Biz();

		bizContent.setChannelId("702100");
		bizContent.setUserReceiveFlag("152XXXXXXXX");
		bizContent.setStartTime("");
		bizContent.setEndTime("");
		bizContent.setActivityId("1");
		bizContent.setCouponId("9");
		bizContent.setPeriodStage("0");
		bizContent.setReceiveStatus("");

		request.setBizContent(bizContent);

		EiopCouponResultQueryResponseV1 response = new EiopCouponResultQueryResponseV1();
		try {
			response = client.execute(request, "msgId");
			if (response.isSuccess()) {
				EiopCouponResultQueryResponseV1.ResultResponseV1Message result  = response.getResult();
				// 业务成功处理
			} else {
				// 失败
				int returnCode = response.getReturnCode();
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}

	}
}
