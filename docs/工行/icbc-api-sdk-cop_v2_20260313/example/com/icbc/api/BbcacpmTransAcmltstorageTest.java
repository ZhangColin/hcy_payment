...
import com.icbc.api.request.BbcacpmTransAcmltstorageRequestV2;
import com.icbc.api.request.BbcacpmTransAcmltstorageRequestV2.BbcacpmTransAcmltstorageRequestBizV2;
import com.icbc.api.response.BbcacpmTransAcmltstorageResponseV2;

public class BbcacpmTransAcmltstorageTest {
    ...
    public static void main(String[] args) throws Exception {
        String URI = "bbcacpm/trans/acmltstorage/V2"; // api地址
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
        try {

            BbcacpmTransAcmltstorageRequestBizV2 bizContent =
                    new BbcacpmTransAcmltstorageRequestBizV2();
            BbcacpmTransAcmltstorageRequestV2 request = new BbcacpmTransAcmltstorageRequestV2();

            // 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
            bizContent.getGrpHdr().setRefrence("0118011800003042");
            bizContent.getGrpHdr().setBusinessCode("10081");
            ...
            bizContent.getAcpmTransferStorageBusitext().setOgcode("999999990001");
            bizContent.getAcpmTransferStorageBusitext().setCurrtype("RMB");
            bizContent.getAcpmTransferStorageBusitext().setProdacc("0200010231023");
            bizContent.getAcpmTransferStorageBusitext().setPrice(1000);
            bizContent.getAcpmTransferStorageBusitext().setCashamt(1000);
            bizContent.getAcpmTransferStorageBusitext().setTrxserno("123123123");
            bizContent.getAcpmTransferStorageBusitext().setTimestmp("2018-05-30-13.45.51.870045");


            // 通讯区参数设置
            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(bizContent);
            // 输出支付指令提交响应对象，需根据实际调用服务更换（BbcacpmTransAcmltstorageResponseV2）
            BbcacpmTransAcmltstorageResponseV2 response = client.execute(request);
