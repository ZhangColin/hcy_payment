package com.icbc.api;

import java.util.Random;
import java.util.logging.Logger;

import com.icbc.api.request.EnterpriseBranchRecoverGroupinfoRequestV1;
import com.icbc.api.request.EnterpriseBranchRecoverGroupinfoRequestV1.EnterpriseBranchRecoverGroupinfoRequestV1Biz;
import com.icbc.api.request.EnterpriseBranchRecoverGroupinfoRequestV1.EnterpriseBranchRecoverGroupinfoRequestV1Biz.AlterInfo;
import com.icbc.api.request.EnterpriseBranchRecoverGroupinfoRequestV1.EnterpriseBranchRecoverGroupinfoRequestV1Biz.ChanCommV10;
import com.icbc.api.response.EnterpriseBranchRecoverGroupinfoResponseV1;

public class EnterpriseBranchRecoverGroupinfoTest {
	protected static final String MY_PRIVATE_KEY = "";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "0200EH0005040"; 

	protected static final String password = "12345678";

	protected static final String MY_PUB_KEY = "-----请使用正确的APP私钥----------请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----";

	public static void main(String[] args) {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				MY_PRIVATE_KEY, APIGW_PUBLIC_KEY, MY_PUB_KEY, password);

		EnterpriseBranchRecoverGroupinfoRequestV1 request = new EnterpriseBranchRecoverGroupinfoRequestV1();
		//地址(如果是内部API，地址需要使用http而不是https)
		request.setServiceUrl("https://ip:port/api/enterprise/branch/recover/groupinfo/V1");
		EnterpriseBranchRecoverGroupinfoRequestV1Biz bizContent = new EnterpriseBranchRecoverGroupinfoRequestV1Biz();
		bizContent.setGroupId("1111111");
		AlterInfo alterInfo = bizContent.new AlterInfo();
		alterInfo.setBzSwitch("0");
		bizContent.setAlterInfo(alterInfo);
		ChanCommV10 chanCommV10 = bizContent.new ChanCommV10();
		chanCommV10.setSerialno("111111111");
		bizContent.setChanCommV10(chanCommV10);

		request.setBizContent(bizContent);
		EnterpriseBranchRecoverGroupinfoResponseV1 response;

		Random rand = new Random();
		String msgId = rand.nextInt(99999) + "msg";

		try {
			Logger log = Logger.getLogger(EnterpriseBranchRecoverGroupinfoTest.class.getName());
			log.info("msgId:"+ msgId);
			response = client.execute(request, msgId);
			if (response.getReturnCode() == 0) {
				log.info("ReturnCode:"+ response.getReturnCode()+"\n Response:"+response.getReturnMsg());
			} else {
				log.info("ReturnCode:"+ response.getReturnCode()+"\n Response:"+response.getReturnMsg());
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}	
		
	}
}
