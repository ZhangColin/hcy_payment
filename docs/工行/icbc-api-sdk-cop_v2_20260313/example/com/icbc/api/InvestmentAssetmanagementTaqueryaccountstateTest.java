package com.icbc.api;

import com.icbc.api.request.InvestmentAssetmanagementTaqueryaccountstateRequestV1;
import com.icbc.api.request.InvestmentAssetmanagementTaqueryaccountstateRequestV1.FtasChannelTaqueryaccountstateRequestV1Biz;
import com.icbc.api.response.InvestmentAssetmanagementTaqueryaccountstateResponseV1;

import java.util.logging.Logger;

/**
 * @ClassName InvestmentAssetmanagementTaqueryaccountstateTest
 * @Description TA账户状态及份额司法冻结情况查询测试
 * @author: kfzx-wangyh1
 * @create: 2021-12-24 09:21
 **/
public class InvestmentAssetmanagementTaqueryaccountstateTest {
	/**
     * appId
     */
    protected static String APP_ID = "id";
    /**
     * 行外
     */
    protected static String BASE_URL = "http://ip:port/api";

	/**
     * 网关公钥
     */
    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16\n" +
            "bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL\n" +
            "T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP\n" +
            "3CiNgg0W6O3AGqwIDAQAB";
    /**
     * 应用私钥（与内管平台注册app时的“签名公钥”配对）
     */
    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";

    protected static Logger log = Logger.getLogger(InvestmentAssetmanagementTaqueryaccountstateTest.class.getName());

    public static void main(String[] args) throws Exception {
    	// api地址
        String URI = "investment/assetmanagement/taqueryaccountstate/V1";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, PRI_KEY, APIGW_PUBLIC_KEY);
        try {

			// 请求实体
			InvestmentAssetmanagementTaqueryaccountstateRequestV1 request = new InvestmentAssetmanagementTaqueryaccountstateRequestV1();
			 // bizContent实体
			FtasChannelTaqueryaccountstateRequestV1Biz bizContent = new FtasChannelTaqueryaccountstateRequestV1Biz();

			// 请求报文IN节点实体 设置属性
			bizContent.setAgencyNo("002");
			bizContent.setTaAcco("GY8002190");
			bizContent.setQueryType("1");

			// 请求实体 设置属性
			request.setServiceUrl(BASE_URL + "/" + URI);
			request.setBizContent(bizContent);

			log.info(request.getMethod());
			InvestmentAssetmanagementTaqueryaccountstateResponseV1 response = (InvestmentAssetmanagementTaqueryaccountstateResponseV1) client.execute(request);
			if (response.isSuccess()) {
				log.info(response.getReturnCode() + ";"+ response.getReturnMsg() + ";" + response.getReturnContent());
				log.info("success");
			} else {
				log.info(response.getMsgId()  + ";" + response.getReturnCode()
                        + ";" + response.getReturnMsg() +";" + response.getReturnContent());
				log.info("error");
			}

		} catch (Exception e) {
			e.printStackTrace();
			log.info(e.getMessage());
		}
	}
}
