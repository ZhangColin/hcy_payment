...
import com.icbc.api.request.EcspPackageOperaRequestV1;
import com.icbc.api.response.EcspPackageOperaResponseV1;

/**
 * @author kfzx-dengzp2
 */
public class EcspPackageOperaV1Test {
    // 1、网关公钥
    ...
    public static void main(String[] args) throws Exception {
        ...
        EcspPackageOperaRequestV1 request = new EcspPackageOperaRequestV1();

        // 4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://ip:port/api/ecsp/oc/prop/car/packageOpera/V1");
        // 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        EcspPackageOperaRequestV1.EcspPackageOperaRequestV1Biz bizContent = new EcspPackageOperaRequestV1.EcspPackageOperaRequestV1Biz();
        bizContent.setCorpId("0000000001");
        bizContent.setPackageId(100001);
        bizContent.setPackageName("测试套餐");
        bizContent.setParkingId("01");
        bizContent.setPackageAmount(1000L);
        bizContent.setFreeMinute(10);
        bizContent.setPrice(500L);
        bizContent.setMaxFee(10000L);
        bizContent.setType("1");
        request.setBizContent(bizContent);

        EcspPackageOperaResponseV1 response;
        try {
            response = client.execute(request, "23344544454120");// msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
