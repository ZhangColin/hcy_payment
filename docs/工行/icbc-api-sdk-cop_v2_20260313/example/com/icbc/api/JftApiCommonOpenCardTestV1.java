package com.icbc.ndf.jft.ncms.controller;


import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.JftApiCommonOpenCardRequestV1;
import com.icbc.api.response.JftApiCommonMcOpenCardResponseV1;


public class JftApiPaySplitAccountTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "11000000000000010601";

	
	public static void main(String[] args) {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8",
				"json",APIGW_PUBLIC_KEY,"AES","Nttz4Bs/k8eqWGn1c6pOMg==", "", "");

		JftApiCommonOpenCardRequestV1 request = new JftApiCommonOpenCardRequestV1();
		request.setServiceUrl("https://ip:port:443/api/jft/api/common/openMcard/V1");
		JftApiCommonOpenCardRequestV1.JftApiCommonOpenCardRequestV1Biz bizContent = new JftApiCommonOpenCardRequestV1.JftApiCommonOpenCardRequestV1Biz();
		bizContent.setAppId(APP_ID);//平台商户标识
		bizContent.setCardName("管家卡名称");//管家卡名称
		bizContent.setBizId("12018");//业务编号
		bizContent.setCardNameMode("1111021");//卡名称规则
		bizContent.setMemberId("00");//会员编号
		bizContent.setRegisterTime("");//请求时间
		bizContent.setUserId("");//用户编号
		bizContent.setSettleAccountNo("465465465464");//结算账号
		bizContent.setType("");//开卡类型
		bizContent.setSubVendorId("");//子商户编号
		bizContent.setRegisterType("");//注册类型
		bizContent.setIsRealTime("");//实时状态
		bizContent.setSenceType("");//场景类型

		request.setBizContent(bizContent);

		JftApiCommonMcOpenCardResponseV1 response;
		try {
			response = client.execute(request, System.currentTimeMillis()+"");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
			if (response.isSuccess()) {
				//6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("response:" + response);
			} else {
				//失败
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("ReturnMsg:"+response.getReturnMsg());
			}		
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}

