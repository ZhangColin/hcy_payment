package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MybankAccountFarmSyncRefundNotifyRequestV1;
import com.icbc.api.response.MybankAccountFarmSyncRefundNotifyResponseV1;

/**
 * 涓夊啘APP閫氱敤鏌ヨ鎺ュ彛
 * 
 * 璇峰晢鎴烽厤缃甋DK涓殑lib鏂囦欢澶逛腑鐨刯ar鍖呭悗锛屾牴鎹娉?1-6鎻愮ず杩涜鏁版嵁鏇挎崲
 */
public class MybankAccountFarmSyncRefundNotifyTestV1 {
	         

protected static String APP_ID = "10000000000004096993";
	

	private  static String BASE_URL = "http://ip:port/api";
	

	protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
     





	public static void main(String[] args) throws Exception {
		String URI = "mybank/account/farm/syncRefundNotify/V1";

		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
		try {
			MybankAccountFarmSyncRefundNotifyRequestV1 request = new MybankAccountFarmSyncRefundNotifyRequestV1();
			MybankAccountFarmSyncRefundNotifyRequestV1.MybankAccountFarmSyncRefundNotifyRequestBizV1 requestBizV1 = new MybankAccountFarmSyncRefundNotifyRequestV1.MybankAccountFarmSyncRefundNotifyRequestBizV1();

			requestBizV1.setfSeqNo("1234567890");
			requestBizV1.setTransDate("20220202");
			requestBizV1.setCorpCode("icbc");
			requestBizV1.setParamType("1");
			requestBizV1.setDetail("{\"billId\":\"YJYprotocolId84148406\",\"projectId\":\"YJYproductId555\",\"propertyId\":\"LYJYgovId123\",\"sellerId\":\"C-YJYbusiClientId\",\"busiNo\":\"YJYhuabo84148407\",\"refundType\":\"0\",\"refundAmt\":\"6\"}");

			request.setServiceUrl(BASE_URL +"/"+ URI);
			request.setBizContent(requestBizV1);
			
	        System.out.println(JSON.toJSONString(request));

	        MybankAccountFarmSyncRefundNotifyResponseV1 response = client.execute(request);
			if (response.isSuccess()) {
				System.out.println("success");
				System.out.println(JSONObject.toJSONString(response));
			} else {
				System.out.println("error");
				System.out.println(JSONObject.toJSONString(response));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

	}

	
}


