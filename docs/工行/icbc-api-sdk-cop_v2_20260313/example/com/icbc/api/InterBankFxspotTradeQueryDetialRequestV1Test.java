package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.InterBankFxspotTradeQueryDetialRequestV1;
import com.icbc.api.request.InterBankFxspotTradeQueryDetialRequestV1.InterBankFxspotTradeQueryDetialRequestBiz;
import com.icbc.api.response.InterBankFxspotTradeQueryDetialResponseV1;

public class InterBankFxspotTradeQueryDetialRequestV1Test {
	public static void main(String[] args) {
        // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值

        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "10000000000004096790";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //4、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
        //5、设置请求对象
        InterBankFxspotTradeQueryDetialRequestV1 request = new InterBankFxspotTradeQueryDetialRequestV1();

        //6、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("https://ip:port/api/interbank/fxspot/trade/queryDetial/V1");
        //7、设置业务参数
        InterBankFxspotTradeQueryDetialRequestBiz bizContent = new InterBankFxspotTradeQueryDetialRequestBiz();
        bizContent.setBaseDirection("1");
        bizContent.setBeDealDate("20230328");
        bizContent.setBegNum("1");
        bizContent.setCino("");
        bizContent.setDealCcy("");
        bizContent.setEnDealDate("20230330");
        bizContent.setFetchNum("30");
        bizContent.setProductId("");
        bizContent.setTenor("");
        request.setBizContent(bizContent);

        InterBankFxspotTradeQueryDetialResponseV1 response;
        try {
            System.out.println("request:"+ JSON.toJSONString(request));
            response = client.execute(request);
            System.out.println("response:" + JSON.toJSONString(response));
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
