...
import com.icbc.api.request.JftApiVendorInfoCloudcheckRequestV1;
import com.icbc.api.response.JftApiVendorInfoCloudcheckResponseV1;

import static com.icbc.api.request.JftApiVendorInfoCloudcheckRequestV1.JftApiVendorInfoCloudcheckRequestV1Biz;
 ...
 */
public class JftApiVendorInfoCloudcheckTestV1 {

	//1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//2、appid
	protected static final String APP_ID = "10000000000004094718";
	...
	public static void main(String[] args) throws Exception {
		...
		JftApiVendorInfoCloudcheckRequestV1 request = new JftApiVendorInfoCloudcheckRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/jft/api/vendor/info/cloudcheck/V1");
		
		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		JftApiVendorInfoCloudcheckRequestV1Biz bizContent = new JftApiVendorInfoCloudcheckRequestV1Biz();
		bizContent.setAppId("10000000000000081011");
		bizContent.setOutVendorId("01329555342");
		bizContent.setOutUserId("013285124231");
		bizContent.setOutUpperVendorId("10000000000000081011");

		request.setBizContent(bizContent);

		JftApiVendorInfoCloudcheckResponseV1 response;
		try {
			response =  (JftApiVendorInfoCloudcheckResponseV1) client.execute(request, System.currentTimeMillis()+"");
			if (response.isSuccess()) {
				//6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("response:" +response.getCloudCheckUrl());
			} else {
				//失败
				System.out.println("ReturnCode:"+response.getReturnCode());
