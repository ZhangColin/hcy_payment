package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiGetAcctInfoOutRequestV1;
import com.icbc.api.request.JftApiGetAcctInfoOutRequestV1.JftApiGetAcctInfoOutRequestV1Biz;
import com.icbc.api.response.JftApiGetAcctInfoOutResponseV1;

public class JftApiGetAcctInfoOutTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	protected static final String APP_ID = "10000000000000004964";

	@SuppressWarnings("unchecked")

	public void test_cop() {
		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","eyiSiHvFqYFLu/AZDCt+4w==", "", "");
		JftApiGetAcctInfoOutRequestV1 request = new JftApiGetAcctInfoOutRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/api/vacct/list/query/V1");
		JftApiGetAcctInfoOutRequestV1Biz bizContent = new JftApiGetAcctInfoOutRequestV1Biz();
		bizContent.setAcctType("00");
		bizContent.setAppId("1201803300000000631");
		bizContent.setOutVendorId("");
		bizContent.setPage("");
		bizContent.setPageSize("");
		bizContent.setStatus("00");
		bizContent.setVendorName("");
		bizContent.setVersion("1.1.1");//设置接口访问版本号

		request.setBizContent(bizContent);
		JftApiGetAcctInfoOutResponseV1 response;
		try {
			response = (JftApiGetAcctInfoOutResponseV1) client.execute(request, System.currentTimeMillis()+"");
		
		System.out.println(JSONObject.toJSONString(response));
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
	
}

