package com.icbc.api;

import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.icbc.api.request.JftUiPayAddLoanOrderRequestV1;
import com.icbc.api.request.JftUiPayAddLoanOrderRequestV1.JftUiPayAddLoanOrderRequestV1Biz;

public class JftUiPayAddLoanOrderTestV1 extends HttpServlet {
	private static final long serialVersionUID = 1L;

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		doPost(req, resp);
	}

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException {

		try {
			String APP_ID = "100000000000045510";
			String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
			UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
			JftUiPayAddLoanOrderRequestV1 request = new JftUiPayAddLoanOrderRequestV1();
			
			request.setServiceUrl("https://ip:port/ui/jft/ui/pay/add/loan/order/V1");
			JftUiPayAddLoanOrderRequestV1Biz bizContent = new JftUiPayAddLoanOrderRequestV1Biz();
			bizContent.setAppId(APP_ID);
			bizContent.setOutVendorId("20180426");
			bizContent.setOutUserId("M201801112326");
			bizContent.setOutOrderId("20210617101211");
			bizContent.setPayAmount("0.02");
			bizContent.setNotifyUrl("http://www.aa.com");
			bizContent.setJumpUrl("http://www.bb.com");
			bizContent.setGoodsId("1982345");
			bizContent.setGoodsName("商品信息");
			bizContent.setTrxIp("ip:port");
			bizContent.setTrxIpCity("北京");
			bizContent.setTrxChannel("02");
			bizContent.setMac("a3-b4-c5");
			bizContent.setVarNote("3212");
			bizContent.setOutBusinessCode("012345");
			bizContent.setUserType("02");
			
			request.setBizContent(bizContent);

			resp.setHeader("Content-Type", "text/html;charset="
					+ IcbcConstants.CHARSET_UTF8);
			PrintWriter out = resp.getWriter();
			System.out.println(client.buildPostForm(request));

			out.write("<html>");
			out.write("<head>");
			out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset="
					+ IcbcConstants.CHARSET_UTF8 + "\">");
			out.write("</head>");
			out.write("<body>");
			out.write(client.buildPostForm(request));
			out.write("</body>");
			out.write("</html>");

		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
