package com.icbc.api.test;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BcssOrderCenterOrderCreateRequestV1;
import com.icbc.api.response.BcssOrderCenterOrderCreateResponseV1;

import java.math.BigDecimal;
import java.util.ArrayList;
import java.util.List;

/**
 * @Author jieyd
 * @Date 2021/11/23 15:17
 */
public class BcssOrderCenterOrderCreateTestV1 {
    String puv = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----";
    String prv = "-----请使用正确的APP私钥----- ";
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000004097499";
    protected static final String RSA = "RSA";
    protected static final String RSA2 = "RSA2";

    public static void main(String[] args) throws Exception {
        // TODO Auto-generated method stub
        BcssOrderCenterOrderCreateTestV1 testV1 = new BcssOrderCenterOrderCreateTestV1();
        testV1.orderCreate();

    }

    public void orderCreate() throws IcbcApiException {
        BcssOrderCenterOrderCreateRequestV1 request = new BcssOrderCenterOrderCreateRequestV1();
//        request.setServiceUrl("https://ip:port/bcss/order/pay/V1");
        request.setServiceUrl("http://ip:port/api/bcss/order1/pay/V1");
//        request.setServiceUrl("http://ip:port/api/bcss/order/pay/V1");
//        request.setServiceUrl("http://ip:port/api/cardbusiness/mer/member/scenicfaceresultget/V1");
//        request.setServiceUrl("http://ip:port/api/bcss/order/pay/V1");
//        request.setServiceUrl("http://ip:port/api/bcss/order/pay/V1");
        BcssOrderCenterOrderCreateRequestV1.BcssOrderCenterOrderCreateRequestBizV1 requestBizV1 = new BcssOrderCenterOrderCreateRequestV1.BcssOrderCenterOrderCreateRequestBizV1();
        //majorOrderInfo
        BcssOrderCenterOrderCreateRequestV1.OrderMajorOrderInfoDto majorOrderInfoDto = new BcssOrderCenterOrderCreateRequestV1.OrderMajorOrderInfoDto();
        majorOrderInfoDto.setCorpId("2000000049");
        majorOrderInfoDto.setClearFlag(0);
        //红利商圈
        majorOrderInfoDto.setStartIndustry("25");
        majorOrderInfoDto.setCustId("1008611");
        majorOrderInfoDto.setDataSrc(4);
        majorOrderInfoDto.setTotalAmt(BigDecimal.TEN);
        requestBizV1.setMajorOrderInfo(majorOrderInfoDto);

        //subOrderInfo--子订单集合
        List arrayList = new ArrayList<>();
        BcssOrderCenterOrderCreateRequestV1.OrderSubOrderInfoDto subOrderInfoDto = new BcssOrderCenterOrderCreateRequestV1.OrderSubOrderInfoDto();
        subOrderInfoDto.setShopId("123456");
        subOrderInfoDto.setSubTotalAmt(BigDecimal.TEN);
        requestBizV1.setSubOrderInfo(arrayList);

        request.setBizContent(requestBizV1);
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, RSA2, prv, APIGW_PUBLIC_KEY);
        BcssOrderCenterOrderCreateResponseV1 response = client.execute(request);
        System.out.println(JSONObject.toJSONString(response));
    }
}
