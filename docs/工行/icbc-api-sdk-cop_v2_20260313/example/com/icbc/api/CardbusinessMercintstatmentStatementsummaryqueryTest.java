package com.icbc.api;

import java.util.List;

import com.icbc.api.request.CardbusinessMercintstatmentStatementsummaryqueryRequestV1;
import com.icbc.api.request.CardbusinessMercintstatmentStatementsummaryqueryRequestV1.CardbusinessMercintstatmentStatementsummaryqueryRequestV1Biz;
import com.icbc.api.request.CardbusinessMercintstatmentStatementsummaryqueryRequestV1.CardbusinessMercintstatmentStatementsummaryqueryRequestV1Pub;
import com.icbc.api.request.CardbusinessMercintstatmentStatementsummaryqueryRequestV1.CardbusinessMercintstatmentStatementsummaryqueryRequestV1In;
import com.icbc.api.response.CardbusinessMercintstatmentStatementsummaryqueryResponseV1;
import com.icbc.api.response.CardbusinessMercintstatmentStatementsummaryqueryResponseV1.CardbusinessMercintstatmentStatementsummaryqueryResponseV1Pub;
import com.icbc.api.response.CardbusinessMercintstatmentStatementsummaryqueryResponseV1.CardbusinessMercintstatmentStatementsummaryqueryResponseV1AppStatV10;
import com.icbc.api.response.CardbusinessMercintstatmentStatementsummaryqueryResponseV1.CardbusinessMercintstatmentStatementsummaryqueryResponseV1Out;
import com.icbc.api.response.CardbusinessMercintstatmentStatementsummaryqueryResponseV1.CardbusinessMercintstatmentStatementsummaryqueryResponseV1Total;
import com.icbc.api.response.CardbusinessMercintstatmentStatementsummaryqueryResponseV1.CardbusinessMercintstatmentStatementsummaryqueryResponseV1Detail;

import java.util.logging.Logger;

public class CardbusinessMercintstatmentStatementsummaryqueryTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

//	protected static final String APP_ID = "10000000000000002116";

	public static void main(String[] args) throws IcbcApiException {
//		String APP_ID = "10000000000000002116";
		String APP_ID = args[0];
		
		String castr = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				"",
				APIGW_PUBLIC_KEY, castr, "12345678");
		
		CardbusinessMercintstatmentStatementsummaryqueryRequestV1 request = new CardbusinessMercintstatmentStatementsummaryqueryRequestV1();
		request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cardbusiness/mercintstatment/statementsummaryquery/V1");

		CardbusinessMercintstatmentStatementsummaryqueryRequestV1Biz bizContent = new CardbusinessMercintstatmentStatementsummaryqueryRequestV1Biz();
		
		CardbusinessMercintstatmentStatementsummaryqueryRequestV1Pub pub = new CardbusinessMercintstatmentStatementsummaryqueryRequestV1Pub();
		pub.setCode("29132");
		pub.setZone("0200");
		pub.setFr("0");
		pub.setDt("2018-12-26");
		
		CardbusinessMercintstatmentStatementsummaryqueryRequestV1In in = new CardbusinessMercintstatmentStatementsummaryqueryRequestV1In();
		in.setZoneno("0200");
		in.setMerno("123412413131000");
		in.setAgreeno("");
		in.setDateb("2017-07-06");
		in.setDatee("2017-07-06");
		in.setQuerytype("1");
		in.setPage("1");
		
		bizContent.setPub(pub);
		bizContent.setIn(in);
		
		request.setBizContent(bizContent);

		CardbusinessMercintstatmentStatementsummaryqueryResponseV1 response = client.execute(request, "msgId");
		Logger log = Logger.getLogger(CardbusinessMercintstatmentStatementsummaryqueryTest.class.getName());

		if (response.isSuccess()) {
			// 接收成功
//			CardbusinessMercintstatmentStatementsummaryqueryResponseV1Pub RetPub = response.getPub();
			CardbusinessMercintstatmentStatementsummaryqueryResponseV1AppStatV10 RetAppStatV10 = response.getAppStatV10();
			CardbusinessMercintstatmentStatementsummaryqueryResponseV1Out RetOut = response.getOut();
			if(RetAppStatV10.getReturnCode().equals("00000000")){
				//查询成功 获取返回结果
				List<CardbusinessMercintstatmentStatementsummaryqueryResponseV1Detail> resultList = RetOut.getDetail();
				//获取结果集中字段
				for(CardbusinessMercintstatmentStatementsummaryqueryResponseV1Detail detail:resultList){
					//获取信息.....
					detail.getProdname();
				}
			} else {
				//查询失败
				log.info(RetAppStatV10.getReturnCode());
			}
		} else {
			// 失败
			log.info(response.getReturnMsg());
		}
	}
}
