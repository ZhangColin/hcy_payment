package com.icbc.api.TEST;


import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.JftApiPayB2bPayMedicalRefundRequestV1;
import com.icbc.api.response.JftApiPayB2bPayMedicalRefundResponseV1;

import java.util.ArrayList;
import java.util.List;

public class JftApiPayB2bPayMedicalRefundTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000205510";
											
	protected static final String AES_Key = "5xGJdh7qb+B95SUoxDlatg==";
	public static void main(String[] args) {


		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8",
				"json",APIGW_PUBLIC_KEY,"AES",AES_Key, "", "");
		JftApiPayB2bPayMedicalRefundRequestV1 request=new JftApiPayB2bPayMedicalRefundRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/api/pay/b2bpay/medicalTransfer/V1");
		JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz
				bizContent=new JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz();
		bizContent.setAppId("10000000000007354589");
		bizContent.setOutUserId("71810240115");
		bizContent.setNotifyUrl("http://www.ceshi.com");
		bizContent.setOutVendorId("3324343221232");
		bizContent.setBatchId("100000222334");
		bizContent.setRefundAmount("11.11");
		bizContent.setRefundType("17");
		bizContent.setSceneType("05");
		bizContent.setTradeTime("2024-07-11 14:15:11");
		bizContent.setSummary("111");
		List<JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz.ProjectInfo> projectInfoList = new ArrayList<>();
		JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz.ProjectInfo projectInfo = new JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz.ProjectInfo();
		projectInfo.setProjectId("12232132144");
		projectInfo.setTrxAmount("100.00");
		List<JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz.OriTrx> oriTrxList = new ArrayList<>();
		JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz.OriTrx oriTrx = new JftApiPayB2bPayMedicalRefundRequestV1.JftApiPayB2bPayMedicalRefundRequestV1Biz.OriTrx();
		oriTrx.setOriAmount("100");
		oriTrx.setDetailNo("3123213333321");
		oriTrx.setOriSerialNo("3232132333333111");
		oriTrxList.add(oriTrx);
		projectInfo.setOriTrxList(oriTrxList);
		projectInfoList.add(projectInfo);
		bizContent.setProjectInfoList(projectInfoList);
		bizContent.setImageFile("dashdkldasndkl2nnndsla");
		bizContent.setRecAcctNum("10000000000007354581");
		bizContent.setRecAcctName("发火点发货十六分华盛顿林肯");
		bizContent.setPayAcctNum("10000000000007354581");
		request.setBizContent(bizContent);

		JftApiPayB2bPayMedicalRefundResponseV1 response;
		try {
			response = client.execute(request, System.currentTimeMillis()+"");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
			if (response.getReturnCode()==10100000) {
				//6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("response:" + response);
			} else {
				//失败
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("ReturnMsg:"+response.getReturnMsg());
			}		
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}

