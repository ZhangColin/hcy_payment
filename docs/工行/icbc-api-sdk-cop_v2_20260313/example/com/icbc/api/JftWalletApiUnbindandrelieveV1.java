package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftWalletApiUnbindandrelieveRequestV1;
import com.icbc.api.request.JftWalletApiUnbindandrelieveRequestV1.UnbindAndRelieveInBiz;
import com.icbc.api.response.JftWalletApiUnbindandrelieveResponseV1;

/**
 * 解绑及解协议测试类
 * @author kfzx-gaona
 *
 */
public class JftWalletApiUnbindandrelieveV1 {
	String MY_PRIVATE_KEY2 = "-----请使用正确的APP私钥-----";
	String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000012090";

	// 解绑及解协议

	public void test_cop1() {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA", MY_PRIVATE_KEY2, "UTF-8", "json",
				APIGW_PUBLIC_KEY, "AES", "zl+5SxQAx6P7bjpqRu693Q==", "", "");

		JftWalletApiUnbindandrelieveRequestV1 request = new JftWalletApiUnbindandrelieveRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/wallet/api/unbindandrelieve/V1");// 11月
		UnbindAndRelieveInBiz bizContent = new UnbindAndRelieveInBiz();
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		bizContent.setAppId("10000000000000012090");// 商户appid
		bizContent.setCorpSerno("AasD10999988889876");// 每次交易改流水号
		bizContent.setMediumId("111117778855555");//工行联名卡号
		bizContent.setBindMedium("6222080200022954104");// 绑定的I类卡号
		bizContent.setCorpDate("2017-03-15");
		bizContent.setCorpMediumId("1000000001854");// 合作方介质号 和userid一样
		bizContent.setMobileNo("18952000005");// 手机号
		bizContent.setOutServiceCode("unbinding");// 固定不改
		bizContent.setTrxAccDate("2017-03-15");
		bizContent.setTrxAccTime("13:30:01");
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		System.out.println(bizContent);
		request.setBizContent(bizContent);

		JftWalletApiUnbindandrelieveResponseV1 response;
		try {
			response = (JftWalletApiUnbindandrelieveResponseV1) client.execute(request, System.currentTimeMillis() + "");
			System.out.println("~~~~~~~~~");
			System.out.println(JSONObject.toJSONString(response));
			System.out.println("~~~~~~~~~");
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}

}
