package com.icbc.api;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiVacctNovelAcctOpenCardLogoutRequestV1;
import com.icbc.api.request.JftApiVacctNovelAcctOpenCardLogoutRequestV1.JftApiVacctNovelAcctOpenCardLogoutRequestV1Biz;
import com.icbc.api.response.JftApiVacctNovelAcctOpenCardLogoutResponseV1;
import org.junit.Test;

public class JftApiVacctNovelAcctOpenCardLogoutTest {
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000205520";

    @SuppressWarnings("unchecked")

    public void test_cop() {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");
        JftApiVacctNovelAcctOpenCardLogoutRequestV1 request = new JftApiVacctNovelAcctOpenCardLogoutRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/vacct/novel/acct/open/card/logout/V1");
        JftApiVacctNovelAcctOpenCardLogoutRequestV1Biz bizContent = new JftApiVacctNovelAcctOpenCardLogoutRequestV1Biz();
        bizContent.setAppId("10000000000000286524");
        bizContent.setOutSubVendorId("05280940");
        bizContent.setAcctNo("2021070110590000000044");
        request.setBizContent(bizContent);
        JftApiVacctNovelAcctOpenCardLogoutResponseV1 response;
        try {
            response = (JftApiVacctNovelAcctOpenCardLogoutResponseV1) client.execute(request, System.currentTimeMillis()+"");

            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
