package com.icbc.api;

import com.icbc.api.request.MybankEnterpriseGyjrQrybanknoteinfoRequestV1;
import com.icbc.api.request.MybankEnterpriseGyjrQrybanknoteinfoRequestV1.MybankEnterpriseGyjrQrybanknoteinfoRequestBizV1;
import com.icbc.api.request.MybankEnterpriseGyjrQrybanknoteinfoRequestV1.TransCommV10;
import com.icbc.api.response.MybankEnterpriseGyjrQrybanknoteinfoResponseV1;

public class MybankEnterpriseGyjrQrybanknoteinfoTest {

    protected static final String BASE_URL = "[需替换]工行API地址"; // https://xxxx/api/

    protected static final String APP_ID = "11000000000000001051";
    // 应用私钥(变量值需替换)
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    // API网关公钥(变量值需替换)
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16" +
            "bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL" +
            "T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP" +
            "3CiNgg0W6O3AGqwIDAQAB";

    public static void main(String[] args) throws Exception {
        String serviceUri = BASE_URL + "mybank/enterprise/gyjr/qrybanknoteinfo/V1";
        IcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        MybankEnterpriseGyjrQrybanknoteinfoRequestBizV1 bizContent =
        new MybankEnterpriseGyjrQrybanknoteinfoRequestBizV1();
        MybankEnterpriseGyjrQrybanknoteinfoRequestV1 request =
        new MybankEnterpriseGyjrQrybanknoteinfoRequestV1();
        TransCommV10 transCommV10 = new TransCommV10();
        // 请对照接口文档用bizContent.setXxx()方法对业务上送数据进行赋值
        transCommV10.setEventNo("1234567678");
        transCommV10.setProtocolCode("IFSBBB202107303232");
        transCommV10.setCodeType("100");
        transCommV10.setBusinessItems("1");
        transCommV10.setQueryScope("2");
        transCommV10.setPageIndex("1");
        transCommV10.setPageSize("10");
        bizContent.setTransCommV10(transCommV10);
        request.setServiceUrl(serviceUri);
        request.setBizContent(bizContent);
        String validate = ExceptionUtils.validate(bizContent);
        if (null == validate) {
        MybankEnterpriseGyjrQrybanknoteinfoResponseV1 response = client.execute(request);
            if (response.isSuccess()) { // 业务成功处理
                System.out.println("success");
            } else { // 失败
                System.out.println("error");
            }
        } else { // 校验不通过
            System.err.println(validate);
        }
    }
}
