package com.icbc.api;

import com.icbc.api.request.MybankFarmFarmplatfCorpAuthVerifyRequestV1;
import com.icbc.api.response.MybankFarmFarmplatfCorpAuthVerifyResponseV1;

public class MyBankFarmFarmPlatfCorpAuthVerifyTest {
	protected static String APP_ID = "10000000000004096993"; //10000000000004096993
    protected static String BASE_URL = "http://ip:port/api";

    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
    		
    public static void main(String[] args){

        String URI = "/mybank/farm/farmplatf/corpAuthVerify/V1";
        try {
        	DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
        	
        	MybankFarmFarmplatfCorpAuthVerifyRequestV1 request = new MybankFarmFarmplatfCorpAuthVerifyRequestV1();
        	MybankFarmFarmplatfCorpAuthVerifyRequestV1.MybankFarmFarmplatfCorpAuthVerifyResponseV1Biz bizContent
                    = new MybankFarmFarmplatfCorpAuthVerifyRequestV1.MybankFarmFarmplatfCorpAuthVerifyResponseV1Biz();
        	bizContent.setCorpCode("xxx");
        	bizContent.setParamType("1");
        	bizContent.setRequestId("1232321321321321321321321");
        	bizContent.setDetail("{\"verifyinfo\":\"2ae007570b6e8b117719c757c83496cac8156f6ecfe5b0b69089f7a39536e2c7dc1a351\"}");
       
            request.setBizContent(bizContent);
            request.setServiceUrl(BASE_URL+URI);

            MybankFarmFarmplatfCorpAuthVerifyResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
            	System.out.println("Success");
                System.out.println("returnCode:"+response.getReturnCode());
                System.out.println("returnMsg:" +response.getReturnMsg());
                System.out.println("requestId:" +response.getRequestId());
                System.out.println("detail:" +response.getDetail());
            }else {
            	System.out.println("Fail");
                System.out.println("returnCode:"+response.getReturnCode());
                System.out.println("returnMsg:" +response.getReturnMsg());
                System.out.println("requestId:" +response.getRequestId());
                System.out.println("detail:" +response.getDetail());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
