package com.icbc.api;

import static org.junit.Assert.assertNotNull;

import java.text.SimpleDateFormat;
import java.util.Date;

import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

import com.icbc.api.crypt.AES;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BcssActivityCoinConsumeDetailRequestV1;
import com.icbc.api.request.BcssActivityCoinConsumeDetailRequestV1.BcssActivityCoinConsumeDetailRequestBizV1;
import com.icbc.api.response.BcssActivityCoinConsumeDetailResponseV1;
import org.apache.commons.logging.Log;
public class BcssActivityCoinConsumeDetailTestV1 {

	private static final Log logger = LogFactory.getLog(BcssActivityCoinConsumeTestV1.class);
	// MY_PRIVATE_KEY
		protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

		// APIGW_PUBLIC_KEY
		protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

		// APP_ID
		protected static final String APP_ID = " ";//10000000000004096876

		// 10000000000004096876
		public static void main(String[] args) throws IcbcApiException {
			
			//http://ip:port:8005/api-member/bcssGetExchangeNoteService/query?corpId=2000000067
			//&personId=d9xqFJMpYN1/7QVR6Ae1UqN94QsIlUPCcSiBtgALJE3/4TMHu6u/NDllWuBVyhnr
			//&timeStamp=2020-02-02 12:00:00&trxSrc=1
				
			BcssActivityCoinConsumeDetailRequestV1 request = new BcssActivityCoinConsumeDetailRequestV1();
			request.setServiceUrl("http://api.com/api/bcss/activity/getActivitiesDetail/V1");
			BcssActivityCoinConsumeDetailRequestBizV1 hrb = new BcssActivityCoinConsumeDetailRequestBizV1();

			hrb.setCorpId("2000000067");//2000000067
			//String personId = "Mg9Jz18sUGEmDSc/Z6rzxJpl3kUbASmeB0307mw1A3vOIengQdtb3CcvUrDo1i6p";
			String personId = "d9xqFJMpYN1/7QVR6Ae1UqN94QsIlUPCcSiBtgALJE3/4TMHu6u/NDllWuBVyhnr" ;
			hrb.setPersonId(personId);
			hrb.setTrxSrc("1");
			hrb.setQuerySeason("202001");
			
		 
			//System.out.println(AES.aesDecrypt(personId, "645r6z1azEItiR+ghwF4+g=="));
			
			String timeStmp = new SimpleDateFormat("yyyy-MM-dd hh:mm:ss").format(new Date());
			hrb.setTimeStamp(timeStmp);
			request.setBizContent(hrb);

			DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
			BcssActivityCoinConsumeDetailResponseV1 response = client.execute(request);
			logger.info(JSONObject.toJSONString(response));
			 
		}
}
