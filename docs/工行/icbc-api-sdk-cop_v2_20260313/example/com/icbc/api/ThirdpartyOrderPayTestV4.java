...
import com.icbc.api.request.ThirdpartyOrderPayRequestV4;
import com.icbc.api.request.ThirdpartyOrderPayRequestV4.ThirdpartyOrderPayRequestV4Biz;
 ...
 */
public class ThirdpartyOrderPayTestV4 {
	...
	public void order(HttpServletRequest request1, HttpServletResponse response) throws Exception {
		...
		ThirdpartyOrderPayRequestV4 request = new ThirdpartyOrderPayRequestV4();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://ip:port/ui/thirdparty/order/pay/V4");
		
		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		ThirdpartyOrderPayRequestV4Biz bizContent = new ThirdpartyOrderPayRequestV4Biz();
		...
		bizContent.setAutoSubmitFlag("1");
		bizContent.setGoodsName("商品001");// 该字段非必输项
		bizContent.setAllPointsFlag("0");// 该字段非必输项
		bizContent.setGoodPoints("");// 该字段非必输项
		bizContent.setInstallmentFlag("1");// 该字段非必输项
		bizContent.setInstallmentTimes("3");// 该字段非必输项
		bizContent.setCardType("111");// 该字段非必输项
		
		request.setBizContent(bizContent);

		System.out.println(client.buildPostForm(request,"msgId"));//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
