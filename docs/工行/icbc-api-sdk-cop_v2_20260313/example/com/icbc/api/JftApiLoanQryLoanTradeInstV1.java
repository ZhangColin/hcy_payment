package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiLoanQryLoanTradeInstRequestV1;
import com.icbc.api.request.JftApiLoanQryLoanTradeInstRequestV1.JftApiLoanQryLoanTradeInstRequestV1Biz;
import com.icbc.api.response.JftApiLoanQryLoanTradeInstResponseV1;
import org.junit.Test;

public class JftApiLoanQryLoanTradeInstV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "xxx205520";

    @SuppressWarnings("unchecked")

    public void test_cop() {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");
        JftApiLoanQryLoanTradeInstRequestV1 request = new JftApiLoanQryLoanTradeInstRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/loan/qryloantradeinst/V1");
        JftApiLoanQryLoanTradeInstRequestV1Biz bizContent = new JftApiLoanQryLoanTradeInstRequestV1Biz();
        bizContent.setAppId(APP_ID);
        bizContent.setOutSubVendorId(APP_ID);
        bizContent.setOutSerialNo("20190801xxxxx0010100");
        bizContent.setTradeType("00");
        bizContent.setLoanType("1");
        bizContent.setOutUserId("");
        bizContent.setVersion("1.0.0");//设置接口访问版本号

        request.setBizContent(bizContent);
        JftApiLoanQryLoanTradeInstResponseV1 response;
        try {
            response = (JftApiLoanQryLoanTradeInstResponseV1) client.execute(request, System.currentTimeMillis()+"");

            //System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
				assertEquals(true, response.isSuccess());
                // 业务成功处理
                //System.out.println(response.getReturnCode());
            } else {
                // 失败
				assertEquals(true, response.isSuccess());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
