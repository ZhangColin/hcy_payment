package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.DepositFixedPurchaseRequestV1;
import com.icbc.api.request.DepositFixedPurchaseRequestV1.DepositFixedPurchaseRequestV1Biz;
import com.icbc.api.response.DepositFixedPurchaseResponseV1;

public class DepositFixedPurchaseTest{
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		DepositFixedPurchaseRequestV1 request = new DepositFixedPurchaseRequestV1();
		DepositFixedPurchaseRequestV1Biz bizContent = new DepositFixedPurchaseRequestV1Biz();
		request.setServiceUrl("ip:port/api/deposit/fixed/purchase/V1");
		bizContent.setAppId("corpJd1234"); //合作方机构编号
		bizContent.setWorkdate("2017-03-14"); //合作方交易日期
		bizContent.setWorktime("00:00:00"); //合作方交易时间
		bizContent.setExternalEventNo("20202020"); //合作方交易单号
		bizContent.setOutServiceCode("fixedpurchase"); //外部服务代码
		bizContent.setSecretKey("h8ZujhDntpKRohwFmGXcnXygNjJHRQbyuVG3183uOdXI2fRgCXeDED9zOw5dO2JucVy7vZ19od7CSUxI2/sVOn6dPklVoaquVElgKRUPlr6D/cqLu25K7hJ3Pt/uOnx4qF/ykmO/IB2qS0rs/sp0Zw=="); 
		bizContent.setMediumId("6214760200000022233"); //工行卡号
		bizContent.setProdCode("10030000043"); //产品代码
		bizContent.setAmount("100000000"); //交易金额
		bizContent.setTerm("3"); //证件类型
		bizContent.setIdCode("11111111111111111X"); //证件号码
		bizContent.setName("张三"); //户名
		bizContent.setCashExFlag("0"); //钞汇标志
		bizContent.setCcy("1"); //币种
		bizContent.setLgldoctp("0"); //证件类型
        request.setBizContent(bizContent);
		DepositFixedPurchaseResponseV1 response = client.execute(request, "msgId");
		if (response.isSuccess() ) {
			//成功
			//response.getAccountBalance();
		} else {
			//失败
		}
	}
}
