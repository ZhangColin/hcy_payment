package com.icbc.api;

import java.util.List;

import org.junit.Test;

import com.icbc.api.request.B2cOrderQueryRequestV1;
import com.icbc.api.request.B2cOrderQueryRequestV1.B2cOrderQueryRequestV1Biz;
import com.icbc.api.request.B2cOrderQueryRequestV1.B2cOrderQueryRequestV1CommonInfo;
import com.icbc.api.request.B2cOrderQueryRequestV1.B2cOrderQueryRequestV1TradeInfo;
import com.icbc.api.response.B2cOrderQueryResponseV1;
import com.icbc.api.response.B2cOrderQueryResponseV1.B2cOrderQueryResponseV1CommonRetInfo;
import com.icbc.api.response.B2cOrderQueryResponseV1.B2cOrderQueryResponseV1DetailRetInfo;
import com.icbc.api.response.B2cOrderQueryResponseV1.B2cOrderQueryResponseV1TradeRetInfo;

public class B2cOrderQueryTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002116";


	public void test_CA() throws IcbcApiException {
		
		String castr = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				"",
				APIGW_PUBLIC_KEY, castr, "12345678");
		
		B2cOrderQueryRequestV1 request = new B2cOrderQueryRequestV1();
		request.setServiceUrl("http://ip:port/api/b2c/order/V1/query");

		B2cOrderQueryRequestV1Biz bizContent = new B2cOrderQueryRequestV1Biz();
		
		B2cOrderQueryRequestV1CommonInfo commonInfo = new B2cOrderQueryRequestV1CommonInfo();
		commonInfo.setTransCode("B2CPAYINF");
		commonInfo.setCis("020090001693059");
		commonInfo.setBankCode("102");
		commonInfo.setId("2t321.y.0200");
		commonInfo.setTranDate("20170224");
		commonInfo.setTranTime("204438000");
		commonInfo.setFseqno("98111314511403031941352");
		
		B2cOrderQueryRequestV1TradeInfo tradeInfo = new B2cOrderQueryRequestV1TradeInfo();
		tradeInfo.setQryFlag("1");
		tradeInfo.setShopType("2");
		tradeInfo.setShopCode("0200EC27246674");
		tradeInfo.setShopAcct("0200041909200052886");
		tradeInfo.setQrySerialNo("HEZ000008312732913");
		tradeInfo.setBeginDate("20170101");
		tradeInfo.setEndDate("20171231");
		tradeInfo.setQueryType("1");
		
		bizContent.setCommonInfo(commonInfo);
		bizContent.setTradeInfo(tradeInfo);
		
		request.setBizContent(bizContent);

		B2cOrderQueryResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess()) {
			// 接收成功
			B2cOrderQueryResponseV1CommonRetInfo commonRetInfo = response.getCommonRetInfo();
			B2cOrderQueryResponseV1TradeRetInfo tradeRetInfo = response.getTradeRetInfo();
			if(commonRetInfo.getRetCode().equals("0")){
				//查询成功 获取返回结果
				List<B2cOrderQueryResponseV1DetailRetInfo> resultList = tradeRetInfo.getDetailRetInfo();
				//获取结果集中字段
				for(B2cOrderQueryResponseV1DetailRetInfo detail:resultList){
					//获取信息.....
					detail.getResult();
				}
			} else {
				//查询失败
				System.out.println(commonRetInfo.getRetMsg());
			}
		} else {
			// 失败
			System.out.println(response.getReturnMsg());
		}
	}
}
