package com.icbc.api;

import com.icbc.api.request.JftApiB2bPayQueryOrderRequestV1;
import com.icbc.api.request.JftApiB2bPayQueryOrderRequestV1.JftApiB2bPayQueryOrderRequestV1Biz;
import com.icbc.api.response.JftApiB2bPayQueryOrderResponseV1;

public class JftApiB2bPayQueryOrderTestV1 {
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000012146";


    public static void main(String[] args) {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8",
                "json",APIGW_PUBLIC_KEY,"AES","ACKHXum+B2SWcyQ/VPK9qg==", "", "");
        JftApiB2bPayQueryOrderRequestV1 request=new JftApiB2bPayQueryOrderRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/pay/b2bPay/queryPaymentStatus/V1");
        JftApiB2bPayQueryOrderRequestV1Biz bizContent=new JftApiB2bPayQueryOrderRequestV1Biz();



        bizContent.setAppId("10000000000000012146");//平台商户标识
        bizContent.setOutVendorId("71810240113");//子商户标识
        bizContent.setOutOrderId("201811233451216");//平台订单ID
        bizContent.setTradeTime("2018-12-06 15:00:20");//交易时间
        bizContent.setOrderType("01");//订单类型

        request.setBizContent(bizContent);

        JftApiB2bPayQueryOrderResponseV1 response;
        try {
            response = client.execute(request, System.currentTimeMillis()+"");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
            if (response.isSuccess()) {
                //6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
                System.out.println("ReturnCode:"+response.getReturnCode());
                System.out.println("response:" + response);
            } else {
                //失败
                System.out.println("ReturnCode:"+response.getReturnCode());
                System.out.println("ReturnMsg:"+response.getReturnMsg());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }

    }
}
