package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MybankAccountFarmSyncMatterRequestV1;
import com.icbc.api.response.MybankAccountFarmSyncMatterResponseV1;

public class MybankAccountFarmSyncMatterTestV1 {
protected static String APP_ID = "10000000000004096993";
	

	private  static String BASE_URL = "http://ip:port/api";
	

	protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
     





	public static void main(String[] args) throws Exception {
		String URI = "mybank/account/farm/syncMatter/V1";

		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
		try {
			MybankAccountFarmSyncMatterRequestV1 request = new MybankAccountFarmSyncMatterRequestV1();
			MybankAccountFarmSyncMatterRequestV1.MybankAccountFarmSyncMatterBizV1 requestBizV1 = new MybankAccountFarmSyncMatterRequestV1.MybankAccountFarmSyncMatterBizV1();

			requestBizV1.setfSeqNo("124325");
			requestBizV1.setTransDate("20220202");
			requestBizV1.setCorpCode("icbc");
			requestBizV1.setParamType("1");
			requestBizV1.setDetail("{\"protocolName\":\"协议1\",\"busiClientId\":\"2022062700007802\",\"protocolOrigAmt\":\"1234\",\"issAcctNo\":\"900\",\"issAcctName\":\"张三\"}");

			request.setServiceUrl(BASE_URL +"/"+ URI);
			request.setBizContent(requestBizV1);
			
	        System.out.println(JSON.toJSONString(request));

	        MybankAccountFarmSyncMatterResponseV1 response = client.execute(request);
			if (response.isSuccess()) {
				System.out.println("success");
				System.out.println(JSONObject.toJSONString(response));
			} else {
				System.out.println("error");
				System.out.println(JSONObject.toJSONString(response));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

	}

}
