package com.icbc.api;

import java.text.SimpleDateFormat;
import java.util.Date;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BusinessopOrdermanageCashorderupdateRequestV1;
import com.icbc.api.request.BusinessopOrdermanageCashorderupdateRequestV1.BusinessopOrdermanageCashorderupdateRequestV1Biz;
import com.icbc.api.request.BusinessopOrdermanageCashorderupdateRequestV1.ReqPriField;
import com.icbc.api.request.BusinessopOrdermanageCashorderupdateRequestV1.ReqPubField;
import com.icbc.api.response.BusinessopOrdermanageCashorderupdateResponseV1;

public class BusinessopOrdermanageCashorderupdateTestV1 {


	//密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
		
	//网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//应用ID，APP申请分配appid
	protected static final String APP_ID = "xxxxxxxxx";


	public void test_cop() throws IcbcApiException {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		BusinessopOrdermanageCashorderupdateRequestV1 request = new BusinessopOrdermanageCashorderupdateRequestV1();
		//根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://ip:port/api/businessop/ordermanage/cashorderupdate/V1");

		BusinessopOrdermanageCashorderupdateRequestV1Biz bizContent = new BusinessopOrdermanageCashorderupdateRequestV1Biz();
		ReqPubField pubField = new ReqPubField();
		pubField.setChancode("702");
		pubField.setOapp("F-BIOM");
		pubField.setOtellerno("49");
		bizContent.setPubField(pubField);
		
		ReqPriField priField = new ReqPriField();
		priField.setBusiType("6");
		priField.setBussnessId("21431546457");
		priField.setStatus("2");//2-订单完成；3-订单作废
		priField.setAddInfo("");
		priField.setBack1("");
		
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
		String sysDate = formatter.format(new Date());
		
		priField.setOperDate(sysDate);
		priField.setOperApp("");
		priField.setOperStru("");
		priField.setOperTell("1234");
		priField.setOperTellname("操作柜员");
		priField.setCheckTell("1244");
		priField.setCheckTellname("柜员");
		priField.setBeforeStat("");
		priField.setAfterStat("");
		priField.setMsg("订单已完成");
		priField.setBack1("");
		priField.setBack1("");
		priField.setBack1("");
		bizContent.setPriField(priField);
		
		//System.out.println(JSONObject.toJSONString(bizContent));
		request.setBizContent(bizContent);

		BusinessopOrdermanageCashorderupdateResponseV1 response = client.execute(request,System.currentTimeMillis()+"");//请求通讯唯一编号
		boolean testFlag = true;
		if (response.isSuccess() && response.getReturnCode() == 0) {// 业务成功处理
			logger.info(response.getReturnCode());
			logger.info(response.getReturnMsg());
			assertEquals(true, testFlag);
		} else {// 失败
			logger.info(response.getReturnCode());
			logger.info(response.getReturnMsg());
			assertEquals(false, testFlag);
		}

	}

}
