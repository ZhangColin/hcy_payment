package com.icbc.api;

import com.icbc.api.request.DebitmediumManageBranchPostdebitcardcancelRequestV1;
import com.icbc.api.response.DebitmediumManageBranchPostdebitcardcancelResponseV1;
import org.junit.Test;

/**
 *  @author: kfzx-lihw03
 *  @date: 2022/1/14 11:45
 *  需求子条目编号：S20220112-00683
 *  邮寄借记卡撤单服务api test
 */
public class DebitmediumManageBranchPostdebitcardcancelTest {
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
    //APP管理中查看
    protected static final String APP_ID = "10000000000000021587";


    public void test_cop() throws IcbcApiException {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        DebitmediumManageBranchPostdebitcardcancelRequestV1 request = new DebitmediumManageBranchPostdebitcardcancelRequestV1();
        request.setServiceUrl("https://ip:port/api/debitmedium/manage/branch/postdebitcardcancel/V1");
        DebitmediumManageBranchPostdebitcardcancelRequestV1.DebitmediumManageBranchPostdebitcardcancelRequestV1Biz bizContent = new DebitmediumManageBranchPostdebitcardcancelRequestV1.DebitmediumManageBranchPostdebitcardcancelRequestV1Biz();

        /**
        bizContent.setChanCommV10(?);
        bizContent.setInfoCommV10(?);
        bizContent.setCommpi20063(?);
         */
        request.setBizContent(bizContent);

        DebitmediumManageBranchPostdebitcardcancelResponseV1 response = client.execute(request, "msgId");

        if (response.isSuccess() && response.getReturnCode() == 0) {
            // 业务成功处理
            System.out.println("success");
        } else {
            // 失败
            System.err.println(response.getReturnCode());//
            System.err.println(response.getReturnMsg());//
        }
    }
}
