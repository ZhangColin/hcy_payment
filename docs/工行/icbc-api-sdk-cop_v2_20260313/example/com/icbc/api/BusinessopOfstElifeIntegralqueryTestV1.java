package com.icbc.api;
import com.icbc.api.IcbcClient;
import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.BusinessopOfstElifeIntegralqueryRequestV1;
import com.icbc.api.request.BusinessopOfstElifeIntegralqueryRequestV1.BusinessopOfstElifeIntegralqueryRequestV1Biz;
import com.icbc.api.response.BusinessopOfstElifeIntegralqueryResponseV1;

import java.util.logging.Logger;

public class BusinessopOfstElifeIntegralqueryTestV1 {
    // 1、网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    // 2、密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    // 3、appid
    protected static final String APP_ID = "xxxxxxxxx";

    private static final Logger logger = Logger.getLogger(BusinessopOfstElifeIntegralqueryTestV1.class.getName());

	public static void main(String[] args) throws Exception {
        String ciNoEncryp = "1076dAbpBsrJXpI4JHXthaHj+mORib";

        String msg_id = String.valueOf(System.currentTimeMillis());

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
                MY_PRIVATE_KEY,
                APIGW_PUBLIC_KEY);

        BusinessopOfstElifeIntegralqueryRequestV1 request = new BusinessopOfstElifeIntegralqueryRequestV1();

        request.setServiceUrl("https://ip:port/api/businessop/ofst/elife/integralquery/V1");

        BusinessopOfstElifeIntegralqueryRequestV1Biz bizContent = new BusinessopOfstElifeIntegralqueryRequestV1Biz();
        bizContent.setCiNoEncryp(ciNoEncryp);
        request.setBizContent(bizContent);
        BusinessopOfstElifeIntegralqueryResponseV1 resp = client.execute(request, msg_id);
        /*if(resp.isSuccess()){
            System.out.println(resp.getReturnCode() + " " + resp.getReturnMsg() + " " + resp.getMsgId());
        }else{
            System.out.println(resp.getReturnMsg());
        }*/
        if (resp.getReturnCode() == 0) {
            logger.info("ReturnCode:" + resp.getReturnCode());
            logger.info("response:" + JSON.toJSONString(resp));
        } else {
            logger.info("response:" + JSON.toJSONString(resp));
            logger.info("ReturnCode:" + resp.getReturnCode());
            logger.info("ReturnMsg:" + resp.getReturnMsg());
        }
    }
        
}
			

	
