...
import com.icbc.api.request.FinanceTppfinanceQueryfintradedetailRequestV1;
import com.icbc.api.request.FinanceTppfinanceQueryfintradedetailRequestV1.FinanceTppfinanceQueryfintradedetailRequestV1Biz;
import com.icbc.api.response.FinanceTppfinanceQueryfintradedetailResponseV1;

public class FinanceTppfinanceQueryfintradedetailV1Test {
	...
	public void test() throws IcbcApiException {
		...
		FinanceTppfinanceQueryfintradedetailRequestV1 request = new FinanceTppfinanceQueryfintradedetailRequestV1();
		request.setServiceUrl("https://ip:port/api/finance/tppfinance/queryfintradedetail/V1");

		// 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值，非必输字段可不上送
		FinanceTppfinanceQueryfintradedetailRequestV1Biz bizContent = new FinanceTppfinanceQueryfintradedetailRequestV1Biz();
		bizContent.setClient_id("ICBCTEST80001"); // 客户ID
		bizContent.setQryprodid("1701ELT"); // 产品代码
		bizContent.setQrycurrtype("CNY"); // 币种
		bizContent.setBegintime("2023-01-01"); // 查询开始时间
		bizContent.setEndtime("2025-12-31"); // 查询截止时间
		bizContent.setInitflag(1); // 动作标识 1-第一次查询 2-上页 3-下页
		bizContent.setFetnum(5); // 查询项数（每次查询最多5条）
		request.setBizContent(bizContent);

		FinanceTppfinanceQueryfintradedetailResponseV1 response = new FinanceTppfinanceQueryfintradedetailResponseV1();
		...
		if (response.isSuccess()) {
			...
			System.out.println("rowset                  :       " + response.getRowset().toString());
