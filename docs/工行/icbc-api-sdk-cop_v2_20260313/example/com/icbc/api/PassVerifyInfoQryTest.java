package com.icbc.api;

import java.util.Arrays;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.PassVerifyInfoQryRequestV1;
import com.icbc.api.PassVerifyInfoQryRequestV1.PassVerifyInfoQryRequestV1Biz;
import com.icbc.api.PassVerifyInfoQryResponseV1;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;

public class PassVerifyInfoQryTest {

	//1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	//2、appid 行外通行证APP 
	protected static final String APP_ID = "10000000000004094587";
	
	//3、密钥对认证方式，公钥在API平台登记，此处为私钥 行外访问
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	public static void main(String[] args) throws Exception {
		
		String whitelistid = "1,2,4,5";
		String loginid = "";
		String[] whitelist = whitelistid.split(",");
		boolean flag=Arrays.asList(whitelist).contains(loginid);
		if(flag)
			System.out.println("yes");
		else
			System.out.println("no");
		
		//签名类型为SHA256WithRSA时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		PassVerifyInfoQryRequestV1 request = new PassVerifyInfoQryRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		//行外流程ip:port
		request.setServiceUrl("http://ip:port/api/cert/epass/verifyinfo/epassverifyinfoqry/V1");

		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值			
		PassVerifyInfoQryRequestV1Biz bizContent = new PassVerifyInfoQryRequestV1Biz();
		
		//chanCommV10
		PassVerifyInfoQryRequestV1.ChanCommV10RequestV1Message chanCommV10 = new PassVerifyInfoQryRequestV1.ChanCommV10RequestV1Message();
		chanCommV10.setChantype("");
		bizContent.setChanCommV10(chanCommV10);
		
		//infoCommV10
		PassVerifyInfoQryRequestV1.InfoCommV10RequestV1Message infoCommV10 = new PassVerifyInfoQryRequestV1.InfoCommV10RequestV1Message();
		infoCommV10.setTrxtype("");
		bizContent.setInfoCommV10(infoCommV10);
		
		//inqCommV10
		PassVerifyInfoQryRequestV1.InqCommV10RequestV1Message inqCommV10 = new PassVerifyInfoQryRequestV1.InqCommV10RequestV1Message();
		inqCommV10.setInqtype("");
		bizContent.setInqCommV10(inqCommV10);
		
		//inputV10
		PassVerifyInfoQryRequestV1.InputV10RequestV1Message inputV10 = new PassVerifyInfoQryRequestV1.InputV10RequestV1Message();
		inputV10.setFcode("PassVerifyInfoQryDSF");
		inputV10.setServer_ip("111");
		inputV10.setDt("11");
		inputV10.setTm("11");
		inputV10.setChannelIdentifier("1S1DF");
		inputV10.setChannelCode("12");
		inputV10.setFieldtype("4");
		inputV10.setFieldvalue("020078900077455");
		bizContent.setInputV10(inputV10);
		
		request.setBizContent(bizContent);
		//建立返回项
		PassVerifyInfoQryResponseV1 response = new PassVerifyInfoQryResponseV1();
		try {
			response = client.execute(request);	
			System.out.println(JSONObject.toJSONString(response));
			String retcode = response.getAppstatv10().getTransok();
			System.out.println("transok:"+retcode);
			String return_code = response.getAppstatv10().getReturn_code();
			System.out.println("return_code:" + return_code);
			System.out.println("return_msg:" + response.getAppstatv10().getReturn_msg());
		} catch (IcbcApiException e) {
			e.printStackTrace();
		} 	
	}
}
