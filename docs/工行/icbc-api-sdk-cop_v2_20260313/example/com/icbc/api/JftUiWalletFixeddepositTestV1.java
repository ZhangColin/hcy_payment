package com.icbc.api;

import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;


import com.icbc.api.UiIcbcClient;
import com.icbc.api.request.JftUiWalletFixeddepositRequestV1;

public class JftUiWalletFixeddepositTestV1 extends HttpServlet {
	
	private static final long serialVersionUID = 7082115106440379278L;

	  protected void doGet(HttpServletRequest req, HttpServletResponse resp)
	    throws ServletException, IOException  {
	    doPost(req, resp);
	  }

	  protected void doPost(HttpServletRequest req, HttpServletResponse resp)throws ServletException, IOException  {
	    PrintWriter out = null;
	    try {
	      String appId = "100000XXXXX0102010";
	      String corpAppid = "1000XXXXX00010533";//合作方机构编号
	      String corpMediumId = "128568XXXXX0344";//用户介质号
	      String custName = "张三";//用户名
	      String certNo = "1285XXXXX0344";//身份证号
	      String phoneNo = "134XXXXX4";//手机号
	      String bacAddr = "www.baidu.com";//回调地址
	      String signType = "RSA2";//
	      

	      String myPrivateKey = "-----请使用正确的APP私钥-----";
	      UiIcbcClient client = new UiIcbcClient(appId, signType, myPrivateKey, "UTF-8");
	      JftUiWalletFixeddepositRequestV1 request = new JftUiWalletFixeddepositRequestV1();
	      request.setServiceUrl("https://ip:port/ui/jft/ui/wallet/fixeddeposit/V1");
	      
	      JftUiWalletFixeddepositRequestV1.JftUiWalletFixeddepositRequestV1Biz bizC = new JftUiWalletFixeddepositRequestV1.JftUiWalletFixeddepositRequestV1Biz();
	      bizC.setCorpAppid(corpAppid);
          bizC.setCorpMediumId(corpMediumId);
          bizC.setCertNo(certNo);
          bizC.setCustName(custName);
          bizC.setPhoneNo(phoneNo);
          bizC.setBacAddr(bacAddr);
          request.setBizContent(bizC);

	      resp.setHeader("Content-Type", "text/html;charset=UTF-8");
	      out = resp.getWriter();

	      out.write("<html>");
	      out.write("<head>");
	      out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=UTF-8\">");

	      out.write("</head>");
	      out.write("<body>");
	      out.write(client.buildPostForm(request));
	      out.write("</body>");
	      out.write("</html>");
	    }
	    catch (Exception e) {
	      e.printStackTrace();
	      //System.out.println(e.getMessage());
	    } finally {
	      if (out != null) {
	        out.flush();
	        out.close();
	      }
	    }
	  }
}
