...
import com.icbc.api.request.EnterpriseSalaryAnnuityTaxcalculateRequestV1;
import com.icbc.api.response.EnterpriseSalaryAnnuityTaxcalculateResponseV1;
import org.junit.Assert;
import org.junit.Test;

/**
 * @Author: kfzx-libl
 * @Description: 个人年金待遇测算测试案例
 * @Date: 2021/1/11
 */
public class EnterpriseSalaryAnnuityTaxcalculateV1Test {
    ...
    public void test_cop() {
        ...
        //6、设置请求对象
        EnterpriseSalaryAnnuityTaxcalculateRequestV1 request = new EnterpriseSalaryAnnuityTaxcalculateRequestV1();
        //7、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("https://ip:port/api/enterprise/salary/annuity/taxcalculate/V1");
        //8、设置业务参数
        EnterpriseSalaryAnnuityTaxcalculateRequestV1.EnterpriseSalaryAnnuityTaxcalculateRequestV1Biz bizContent = new EnterpriseSalaryAnnuityTaxcalculateRequestV1.EnterpriseSalaryAnnuityTaxcalculateRequestV1Biz();
        ...
        bizContent.setMobileNo("13521519767"); //移动电话号码
        bizContent.setPayCalcType("1");// 查询信息类型（1.最优 2.分期）
        bizContent.setPayFreq("12");//年金领取频率（限定为年）
        bizContent.setPayPeriodBegin("");//开始期数（分期方式，则必输，结束期数和开始期数的差不得大于10）
        bizContent.setPayPeriodEnd("");//结束期数（分期方式，则必输，结束期数和开始期数的差不得大于10）

        request.setBizContent(bizContent);

        EnterpriseSalaryAnnuityTaxcalculateResponseV1 response = null;
        try {
            response = client.execute(request);
        } catch (IcbcApiException e) {
            // 测试环境案例经常调不通 给个默认值
            response = new EnterpriseSalaryAnnuityTaxcalculateResponseV1();
        }
