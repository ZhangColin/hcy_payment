...
import com.icbc.api.request.EnterpriseSalaryAnnuityOfferdetailqueryRequestV1;
...
/**
 * @Author: kfzx-libl
 * @Description: 个人年金缴费明细查询测试案例
 * @Date: 2021/1/11
 */
public class EnterpriseSalaryAnnuityOfferdetailqueryV1Test {
    ...
    public void test_cop() {
        ...
        //6、设置请求对象
        EnterpriseSalaryAnnuityOfferdetailqueryRequestV1 request = new EnterpriseSalaryAnnuityOfferdetailqueryRequestV1();
        //7、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("https://ip:port/api/enterprise/salary/annuity/offerdetailquery/V1");
        //8、设置业务参数
        EnterpriseSalaryAnnuityOfferdetailqueryRequestV1.EnterpriseSalaryAnnuityOfferdetailqueryRequestV1Biz bizContent = new EnterpriseSalaryAnnuityOfferdetailqueryRequestV1.EnterpriseSalaryAnnuityOfferdetailqueryRequestV1Biz();
        ...
        bizContent.setBeginDate("2013-01-01");//开始日期( yyyy-MM-dd)（开始日期与结束日期的时间间隔不得超过两年）
        bizContent.setEndDate("2014-01-01");//结束日期( yyyy-MM-dd)（开始日期与结束日期的时间间隔不得超过两年）

        request.setBizContent(bizContent);

        EnterpriseSalaryAnnuityOfferdetailqueryResponseV1 response = null;
        try {
            response = client.execute(request);
        } catch (IcbcApiException e) {
            // 测试环境案例经常调不通 给个默认值
            response = new EnterpriseSalaryAnnuityOfferdetailqueryResponseV1();
        }
