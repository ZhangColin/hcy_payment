package com.icbc.api;

import static org.junit.Assert.assertEquals;

import java.io.UnsupportedEncodingException;
import java.util.logging.Logger;

import org.junit.Test;

import com.icbc.api.request.CardbussinessUnionQuickpayRequestV1;
import com.icbc.api.request.CardbussinessUnionQuickpayRequestV1.CardbussinessUnionQuickpayRequestV1Biz;

public class CardbusinessUnionQuickpayV1Test {
	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	// 2、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected final static String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	// 3、appid
	protected static final String APP_ID = "xxx";
	
	private Logger logger = Logger.getLogger(CardbusinessUnionQuickpayV1Test.class.getName());
	

	public void test_cop() throws IcbcApiException, UnsupportedEncodingException {
		
		UiIcbcClient client = null;
		CardbussinessUnionQuickpayRequestV1 request = new CardbussinessUnionQuickpayRequestV1();
		//request.setNeedEncrypt(true);//设置是否需要对biz_content进行AES解密，默认不需要
		if(request.isNeedEncrypt()){//若需要进行AES解密
			String encryptKey = "xxxx";//AES密钥
			//4、配置签名类型为RSA2，加密类型为AES，AES密钥为encryptKey
			client = new UiIcbcClient(APP_ID,IcbcConstants.SIGN_TYPE_RSA2,MY_PRIVATE_KEY, 
					IcbcConstants.CHARSET_UTF8,IcbcConstants.ENCRYPT_TYPE_AES,encryptKey);
		}else{
			//4、签名类型为RSA2时，需传入appid，私钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
			client = new UiIcbcClient(APP_ID,IcbcConstants.SIGN_TYPE_RSA2,MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
		}
		//5、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/ui/cardbusiness/union/quickpay/V1");
		CardbussinessUnionQuickpayRequestV1Biz bizContent = new CardbussinessUnionQuickpayRequestV1Biz();
		request.setBizContent(bizContent);
		//6、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		bizContent.setIcbc_appid("xxx");
		bizContent.setOut_trade_no("xxx");
		bizContent.setAccess_type("xxx");
		bizContent.setMer_id("xxx");
		bizContent.setMer_prtcl_no("xxx");
		bizContent.setReturn_url("xxx");
		bizContent.setAttach("xxx");
		bizContent.setNotify_type("xxx");
		bizContent.setResult_type("xxx");
		bizContent.setOrder_apd_inf("xxx");
		bizContent.setBody("xxx");
		bizContent.setExpire_time("xxx");
		bizContent.setFail_url("xxx");
		bizContent.setNotify_url("xxx");
		bizContent.setOrder_amt("xxx");
		bizContent.setPackage_name("xxx");
		bizContent.setPay_limit("xxx");
		bizContent.setScheme("xxx");
		bizContent.setAcq_addn_data("xxx");
		bizContent.setLanguage("xxx");
		try {
			logger.info(client.buildPostForm(request));
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
