package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.GyjrB2bFinanceComdrftQryacceptmersignRequestV1;
import com.icbc.api.response.GyjrB2bFinanceComdrftQryacceptmersignResponseV1;

import java.util.List;
import java.util.Random;
import java.util.ArrayList;
import java.util.logging.Logger;

public class GyjrB2bFinanceComdrftQryacceptmersignRequestTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16" +
            "bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL" +
            "T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP" +
            "3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000004097878";

    static Logger log = Logger.getLogger(GyjrB2bFinanceComdrftQryacceptmersignRequestTestV1.class.getName());
    public static void main(String[] args) {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        GyjrB2bFinanceComdrftQryacceptmersignRequestV1 req = new GyjrB2bFinanceComdrftQryacceptmersignRequestV1();

        req.setServiceUrl("http://ip:port/api/gyjr/b2b/finance/comdrft/qryacceptmersign/V1");

        GyjrB2bFinanceComdrftQryacceptmersignRequestV1.GyjrB2bFinanceComdrftQryacceptmersignRequestV1Biz bizContent = new GyjrB2bFinanceComdrftQryacceptmersignRequestV1.GyjrB2bFinanceComdrftQryacceptmersignRequestV1Biz();

        GyjrB2bFinanceComdrftQryacceptmersignRequestV1.TransinfoBiz transInfo = new GyjrB2bFinanceComdrftQryacceptmersignRequestV1.TransinfoBiz();

        transInfo.setProtocolCode("GYJR20211204102832000000002");
        transInfo.setPlatVendorId("64854126126589");
        transInfo.setPageIndex("1");
        transInfo.setPageSize("10");
        transInfo.setUkeyId("81367173515");

        GyjrB2bFinanceComdrftQryacceptmersignRequestV1.SignAcceptInfo signAcceptInfo = new GyjrB2bFinanceComdrftQryacceptmersignRequestV1.SignAcceptInfo();

        List<String> accountNumList = new ArrayList<>();
        List<String> billNoList = new ArrayList<>();

        accountNumList.add("4545");
        accountNumList.add("0355");

        billNoList.add("5785151");
        billNoList.add("4845055145");

        signAcceptInfo.setAccountNumList(accountNumList);
        signAcceptInfo.setBillAmtFrom("");
        signAcceptInfo.setBillAmtTo("");
        signAcceptInfo.setBillDeadDateFrom("20220101");
        signAcceptInfo.setBillDeadDateTo("20990909");
        signAcceptInfo.setBillNoList(billNoList);

        transInfo.setSignAcceptInfo(signAcceptInfo);

        bizContent.setTransInfoBiz(transInfo);
        req.setBizContent(bizContent);

        System.out.println(JSONObject.toJSONString(bizContent));

        GyjrB2bFinanceComdrftQryacceptmersignResponseV1 response;

        Random rand = new Random();
        String msgId = rand.nextInt(99999) + "msg";
        try {
            response = client.execute(req,msgId);
            log.fine("JSONObject.toJSONString(response):"+ JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                log.fine("response.getReturnCode():"+response.getReturnCode());
            } else {
                //失败
                log.fine("response.getReturnCode():"+response.getReturnCode());
                log.fine("response.getReturnMsg():"+response.getReturnMsg());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
