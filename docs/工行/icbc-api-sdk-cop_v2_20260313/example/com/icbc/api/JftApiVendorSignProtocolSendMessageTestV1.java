...
import com.icbc.api.request.JftApiVendorSignProtocolSendMessageRequestV1;
import com.icbc.api.request.JftApiVendorSignProtocolSendMessageRequestV1.JftApiVendorSignProtocolSendMessageRequestV1Biz;
 ...
 */
public class JftApiVendorSignProtocolSendMessageTestV1 {
	...
	public static void main(String[] args) throws Exception {
		...
		JftApiVendorSignProtocolSendMessageRequestV1 request = new JftApiVendorSignProtocolSendMessageRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/jft/api/vendor/sign/protocol/send/message/V1");

		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		JftApiVendorSignProtocolSendMessageRequestV1Biz bizContent = new JftApiVendorSignProtocolSendMessageRequestV1Biz();
		...
		bizContent.setAccountNo("63124821654649846456");

		//VerifyCodeType为2时新增上送
		bizContent.setCertType("0");
		bizContent.setCertNo("1101563199011064578");
		bizContent.setName("测试");


		request.setBizContent(bizContent);

		JftApiVendorSignProtocolSendMessageResponseV1 response;
		try {
			response = (JftApiVendorSignProtocolSendMessageResponseV1) client.execute(request, System.currentTimeMillis() + "");
