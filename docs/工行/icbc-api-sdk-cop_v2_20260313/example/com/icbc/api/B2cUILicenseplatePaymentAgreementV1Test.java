package com.icbc.api;


import com.icbc.api.IcbcApiException;
import com.icbc.api.request.B2cUILicenseplatePaymentAgreementRequestV1;
import com.icbc.api.request.B2cUILicenseplatePaymentAgreementRequestV1.B2cUILicenseplatePaymentAgreementBizContentV1;

public class B2cUILicenseplatePaymentAgreementV1Test {
	
	//appid
	protected static final String APP_ID = "*************";
	
	//合作方私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	//网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	//AES秘钥
	protected static final String AES_KEY = "YBlenAPT1KedFOoHIfSa5w==";
	public static void main(String[] args) throws IcbcApiException {
		UiIcbcClient client = new UiIcbcClient(APP_ID, 
																			IcbcConstants.SIGN_TYPE_RSA2, 
																			MY_PRIVATE_KEY,
																			IcbcConstants.CHARSET_UTF8,
																			IcbcConstants.ENCRYPT_TYPE_AES,
																			AES_KEY);
		B2cUILicenseplatePaymentAgreementRequestV1 request = new B2cUILicenseplatePaymentAgreementRequestV1();
        
		//根据测试环境和生产环境替换相应ip和端口http://gw.dccnet.com.cn/
        request.setServiceUrl("https://open.icbc.com.cn/ui/b2c/ui/licenseplate/payment/agreement/V1");

		
        B2cUILicenseplatePaymentAgreementBizContentV1 bizContent = new B2cUILicenseplatePaymentAgreementBizContentV1();
        String tranData = "ePayMobilePlate=18888888888&clientBackUrl=http://XXXXX/XXXXX&eventID=fgdgdgszdsgds";

		bizContent.setInterfaceName("ICBC_LICENSE_PLATE_PAY");
		bizContent.setTranData(tranData);
		bizContent.setInterfaceVersion("*****");//ip:port
		request.setBizContent(bizContent);
	
		/**
		 * 通过 client.buildPostForm(request) 方式组建 交易报文，
		 * 报文组建完毕后通过页面进行提交完成请求发送过程
		 */
		 client.buildPostForm(request);
	}
	
}
