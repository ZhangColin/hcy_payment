...
import com.icbc.api.request.JftApiVendorConfirmCancellationRequestV1;
import com.icbc.api.request.JftApiVendorConfirmCancellationRequestV1.JftApiVendorConfirmCancellationRequestV1Biz;
import com.icbc.api.response.JftApiVendorConfirmCancellationResponseV1;
 ...
 * @Description:
 */
public class JftApiVendorConfirmCancellationTestV1 {
	//1、网关公钥
    ...
    public static void main(String[] args) throws Exception {
        ...
        JftApiVendorConfirmCancellationRequestV1 request = new JftApiVendorConfirmCancellationRequestV1();

        //4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("https://ip:port/api/jft/api/vendor/confirm/cancellation/V1");

        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值ֵ

        JftApiVendorConfirmCancellationRequestV1Biz bizContent = new JftApiVendorConfirmCancellationRequestV1Biz();

        bizContent.setAppId("10000000000007354590");//appId
        bizContent.setOutVendorId("999221245");//outVendorId
		//bizContent.setOutUserId("12312312");

        request.setBizContent(bizContent);
        System.out.println("request:" + request.toString());

        JftApiVendorConfirmCancellationResponseV1 response;

        try {
            response = (JftApiVendorConfirmCancellationResponseV1)client.execute(request, System.currentTimeMillis()+"");
