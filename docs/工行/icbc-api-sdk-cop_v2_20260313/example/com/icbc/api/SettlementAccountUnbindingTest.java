package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountUnbindingRequestV1;
import com.icbc.api.request.SettlementAccountUnbindingRequestV1.SettlementAccountUnbindingRequestV1Biz;
import com.icbc.api.response.SettlementAccountUnbindingResponseV1;

public class SettlementAccountUnbindingTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	protected static final String APP_ID = "10000000000000021587";


	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountUnbindingRequestV1 request = new SettlementAccountUnbindingRequestV1();
		SettlementAccountUnbindingRequestV1Biz bizContent = new SettlementAccountUnbindingRequestV1Biz();

		request.setServiceUrl("http://ip:port/api/settlement/account/V1/unbinding");
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setTrxAccDate("2017-03-14"); //合作方交易日期
		bizContent.setTrxAccTime("00:00:00"); //合作方交易时间
		bizContent.setCorpDate("2017-03-14"); //合作方工作日期
		bizContent.setCorpSerno("20202020"); //合作方交易单号
		bizContent.setOutServiceCode("unbinding"); //外部服务代码
		bizContent.setMediumId("6888888888888888888"); //工行联名卡号
		bizContent.setBindMedium("6888888888888888888"); //绑定的I类卡号
		bizContent.setMobileNo("13611223388"); //手机号
		bizContent.setSecretKey("ASDQWEQDZCSDFAWWQDA"); //sm4对称密钥
		request.setBizContent(bizContent);

		SettlementAccountUnbindingResponseV1 response = client.execute(request, "msgId");
		if (response.isSuccess() ) {
			//成功
			//response.getAccountBalance();
		} else {
			//失败
		}
	}
}
