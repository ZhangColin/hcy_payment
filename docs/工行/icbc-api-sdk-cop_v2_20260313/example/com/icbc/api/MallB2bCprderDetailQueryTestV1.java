package com.icbc.api;

import java.net.URLDecoder;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MallB2bCprderDetailQueryRequestV1;
import com.icbc.api.request.MallB2bCprderDetailQueryRequestV1.MallB2bCprderDetailQueryRequestV1Biz;
import com.icbc.api.response.MallB2bCprderDetailQueryResponseV1;

public class MallB2bCprderDetailQueryTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000005402";

	public static void main(String[] args) throws Exception{

	DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", 
				"json",APIGW_PUBLIC_KEY,"AES","hdZdNozJr5oATedE9UmuSA==", "", "");
		MallB2bCprderDetailQueryRequestV1 request = new MallB2bCprderDetailQueryRequestV1();

		request.setServiceUrl("http://ip:port/api/mall/b2b/cporder/detail/query/V1");
		MallB2bCprderDetailQueryRequestV1Biz biz = new MallB2bCprderDetailQueryRequestV1Biz();
		biz.setOrderId("202012210000026522");
		biz.setVendorId("chain0021");
		request.setBizContent(biz);

		MallB2bCprderDetailQueryResponseV1 response;
		try {
			response = (MallB2bCprderDetailQueryResponseV1) client.execute(request, System.currentTimeMillis()+"");
			System.out.println(JSONObject.toJSONString(response));
			String resdata = response.getResdata();
			if(resdata != null && resdata.length() >0){
				resdata = URLDecoder.decode(resdata,"UTF-8");
				System.out.println("resdata = " + resdata);
			}else{
				System.out.println("resdata is null");
			}
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}

	}
	
}

