package com.icbc.api;

import java.util.UUID;

import org.junit.Test;

import com.icbc.api.request.FinanceTppfinanceModifyphonenoRequestV1;
import com.icbc.api.request.FinanceTppfinanceModifyphonenoRequestV1.FinanceTppfinanceModifyphonenoRequestV1Biz;
import com.icbc.api.response.FinanceTppfinanceModifyphonenoResponseV1;

public class FinanceTppfinanceModifyphonenoV1Test {

	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	// 2、appid
	protected static final String APP_ID = "10000000000000003916";

	// 3、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";


	public void test() throws IcbcApiException {

		// 签名类型为RSA时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY,
				APIGW_PUBLIC_KEY);

		FinanceTppfinanceModifyphonenoRequestV1 request = new FinanceTppfinanceModifyphonenoRequestV1();
		request.setServiceUrl("https://ip:port/api/finance/tppfinance/modifyphoneno/V1");

		// 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值，非必输字段可不上送
		FinanceTppfinanceModifyphonenoRequestV1Biz bizContent = new FinanceTppfinanceModifyphonenoRequestV1Biz();
		bizContent.setClient_id("ICBCTEST80001"); // 客户ID
		bizContent.setOld_phone_no("18688888888"); // 原手机号
		bizContent.setNew_phone_no("18699999999"); // 新手机号
		bizContent.setSms_id("ICBCTEST8000120210615161443000007"); // 认证短信唯一编号，短信认证时需要和短信编号、短信验证码一同上送
		bizContent.setSms_no("443253"); // 短信编号
		bizContent.setSms_identify_no("0ac63bcfcf1de3be"); // 验证码
		bizContent.setSms_key(
				""); // 验证码密钥（第三方公司随机生成192位对称密钥，使用该密钥加密验证码，并使用工行公钥加密后在本字段上送）
		request.setBizContent(bizContent);

		FinanceTppfinanceModifyphonenoResponseV1 response = new FinanceTppfinanceModifyphonenoResponseV1();
		String msgId = UUID.randomUUID().toString().replaceAll("-", "");
		response = client.execute(request, msgId); // msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一

		if (response.isSuccess()) {

			System.out.println("return_code             :       " + response.getReturnCode());
			System.out.println("return_msg              :       " + response.getReturnMsg());
			System.out.println("msg_id                  :       " + response.getMsgId());

		} else {

			System.out.println("return_code             :       " + response.getReturnCode());
			System.out.println("return_msg              :       " + response.getReturnMsg());
			System.out.println("msg_id                  :       " + response.getMsgId());

		}

	}
}
