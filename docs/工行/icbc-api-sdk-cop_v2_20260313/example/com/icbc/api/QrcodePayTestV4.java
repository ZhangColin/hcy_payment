...
import com.icbc.api.request.QrcodePayRequestV4;
import com.icbc.api.request.QrcodePayRequestV4.QrcodePayRequestV4Biz;
import com.icbc.api.response.QrcodePayResponseV4;;
/**
 * 工银二维码被扫支付接口
 * 
 * 请商户配置SDK中的lib文件夹中的jar包后，根据备注1-6提示进行数据替换
 */
public class QrcodePayTestV4 {
	...
	public static void main(String[] args) throws Exception {
		...
		//签名类型为RSA2时，需传入aphid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		
		QrcodePayRequestV4 request = new QrcodePayRequestV4();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/qrcode/pay/V4");

		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		QrcodePayRequestV4Biz bizContent = new QrcodePayRequestV4Biz();
		...
		bizContent.setAttach("商户自定义信息");
		bizContent.setSubAppId("子商户id");
		request.setBizContent(bizContent);


		QrcodePayResponseV4 response;
		try {
