package com.icbc.api;

import com.icbc.api.request.MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1;
import com.icbc.api.response.MybankAccountFundsarrivalmateFamcustomerinfoUpdateResponseV1;

import java.util.ArrayList;
import java.util.List;

public interface MybankAccountFundsarrivalmateFamcustomerinfoUpdateTestV1 {

    /*public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //4、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
        MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1 request = new MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1();
        request.setServiceUrl("http://ip:port/api/mybank/account/fundsarrivalmate/famcustomerinfoupdate/V1");

        MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfoManageRequestV1Biz bizContent = new MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfoManageRequestV1Biz();

        bizContent.setAgreeno("1");
        bizContent.setOptype("1");

        List<MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfo> customerList = new ArrayList<>();
        MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfo customerInfo1 = new MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfo();
        customerInfo1.setSellerid("1");
        customerInfo1.setCustomerid("1");
        customerInfo1.setCustomername("1");
        customerInfo1.setRecipacc("1");
        customerInfo1.setRecipnam("1");
        customerInfo1.setSmartcardno("1");

        MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfo customerInfo2 = new MybankAccountFundsarrivalmateFamcustomerinfoUpdateRequestV1.FamCustomerInfo();
        customerInfo2.setSellerid("2");
        customerInfo2.setCustomerid("2");
        customerInfo2.setCustomername("2");
        customerInfo2.setRecipacc("2");
        customerInfo2.setRecipnam("2");
        customerInfo2.setSmartcardno("2");

        customerList.add(customerInfo1);
//        customerList.add(customerInfo2);

        bizContent.setList(customerList);

        request.setBizContent(bizContent);
        MybankAccountFundsarrivalmateFamcustomerinfoUpdateResponseV1 response;
        try {
            //测试时，避免apip识别成重放，请修改msgId,每次跑，递加1比如
            response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
                //Assert.assertEquals(response.getReturnMsg(), "success");
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }*/

}
