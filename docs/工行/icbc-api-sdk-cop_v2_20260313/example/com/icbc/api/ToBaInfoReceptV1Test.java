package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONArray;
import java.util.HashMap;
import java.util.Map;

import com.icbc.api.request.ToBaInfoReceptRequestV1;
import com.icbc.api.request.ToBaInfoReceptRequestV1.ToBaInfoReceptRequestV1Biz;
import com.icbc.api.response.ToBaInfoReceptResponseV1;

public class ToBaInfoReceptV1Test {
	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	// 2、appid
	protected static final String APP_ID = "10000000000000012106";

	// 3、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	public static void main(String[] args) throws Exception {

		// 签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY,
				APIGW_PUBLIC_KEY);

		ToBaInfoReceptRequestV1 request = new ToBaInfoReceptRequestV1();

		// 4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://ip:port/api/saes/servmer/ordersubmit/V1");
		// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		ToBaInfoReceptRequestV1Biz bizContent = new ToBaInfoReceptRequestV1Biz();
		bizContent.setSaTrxId("0000000001");
		bizContent.setSaType(1);
		bizContent.setCvBusType(0);
		bizContent.setLegalPersonName("李四");
		bizContent.setRegAmount(1616515619L);
		bizContent.setCvAccount("16961619519651");
		bizContent.setCvCurrenceCode("1");
		bizContent.setCvAccountBalance(616515619L);
		bizContent.setCvAccountStatus("1");
		bizContent.setCvStatus(1);
		bizContent.setToBaTime("2021-07-27 11:05:00");
		bizContent.setBaName("张三");
		bizContent.setOpenBankName("中国工商银行广州分行");
		bizContent.setHandler("王五");
		bizContent.setHandlerNum("18916546540");
		bizContent.setDonateInfo(new JSONArray());

		request.setBizContent(bizContent);

		ToBaInfoReceptResponseV1 response;
		try {
			response = client.execute(request, "23344544454112");// msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一

			System.out.println("response:" + response.getReturnCode());

			if (response.isSuccess()) {
				// 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:" + response.getReturnCode());
				System.out.println("response:" + response);
			} else {
				// 失败
				System.out.println("ReturnCode:" + response.getReturnCode());
				System.out.println("ReturnMsg:" + response.getReturnMsg());
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
}
