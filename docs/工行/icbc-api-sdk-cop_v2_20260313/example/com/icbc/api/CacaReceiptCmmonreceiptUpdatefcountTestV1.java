package com.icbc.api;

import com.icbc.api.request.CacaReceiptCommonreceiptUpdatefcountRequestV1;
import com.icbc.api.response.CacaReceiptCommonreceiptUpdatefcountResponseV1;

import java.util.*;

public class CacaReceiptCmmonreceiptUpdatefcountTestV1 {
    // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
    public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CacaReceiptCommonreceiptUpdatefcountRequestV1 request = new CacaReceiptCommonreceiptUpdatefcountRequestV1();
        request.setServiceUrl("https://ip:port/api/caca/receipt/commonreceipt/updatefcount/V1");
        CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSRequestV1Biz bizContent = new CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSRequestV1Biz();

        // public 通讯区定义赋值
        CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSRequestV1Pub pubObj = new CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSRequestV1Pub();
        pubObj.setCode("29056");
        pubObj.setZone("0200");
        pubObj.setFr("0");
        pubObj.setDt("2017-07-06");
        // private 通讯区定义赋值
        CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSRequestV1In inObj = new CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSRequestV1In();


        inObj.setZone("0200");
        inObj.setBrno("");
        inObj.setTelno("");
        inObj.setChannel("");
        CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSCondRequestV1In updateFCountBySSTSCondRequestV1In = new CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSCondRequestV1In();
        updateFCountBySSTSCondRequestV1In.setAccno("");
        updateFCountBySSTSCondRequestV1In.setMainaccno("");
        updateFCountBySSTSCondRequestV1In.setSeqno("");
        updateFCountBySSTSCondRequestV1In.setCurrtype("");
        updateFCountBySSTSCondRequestV1In.setMgpayacc("");
        updateFCountBySSTSCondRequestV1In.setSerialno("");
        updateFCountBySSTSCondRequestV1In.setBustypno("");
        updateFCountBySSTSCondRequestV1In.setCardno("");
        updateFCountBySSTSCondRequestV1In.setDeviceno("");
        updateFCountBySSTSCondRequestV1In.setHelp_custmoer("");
        updateFCountBySSTSCondRequestV1In.setPartitioncolm("");
        updateFCountBySSTSCondRequestV1In.setAmount("");
        updateFCountBySSTSCondRequestV1In.setMarketno("");
        updateFCountBySSTSCondRequestV1In.setMemberno("");
        ArrayList<CacaReceiptCommonreceiptUpdatefcountRequestV1.UpdateFCountBySSTSCondRequestV1In> cond = new ArrayList<>();
        cond.add(updateFCountBySSTSCondRequestV1In);
        bizContent.setIn(inObj);
        bizContent.setPub(pubObj);
        request.setBizContent(bizContent);
        try {
            CacaReceiptCommonreceiptUpdatefcountResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}

