package com.icbc.api;

import com.icbc.api.request.ScisCustRelMaintainRequestV1;
import com.icbc.api.response.ScisCustRelMaintainResponseV1;

import java.util.logging.Logger;

/**
 * @Description TODO
 * @author: kfzx-lij1
 * @date: 2023/1/29 14:15
 **/
public class ScisCustRelMaintainRequestTestV1 {

    protected static Logger log = Logger.getLogger(ScisCustRelMaintainRequestTestV1.class.getName());

    protected static String APP_ID = "90000000000000003382";

    protected static String APIGW_PUBLIC_KEY =
            "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY"
                    + "0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY =
            "-----请使用正确的APP私钥-----";

    protected static String BASE_URL = "http://ip:port/api";

    public static void main(String[] args) {

        String URI = "scis/cust/rel/maintain/V1";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
        try {
            ScisCustRelMaintainRequestV1.ScisCustomerInfoRequestV1Biz bizContent = new ScisCustRelMaintainRequestV1.ScisCustomerInfoRequestV1Biz();
            ScisCustRelMaintainRequestV1 request = new ScisCustRelMaintainRequestV1();

            String reqData = "<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\n" +
                    "<APPROOT from=\"F-CORP\" to=\"F-SCIS\" mode=\"syn\" >\n" +
                    "\t<PUBLIC>\n" +
                    "\t\t<!-- 交易代码 -->\n" +
                    "\t\t<TxCode>SCI07050</TxCode>\n" +
                    "\t\t<!-- 公司代码 参照工行统一机构代码的分配-->\n" +
                    "\t\t<CorpCode>0018100000</CorpCode>\t\t\t\t\n" +
                    "\t\t<!-- 应用号 -->\n" +
                    "\t\t<AppNo>004</AppNo>\t\t\n" +
                    "\t\t<!-- 交易机构号 子机构代码 -->\n" +
                    "\t\t<SubCorpCode>0018100000</SubCorpCode>\t\t\n" +
                    "\t\t<!-- 隔离单元 -->\n" +
                    "\t\t<IsolationCell>0</IsolationCell>\n" +
                    "\t\t<!-- 业务经营所在国家或地区 -->\n" +
                    "\t\t<AreaCode>999</AreaCode>\n" +
                    "\t\t<!-- 交易发起终端号  客户端请求的端机IP地址或者其它能够唯一标识发起方的信息 -->\n" +
                    "\t\t<SendFrom></SendFrom>\n" +
                    "\t\t<!-- 操作员编号  统一认证号-->\n" +
                    "\t\t<OperNo></OperNo>\n" +
                    "\t\t<!-- 操作员密级 -->\n" +
                    "\t\t<OperSecLevel></OperSecLevel>\t\t\t\n" +
                    "\t\t<!-- 验证码 暂时送空-->\n" +
                    "\t\t<Verify></Verify>\t\t\n" +
                    "\t\t<!-- 发送方日期 -->\n" +
                    "\t\t<SendDate>20120816</SendDate>\n" +
                    "\t\t<!-- 发送方时间 -->\n" +
                    "\t\t<SendTime>193636</SendTime>\t\t\n" +
                    "\t\t<!-- 发送方流水号 -->\t\t\t\n" +
                    "\t\t<SerialNo>20120816193636000002</SerialNo>\n" +
                    "\t\t<!-- 语言集 -->\n" +
                    "\t\t<Language>zh_CN</Language>\n" +
                    "\t\t<!-- 开始记录数 -->\n" +
                    "\t\t<BeginRec></BeginRec>\n" +
                    "\t\t<!-- 一次查询最大记录数 -->\n" +
                    "\t\t<MaxRec></MaxRec>\n" +
                    "\t\t<!-- 版本号 -->\t\t\t\t\t\t\n" +
                    "\t\t<Ver>1.0.2</Ver>\t\t\n" +
                    "\t</PUBLIC>\n" +
                    "\t<PRIVATE>\n" +
                    "\t\t<Category name=\"SCIS_CUST_REL\"> <!-- 客户与客户关系信息 -->\n" +
                    "\t\t\t<field-list>\n" +
                    "\t\t\t\t<!-- 修改模式 1-新增 -->\n" +
                    "\t\t\t\t<ModifyType>1</ModifyType>\t\t\t\n" +
                    "\t\t\t\t<!--客户与客户的关系类型-->\n" +
                    "\t\t\t\t<field name='CONTEXTROLEPLAYER'>3123141412312313</field>\n" +
                    "\t\t\t\t<!--相关角色ID类型-->\n" +
                    "\t\t\t\t<field name='CONTEXTROLEPL_TYPE'>1</field>\n" +
                    "\t\t\t\t<!--角色ID-->\n" +
                    "\t\t\t\t<field name='ROLEPLAYER'>1141231231231212</field>\n" +
                    "\t\t\t\t<!--角色ID类型-->\n" +
                    "\t\t\t\t<field name='ROLEPLAYER_TYPE'>1</field>\n" +
                    "\t\t\t</field-list>\n" +
                    "\t\t</Category>\n" +
                    "\t</PRIVATE>\n" +
                    "</APPROOT>";

            bizContent.setReqData(reqData);
            request.setBizContent(bizContent);
            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(bizContent);
            ScisCustRelMaintainResponseV1 response = (ScisCustRelMaintainResponseV1) client.execute(request);
            log.info(response.getResData());
            if (response.isSuccess()) {
                // 业务成功处理
                log.info("处理成功");
                log.info(response.getResData());
            } else {
                // 失败
                log.info("处理失败");
                log.info(response.getReturnCode()+","+response.getReturnMsg());
            }
        } catch (Exception e) {
            e.printStackTrace();
            log.info(e.getMessage());
        }
    }
}
