package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MybankFarmThirdRoleRequestV1;
import java.util.Random;

/**
 * 功能描述:
 *
 * @author kfzx-suojx
 * @date 2024/6/14
 */
public class MybankFarmThirdLoginTest {
    protected static String APP_ID = "10000000000004096993";
    // gw.icloud.icbc.com.cn
    private static String BASE_URL = "http://ip:port/ui";
    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";

    public static void main(String[] args) throws Exception {
        String URI = "/mybank/farm/farmplatf/corpAuthUser/V1";

        //UiIcbcClient client = new UiIcbcClient(APP_ID, PRI_KEY, "UTF-8");
        UiIcbcClient client = new UiIcbcClient(APP_ID, "RSA2", PRI_KEY, "UTF-8");
        try {

            MybankFarmThirdRoleRequestV1 request = new MybankFarmThirdRoleRequestV1();
            MybankFarmThirdRoleRequestV1.MybankFarmThirdRoleAddRequestBizContent bizContent = new MybankFarmThirdRoleRequestV1.MybankFarmThirdRoleAddRequestBizContent();

            request.setServiceUrl(BASE_URL + URI);
            String symmetricKey = "{\"version\":\"V1\",\"alg\":\"SM4\",\"length\":\"128\",\"usage\":\"DEK\",\"info\":\"SUNCQ19MRlNfOV8wMDAwMF9TTTRfMTI4O0syMDIxMDkxNjAwMDAwMDAxNDE1O0ZTLUxGUzsyMDIxLTEwLTE1OzIwMjYtMTAtMTU=\",\"salt\":\"FXlJcVACaPV1DK/6V1jRnIiK/yX0GfmL5gSe9+GCOGU=\",\"iterate\":\"48\",\"key\":\"nEMZdn/sa2ixlm7i3DxFGgodP0XLlSXa7P7mFkvygOU=\",\"publicKey\":\"BChn+b8nEs/P3l52dzTWFC2a2JHtukpMobyu+usj6Jy0XLfCO9mi2gAl/ixqMERjV9ydHfKqAOeiBAxi3uoF4+Q=\",\"signature\":\"MEYCIQCYLdgKr6tWCMd61dXiyVzNgDdg7u3mfYeo7PSp4R2WgwIhAOwnBrt2J41nk3Joaq3QbOnwTyb37YlqsNXGXbhgRue2\"}";
            bizContent.setfSeqNo(encrypt(symmetricKey, "sjxfSeqNoAuth" + new Random().nextInt(1000)));
            bizContent.setFarmUserId(encrypt(symmetricKey, "SJXtestuserid001"));
            bizContent.setUserName(encrypt(symmetricKey, "sjx角色upt3"));
            bizContent.setInstId(encrypt(symmetricKey, "1101"));
            bizContent.setTranType(encrypt(symmetricKey, "1"));

            request.setBizContent(bizContent);
            System.out.println(JSONObject.toJSONString(request));
            System.out.println("success");
            System.out.println(client.buildPostForm(request));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }

    /**
     * SM4加密，失败时返回空字符串
     *
     * @param keyStr 秘钥
     * @param text   明文字符串
     * @return 密文字符串
     */
    public static String encrypt(String keyStr, String text) {
        return "";
    }
}
