...
import com.icbc.api.request.FinanceTppfinanceBuyproductRequestV1;
import com.icbc.api.request.FinanceTppfinanceBuyproductRequestV1.FinanceTppfinanceBuyproductRequestV1Biz;
import com.icbc.api.response.FinanceTppfinanceBuyproductResponseV1;

public class FinanceTppfinanceBuyproductV1Test {
	...
	public void test() throws IcbcApiException {
		...
		FinanceTppfinanceBuyproductRequestV1 request = new FinanceTppfinanceBuyproductRequestV1();
		request.setServiceUrl("https://ip:port/api/finance/tppfinance/buyproduct/V1");

		// 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值，非必输字段可不上送
		FinanceTppfinanceBuyproductRequestV1Biz bizContent = new FinanceTppfinanceBuyproductRequestV1Biz();
		bizContent.setProdcode("1701ELT");// 产品代码
		bizContent.setCurrtypestr("CNY");// 产品币种
		bizContent.setAmount("5000000");// 购买金额 (单位分）
		bizContent.setClient_id("ICBCTEST80001");// 客户ID
		bizContent.setRollflag("2");// 自动再投标志(1-是2-否)
		bizContent.setRollissue("0");// 自动再投期数
		...
		FinanceTppfinanceBuyproductResponseV1 response = new FinanceTppfinanceBuyproductResponseV1();
		...
		if (response.isSuccess()) {
			...
			System.out.println("applnum                 :       " + response.getApplnum()); // 申请单号
			System.out.println("prodname                :       " + response.getProdname()); // 产品名称
			System.out.println("currtypestr             :       " + response.getCurrtypestr()); // 产品币种
			System.out.println("dealprice               :       " + response.getDealprice()); // 成交价格
			System.out.println("dealcnt                 :       " + response.getDealcnt()); // 成交份额
			System.out.println("dealfee                 :       " + response.getDealfee()); // 手续费
