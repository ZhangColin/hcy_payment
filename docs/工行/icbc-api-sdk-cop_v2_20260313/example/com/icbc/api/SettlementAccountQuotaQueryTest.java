package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountQuotaQueryRequestV1;
import com.icbc.api.request.SettlementAccountQuotaQueryRequestV1.SettlementAccountQuotaQueryRequestV1Biz;
import com.icbc.api.response.SettlementAccountQuotaQueryResponseV1;

public class SettlementAccountQuotaQueryTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	protected static final String APP_ID = "10000000000000021587";


	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountQuotaQueryRequestV1 request = new SettlementAccountQuotaQueryRequestV1();
		SettlementAccountQuotaQueryRequestV1Biz bizContent = new SettlementAccountQuotaQueryRequestV1Biz();

		request.setServiceUrl("http://ip:port/api/settlement/account/quota/query/V1");
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setTrxAccDate("2017-03-15"); //合作方交易日期
		bizContent.setTrxAccTime("13:30:01"); //合作方交易时间
		bizContent.setCorpDate("2017-03-15"); //合作方工作日期
		bizContent.setCorpSerno("ABC123456789"); //合作方交易单号
		bizContent.setOutServiceCode("queryquota"); //外部服务代码
		bizContent.setMediumId1("6214760200000022233"); //工行联名卡号1
		bizContent.setSecretKey("ASDQWEQDZCSDFAWWQDA"); //sm4对称密钥
		request.setBizContent(bizContent);

		SettlementAccountQuotaQueryResponseV1 response = client.execute(request, "msgId");
		if (response.isSuccess() ) {
			//成功
			//response.getAccountBalance();
		} else {
			//失败
		}
	}
}
