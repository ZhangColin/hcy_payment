package com.icbc.api;

import com.icbc.api.request.CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1;
import com.icbc.api.response.CacaReceiptAllreceiptQuerytipsreceiptbyseqnoResponseV1;

import java.util.ArrayList;
import java.util.List;

public class CacaReceiptAllreceiptQuerytipsreceiptbyseqnoTestV1 {
    // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
    public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1 request = new CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1();
        request.setServiceUrl("https://ip:port/api/caca/receipt/allreceipt/querytipsreceiptbyseqno/V1");
        CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoRequestV1Biz bizContent = new CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoRequestV1Biz();

        // public 通讯区定义赋值
        CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoRequestV1Pub pubObj = new CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoRequestV1Pub();
        pubObj.setCode("29060");
        pubObj.setZone("0200");
        pubObj.setFr("0");
        pubObj.setDt("2017-07-06");
        // private 通讯区定义赋值
        CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoRequestV1In inObj = new CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoRequestV1In();

        List<CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoCondRequestV1In> cond = new ArrayList<CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoCondRequestV1In>();
        inObj.setZone("0200");
        inObj.setBrno("");
        inObj.setTelno("");
        inObj.setChannel("");
        CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoCondRequestV1In qTipsReceiptBySeqnoCondRequestV1In = new CacaReceiptAllreceiptQuerytipsreceiptbyseqnoRequestV1.QTipsReceiptBySeqnoCondRequestV1In();
        qTipsReceiptBySeqnoCondRequestV1In.setAccno("");
        qTipsReceiptBySeqnoCondRequestV1In.setCurrtype("");
        qTipsReceiptBySeqnoCondRequestV1In.setSerialno("");
        qTipsReceiptBySeqnoCondRequestV1In.setSadlindate("");
        cond.add(qTipsReceiptBySeqnoCondRequestV1In);
        inObj.setCond(cond);
        bizContent.setIn(inObj);
        bizContent.setPub(pubObj);
        request.setBizContent(bizContent);
        try {
            CacaReceiptAllreceiptQuerytipsreceiptbyseqnoResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}

