...
import com.icbc.api.request.JftApiVendorRescindUserProtocolRequestV1;
import com.icbc.api.request.JftApiVendorRescindUserProtocolRequestV1.JftApiVendorRescindUserProtocolRequestV1Biz;
import com.icbc.api.response.JftApiVendorRescindUserProtocolResponseV1;
 ...
 */
public class JftApiVendorRescindUserProtocolTestV1 {
	...
	public static void main(String[] args) throws Exception {
		...
		JftApiVendorRescindUserProtocolRequestV1 request = new JftApiVendorRescindUserProtocolRequestV1();

		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/jft/api/vendor/rescind/user/protocol/V1");

		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		JftApiVendorRescindUserProtocolRequestV1Biz bizContent = new JftApiVendorRescindUserProtocolRequestV1Biz();
		bizContent.setAppId("10000000000000012150");
		bizContent.setOutUserId("150404199108026335");
		bizContent.setProtocolType("0003");
		bizContent.setAccountNo("6222020200000011615");
		bizContent.setProtocolId("");

		request.setBizContent(bizContent);

		JftApiVendorRescindUserProtocolResponseV1 response;
		try {
			response = (JftApiVendorRescindUserProtocolResponseV1) client.execute(request, System.currentTimeMillis() + "");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
