package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.CacaReceiptAllreceiptQueryallreceiptRequestV1;
import com.icbc.api.response.CacaReceiptAllreceiptQueryallreceiptResponseV1;

public class CacaReceiptAllreceiptQueryallreceiptTestV1 {
    // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
    public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CacaReceiptAllreceiptQueryallreceiptRequestV1 request = new CacaReceiptAllreceiptQueryallreceiptRequestV1();
        request.setServiceUrl("https://ip:port/api/caca/receipt/allreceipt/queryallreceipt/V1");
        CacaReceiptAllreceiptQueryallreceiptRequestV1.QueryAllReceiptRequestV1Biz bizContent = new CacaReceiptAllreceiptQueryallreceiptRequestV1.QueryAllReceiptRequestV1Biz();
        JSONObject input = new JSONObject();
        // public 通讯区定义赋值
        CacaReceiptAllreceiptQueryallreceiptRequestV1.QueryAllReceiptRequestV1Pub pubObj = new CacaReceiptAllreceiptQueryallreceiptRequestV1.QueryAllReceiptRequestV1Pub();
        pubObj.setCode("29056");
        pubObj.setZone("0200");
        pubObj.setFr("0");
        pubObj.setDt("2017-07-06");
        // private 通讯区定义赋值
        CacaReceiptAllreceiptQueryallreceiptRequestV1.QueryAllReceiptRequestV1In inObj = new CacaReceiptAllreceiptQueryallreceiptRequestV1.QueryAllReceiptRequestV1In();
        inObj.setHuidanflag("1");
        inObj.setCard("4580651200001111111");
        inObj.setZone("0200");
        inObj.setBrno("5555");
        inObj.setTelno("");
        inObj.setChannel("203");
        inObj.setAccno("02000241000011111");
        inObj.setSeqaccno("000000001");
        inObj.setCurrtype("001");
        inObj.setPrint_flag("1");
        inObj.setDateB("2009-06-01");
        inObj.setDateE("2009-06-30");
        inObj.setAmountB("100");
        inObj.setAmountE("1000");
        inObj.setMgpayacc("");
        inObj.setDetailflag("");
        inObj.setDrcrf("0");
        inObj.setInit_flag("1");
        inObj.setSeqbusidate("");
        inObj.setSeqhuidantype("");
        inObj.setSeqaccno("");
        inObj.setSeqcurrtype("");
        inObj.setSeqmgpayacc("");
        inObj.setSeqserialno("");
        bizContent.setIn(inObj);
        bizContent.setPub(pubObj);
        request.setBizContent(bizContent);
        try {
            CacaReceiptAllreceiptQueryallreceiptResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}

