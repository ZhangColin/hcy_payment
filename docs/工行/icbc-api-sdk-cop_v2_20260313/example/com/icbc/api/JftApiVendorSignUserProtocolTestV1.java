package com.icbc.api.test;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.JftApiVendorSignUserProtocolRequestV1;
import com.icbc.api.request.JftApiVendorSignUserProtocolRequestV1.JftApiVendorSignUserProtocolRequestV1Biz;
import com.icbc.api.response.JftApiVendorSignUserProtocolResponseV1;

/**
 * 聚富通签订e支付小额免密协议接口
 * 
 * 请商户配置SDK中的lib文件夹中的jar包后，根据备注1-6提示进行数据替换
 */
public class JftApiVendorSignUserProtocolTestV1 {

	//1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//2、appid
	protected static final String APP_ID = "10000000000000012146";
	//3、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	public static void main(String[] args) throws Exception {

		//签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, "UTF-8", "json",
				APIGW_PUBLIC_KEY, "AES", "5xGJdh7qb+B95SUoxDlatg==", "", "");

		JftApiVendorSignUserProtocolRequestV1 request = new JftApiVendorSignUserProtocolRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/jft/api/vendor/sign/user/protocol/V1");
		
		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		JftApiVendorSignUserProtocolRequestV1Biz bizContent = new JftApiVendorSignUserProtocolRequestV1Biz();
		bizContent.setAppId(APP_ID);
		bizContent.setOutUserId("0720909");
		bizContent.setProtocolType("0003");
		bizContent.setAccountNo("6232290200000171718");
		bizContent.setAccountFlag("1");
		bizContent.setMobile("13845845886");
		bizContent.setMessageNo("789545");
		bizContent.setVerifyCode("145878");
		bizContent.setCertNo("230702198901120018");
		bizContent.setCertType("0");
		bizContent.setName("大富贵");
		bizContent.setDefaultCard("0");
		bizContent.setBussinessNo("12387489274");
		bizContent.setSerialNo("12312312312312312313");
		bizContent.setBusinessType("");
		bizContent.setBusinessValue("");

		//新增protocolType=0007时必填
		bizContent.setProtocolSignOrderId("1513848431848156");
		bizContent.setAccountToken("1533515849846466");

		request.setBizContent(bizContent);

		JftApiVendorSignUserProtocolResponseV1 response;
		try {
			response = (JftApiVendorSignUserProtocolResponseV1) client.execute(request, System.currentTimeMillis()+"");
			if (response.isSuccess()) {
				//6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("response:" + response);
			} else {
				//失败
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("ReturnMsg:"+response.getReturnMsg());
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
