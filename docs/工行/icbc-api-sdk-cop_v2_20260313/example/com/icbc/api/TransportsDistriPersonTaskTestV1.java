package com.icbc.api;

import com.icbc.api.request.TransportsDistriPersonTaskRequestV1;
import com.icbc.api.response.TransportsDistriPersonTaskResponseV1;
import org.junit.Assert;
import org.junit.Test;

/**
 * @program: icbc-api-sdk-cop
 * @description 清分-清分员操作服务（测试类）
 * @author: cwp
 * @create: 2021-6-3 09:50:38
 **/
public class TransportsDistriPersonTaskTestV1 {
    //密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    //网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //应用ID，APP申请分配appid
    protected static final String APP_ID = "xxxxxxxxx";


    public void test_cop() throws IcbcApiException {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        TransportsDistriPersonTaskRequestV1 request = new TransportsDistriPersonTaskRequestV1();
        //根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://IP:PORT/api/mybank/pay/transport/transportsdistriperson/V1");

        TransportsDistriPersonTaskRequestV1.ReqPubField pubField = new TransportsDistriPersonTaskRequestV1.ReqPubField();
        pubField.setChancode("702");
        pubField.setOapp("F-BIOM");
        pubField.setZoneno(200);

        TransportsDistriPersonTaskRequestV1.ReqPriField priField = new TransportsDistriPersonTaskRequestV1.ReqPriField();
        priField.setBranchId("0020000002");
        priField.setQrcode("32091021234000001020");
        priField.setTellerno("020000674");

        TransportsDistriPersonTaskResponseV1 response = client.execute(request, System.currentTimeMillis() + "");//请求通讯唯一编号
        boolean testFlag = true;
        if (response.isSuccess() && response.getReturnCode() == 0) {// 业务成功处理
            Assert.assertTrue(testFlag);
        } else {// 失败
            Assert.assertFalse(testFlag);
        }
    }
}
