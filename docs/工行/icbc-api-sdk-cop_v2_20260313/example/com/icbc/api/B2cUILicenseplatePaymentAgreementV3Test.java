package com.icbc.api;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.B2cUiLicenseplatePaymentAgreementRequestV3;
import com.icbc.api.request.B2cUiLicenseplatePaymentAgreementRequestV3.B2cUiLicenseplatePaymentAgreementRequestV3Biz;


public class B2cUiLicenseplatePaymentAgreementV3Test {
		
	//appid
	protected static final String APP_ID = "*************";
	
	//合作方私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	//AES秘钥
	protected static final String AES_KEY = "YBlenAPT1KedFOoHIfSa5w==";
	
	//测试案例
	public static void main(String[] args) throws IcbcApiException {
		
		//需传入appid，合作方私钥和AES密钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2
	    UiIcbcClient client = new UiIcbcClient(APP_ID,IcbcConstants.SIGN_TYPE_RSA2,MY_PRIVATE_KEY,IcbcConstants.CHARSET_UTF8,IcbcConstants.ENCRYPT_TYPE_AES,AES_KEY);
			
	    B2cUiLicenseplatePaymentAgreementRequestV3 request = new B2cUiLicenseplatePaymentAgreementRequestV3();        
			
		//根据测试环境和生产环境替换相应ip和端口http://gw.dccnet.com.cn/
	    request.setServiceUrl("https://open.icbc.com.cn/ui/b2c/ui/licenseplate/payment/agreement/V3");
			
	    //对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
	    B2cUiLicenseplatePaymentAgreementRequestV3Biz bizContent = new B2cUiLicenseplatePaymentAgreementRequestV3Biz();
	    StringBuffer tranDataBuf = new StringBuffer();
	    tranDataBuf.append("ePayMobilePlate=18888888888");
	    tranDataBuf.append("&clientBackUrl=http://XXXXX/XXXXX");
	    tranDataBuf.append("&eventID=fgdgdgszdsgds");
	    tranDataBuf.append("&plateno=%BB%A6SH66532");
	    tranDataBuf.append("&platenoColor=2");
	    tranDataBuf.append("&merBackURL=http://XXXXX/XXXXX");
	    tranDataBuf.append("&isSupportParter=1");
	    tranDataBuf.append("&isSupportH5Face=1");
	    tranDataBuf.append("&custCertType=0");
	    tranDataBuf.append("&custCertNum=111122223333111111");
	    tranDataBuf.append("&custName=%D0%A1%C3%F7");
	    tranDataBuf.append("&cardNum=1111222233331111111");
	    
	    String tranData = tranDataBuf.toString();
		bizContent.setInterfaceName("ICBC_LICENSE_PLATE_PAY");
		bizContent.setTranData(tranData);
		bizContent.setInterfaceVersion("ip:port");
		request.setBizContent(bizContent);
			
		/**
		 * 通过 client.buildPostForm(request) 方式组建交易报文，
		 * 报文组建完毕后通过页面进行提交完成请求发送过程
		 */
		System.out.println(client.buildPostForm(request));
	}
	
}
