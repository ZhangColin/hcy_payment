package com.icbc.api;

import com.icbc.api.request.MybankAccountAccountdetailserviceAdsaccountdtlqryRequestV1;
import com.icbc.api.response.MybankAccountAccountdetailserviceAdsaccountdtlqryResponseV1;

public interface MybankAccountAccountdetailserviceAdsaccountdtlqryTestV1 {
    public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //4、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
        MybankAccountAccountdetailserviceAdsaccountdtlqryRequestV1 request = new MybankAccountAccountdetailserviceAdsaccountdtlqryRequestV1();
        request.setServiceUrl("http://ip:port/com.icbc.api/mybank/account/accountdetailservice/adsaccountdtlqry/V1");

        MybankAccountAccountdetailserviceAdsaccountdtlqryRequestV1.AccountdetailserviceAdsaccountdtlqryRequestV1Biz bizContent = new MybankAccountAccountdetailserviceAdsaccountdtlqryRequestV1.AccountdetailserviceAdsaccountdtlqryRequestV1Biz();
        bizContent.setAcccompno("234156");
        bizContent.setAccount("345213001");
        bizContent.setCorpno("2350001231");
        bizContent.setFseqno("45612354");
        bizContent.setCurrtype("1");
        bizContent.setSerialno("1475896321");
        bizContent.setStartdate("2021-07-01");
        bizContent.setEnddate("2022-07-01");
        bizContent.setAgreeno("321001");

        request.setBizContent(bizContent);
        MybankAccountAccountdetailserviceAdsaccountdtlqryResponseV1 response;
        try {
            //测试时，避免apip识别成重放，请修改msgId,每次跑，递加1比如
            response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
                //Assert.assertEquals(response.getReturnMsg(), "success");
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
