...
import com.icbc.api.request.JftApiPayRevokePosOrderRequestV1;
import com.icbc.api.request.JftApiPayRevokePosOrderRequestV1.JftApiPayRevokePosOrderRequestV1Biz;
import com.icbc.api.response.JftApiPayRevokePosOrderResponseV1;

public class JftApiPayRevokePosOrderTestV1 {
	...
	public static void main(String[] args) {
		...
		JftApiPayRevokePosOrderRequestV1 request = new JftApiPayRevokePosOrderRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/pos/api/pay/order/revoke/V1");
		JftApiPayRevokePosOrderRequestV1Biz bizContent = new JftApiPayRevokePosOrderRequestV1Biz();
		bizContent.setAppId(APP_ID);					//平台商户标识：提交支付请求的平台商户编号                                                                      
		bizContent.setOutVendorId("10000000000000002"); //子商户标识：提交支付请求的子商户编号                                                                        
		bizContent.setOutOrderId("2018040908");         //平台对接方ID：平台商户订单ID，确保唯一	                                                                    
		bizContent.setOperateType("01");				//操作类型 固定值 01	                                                                    
		request.setBizContent(bizContent);
		JftApiPayRevokePosOrderResponseV1 response;
		try {
			response = client.execute(request, System.currentTimeMillis()+"");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
			if (response.isSuccess()) {
				//业务成功处理，请根据接口文档获取同步返回的业务数据
