package com.icbc.api;

import com.icbc.api.request.NfpwChainListqryRequestV1;
import com.icbc.api.request.NfpwChainListqryRequestV1.NfpwChainListqryRequestV1Biz;

// 网络融资产业链信息列表查询服务（页面）
public class MallNfpChainUiNfpchainlistqryTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APP_ID = "10000000000002354554";
	protected static final String SERVICE_URL = "http://ip:port/ui/mall/nfp/chain/ui/nfpchainlistqry/V1";

	public static void main(String[] args) {
		String buildPostForm = "";
		UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
		NfpwChainListqryRequestV1 request = new NfpwChainListqryRequestV1();
		request.setServiceUrl(SERVICE_URL);
		NfpwChainListqryRequestV1Biz bizContent = new NfpwChainListqryRequestV1Biz();
		bizContent.setAppid(APP_ID);
		bizContent.setOutVendorId("190002");
		bizContent.setQueryType("0");
		request.setBizContent(bizContent);
		try {
			buildPostForm = client.buildPostForm(request);
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
		System.out.println(buildPostForm);
	}
}
