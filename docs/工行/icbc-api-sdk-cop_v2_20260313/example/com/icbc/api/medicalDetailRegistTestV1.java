package com.icbc.ndf.jft.b2bpay.controller;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.JftPayB2bMedicalDetailInfoRequestV1;
import com.icbc.api.response.JftApiB2bpayMedicalDetailResponseV1;

import java.util.ArrayList;
import java.util.List;

public class medicalDetailRegistTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000205510";

    protected static final String AES_Key = "5xGJdh7qb+B95SUoxDlatg==";

    public static void main(String[] args) throws IcbcApiException {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8",
                "json",APIGW_PUBLIC_KEY,"AES",AES_Key, "", "");
        JftPayB2bMedicalDetailInfoRequestV1 request = new JftPayB2bMedicalDetailInfoRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/pay/b2bpay/medicalDetailRegist/V1");
        JftPayB2bMedicalDetailInfoRequestV1.JftPayB2bMedicalDetailInfoRequestV1Biz
                bizContent = new JftPayB2bMedicalDetailInfoRequestV1.JftPayB2bMedicalDetailInfoRequestV1Biz();
        bizContent.setAppId("10000000000001234567");
        bizContent.setOutUserId("71810240115");
        bizContent.setOutVendorId("http://www.ceshi.com");
        bizContent.setDetailBatchId("100000222334");
        List<JftPayB2bMedicalDetailInfoRequestV1.JftPayB2bMedicalDetailInfoRequestV1Biz.detail> detailList =new ArrayList<>();
        JftPayB2bMedicalDetailInfoRequestV1.JftPayB2bMedicalDetailInfoRequestV1Biz.detail detail = new JftPayB2bMedicalDetailInfoRequestV1.JftPayB2bMedicalDetailInfoRequestV1Biz.detail();
        detail.setDetailNo("123123");
        detail.setTransactionType("00");
        detail.setCompletionTime("2024-06-17 16:22:22");
        detail.setProductType("J20");
        detail.setUniqueCode("0001");
        detailList.add(detail);
        bizContent.setDetailList(detailList);
        bizContent.setTrxIp("00.xxx.xxx.00");
        bizContent.setMac("1d1sa1d2ssss");
        request.setBizContent(bizContent);
        JftApiB2bpayMedicalDetailResponseV1 response;
        try {
            response = (JftApiB2bpayMedicalDetailResponseV1) client.execute(request,
                    System.currentTimeMillis() + "11");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
            if (response.isSuccess()) {
                System.out.println("ReturnCode:" + response.getReturnCode());
                System.out.println("response:" + response);
            } else {
                //失败
                System.out.println("ReturnCode:" + response.getReturnCode());
                System.out.println("ReturnMsg:" + response.getReturnMsg());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
