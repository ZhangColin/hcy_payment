package com.icbc.api;

import java.util.Random;
import java.util.logging.Logger;

import com.icbc.api.request.CorpgroupInfoQueryRequestV1;
import com.icbc.api.request.CorpgroupInfoQueryRequestV1.CorpgroupInfoQueryRequestV1Biz;
import com.icbc.api.request.CorpgroupInfoQueryRequestV1.CorpgroupInfoQueryRequestV1Biz.ChanCommV10;
import com.icbc.api.response.CorpgroupInfoQueryResponseV1;

public class CorpgroupInfoQueryTest {
	
	protected static final String MY_PRIVATE_KEY = "";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "0200EH0005040"; 

	protected static final String password = "12345678";

	protected static final String MY_PUB_KEY = "-----请使用正确的APP私钥----------请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----";

	public static void main(String[] args) {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				MY_PRIVATE_KEY, APIGW_PUBLIC_KEY, MY_PUB_KEY, password);

		CorpgroupInfoQueryRequestV1 request = new CorpgroupInfoQueryRequestV1();
		//地址(如果是内部API，地址需要使用http而不是https)
		request.setServiceUrl("https://ip:port/api/corpgroup/info/query/V1");
		CorpgroupInfoQueryRequestV1Biz bizContent = new CorpgroupInfoQueryRequestV1Biz();
		bizContent.setGROUPID("999999999");
		ChanCommV10 chanCommV10 = bizContent.new ChanCommV10();
		chanCommV10.setSerialno("111111111");
		bizContent.setChanCommV10(chanCommV10);
//		bizContent.setAppBatserialNo("111111");
//			bizContent.setAppSerialNo("11111-1234567890");
//			bizContent.setAppRemark("abc100");
//			bizContent.setStart_Id("1");
//			bizContent.setEnd_Id("50");
		request.setBizContent(bizContent);
		CorpgroupInfoQueryResponseV1 response;

		Random rand = new Random();
		String msgId = rand.nextInt(99999) + "msg";

		try {
			Logger log = Logger.getLogger(CorpgroupInfoQueryTest.class.getName());
			log.info("msgId:"+ msgId);
			response = client.execute(request, msgId);
			if (response.getReturnCode() == 0) {
				log.info("ReturnCode:"+ response.getReturnCode()+"\n Response:"+response.getReturnMsg());
			} else {
				log.info("ReturnCode:"+ response.getReturnCode()+"\n Response:"+response.getReturnMsg());
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}	
		
	}

}
