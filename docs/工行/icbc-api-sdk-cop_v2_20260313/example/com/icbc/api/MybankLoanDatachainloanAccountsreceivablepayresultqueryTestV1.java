package com.icbc.api;

import com.icbc.api.request.MybankLoanDatachainloanAccountsreceivablepayresultqueryRequestV1;
import com.icbc.api.request.MybankLoanDatachainloanAccountsreceivablepayresultqueryRequestV1Biz;
import com.icbc.api.response.MybankLoanDatachainloanAccountsreceivablepayresultqueryResponseV1;
import org.apache.commons.logging.Log;
import org.apache.commons.logging.LogFactory;

public class MybankLoanDatachainloanAccountsreceivablepayresultqueryTestV1 {
	private static Log log = LogFactory.getLog(MybankLoanDatachainloanAccountsreceivablepayresultqueryTestV1.class);

	protected static String APP_ID = "xxxx";
	    protected static String APIGW_PUBLIC_KEY =
	            "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16"
	            +"bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL"
	            +"T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP"
	            +"3CiNgg0W6O3AGqwIDAQAB";
	    protected static String PRI_KEY =
	    		"-----请使用正确的APP私钥-----";
	    protected static String BASE_URL = "http://ip:port/api";
	
	public static void main(String[] args) {
		String URI = "mybank/loan/datachainloan/orderwithrepayresultquery/V1";
     DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
     try {

		 MybankLoanDatachainloanAccountsreceivablepayresultqueryRequestV1Biz bizContent =
                 new MybankLoanDatachainloanAccountsreceivablepayresultqueryRequestV1Biz();

		 bizContent.setSerialNo("201805131234567");
		 bizContent.setAppNo("F-APIP");
		 bizContent.setAreaCode("");
		 bizContent.setEmployeeCode("");
		 bizContent.setLanguage("zh_CN");
		 bizContent.setTransNo("IDataChainLoanCocoaSerivce");
		 bizContent.setVer("1.0");
		 bizContent.setClientFlatID("PF001");
		 bizContent.setBorrowerName("XXXXX");
		 bizContent.setBorrowerCerType("3");
		 bizContent.setBorrowerCerId("110611199901010028");
		 bizContent.setRepayInstruct("20231000243112");

		 MybankLoanDatachainloanAccountsreceivablepayresultqueryRequestV1 request =
                 new MybankLoanDatachainloanAccountsreceivablepayresultqueryRequestV1();
         request.setServiceUrl(BASE_URL + "/" + URI);
         request.setBizContent(bizContent);
		 MybankLoanDatachainloanAccountsreceivablepayresultqueryResponseV1 response = client.execute(request);
		 if (response.isSuccess()) {
			 String a = response.getReturn_content().toString();
			 log.info(a);
		 } else {
			 String a = response.getReturn_content().toString();
			 log.error(a);
		 }

     } catch (Exception e) {
         e.printStackTrace();
     }
	}
}
