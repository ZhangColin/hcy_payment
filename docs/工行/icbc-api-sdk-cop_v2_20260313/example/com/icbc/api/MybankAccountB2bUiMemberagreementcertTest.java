package com.icbc.api;
import java.util.regex.Matcher;
import java.util.regex.Pattern; 
import com.icbc.api.IcbcConstants;
import com.icbc.api.UiIcbcClient;
import com.icbc.api.request.MybankAccountB2bUiMemberagreementcertRequestV1;
import com.icbc.api.request.MybankAccountB2bUiMemberagreementcertRequestV1.MybankAccountB2bUiMemberagreementcertRequestV1Biz;
public class MybankAccountB2bUiMemberagreementcertTest {


	protected static final String MY_PRIVATE_KEY = "";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APP_ID = "10000000000000004447";
	
	protected static final String PASSWORD = "12345678";
	

	public static void main(String args[]) throws IcbcApiException {
		// 构造client对象
		String castr = APIGW_PUBLIC_KEY;
		// 去除签名数据及证书数据中的空格 added for Safari
		Pattern p = Pattern.compile("\\s*|\t");
		Matcher m2 = p.matcher(castr);
		castr = m2.replaceAll("");
		UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8, castr, PASSWORD);
		
		// 设置请求对象request
		MybankAccountB2bUiMemberagreementcertRequestV1 request = new MybankAccountB2bUiMemberagreementcertRequestV1();

		MybankAccountB2bUiMemberagreementcertRequestV1Biz bizContent = new MybankAccountB2bUiMemberagreementcertRequestV1Biz();
		
		// 设置请求路径
		// 设置请求路径
		request.setServiceUrl("http://xxx.xxx.xxx.xxx:xxx/ui/mybank/account/b2b/ui/memberagreementcert/V1");

		bizContent.setApiName("MULTIPAY");
		bizContent.setApiVersion("001.002.001.001");
		bizContent.setPartnerProno("1800137");
		bizContent.setMemberNo("2018092103");	
		request.setBizContent(bizContent);
		// 生成自提交的表单返回客户浏览器，该表单会自动提交完成调用
		System.out.println(client.buildPostForm(request));
	}	

}
