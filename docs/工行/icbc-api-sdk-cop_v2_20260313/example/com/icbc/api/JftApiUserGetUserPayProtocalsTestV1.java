package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiUserGetUserPayProtocalsRequestV1;
import com.icbc.api.request.JftApiUserGetUserPayProtocalsRequestV1.JftApiUserGetUserPayProtocalsRequestV1Biz;
import com.icbc.api.response.JftApiUserGetUserPayProtocalsResponseV1;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;

public class JftApiUserGetUserPayProtocalsTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000108514";


    public void test_cop() {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","eyiSiHvFqYFLu/AZDCt+4w==", "", "");
        JftApiUserGetUserPayProtocalsRequestV1 request = new JftApiUserGetUserPayProtocalsRequestV1();
        request.setServiceUrl("http://gateway-cfjs-app.sdc.cs.icbc:9080/api/jft/api/user/get/sign/protocol/V1");
        JftApiUserGetUserPayProtocalsRequestV1Biz bizContent = new JftApiUserGetUserPayProtocalsRequestV1Biz();
        bizContent.setAppId("10000000000000108514");
        bizContent.setOutUserId("21543653456546");
        bizContent.setProtocolType("0003");

        request.setBizContent(bizContent);
        JftApiUserGetUserPayProtocalsResponseV1 response;
        try {
            response = (JftApiUserGetUserPayProtocalsResponseV1) client.execute(request, System.currentTimeMillis()+"");

            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
  
  	public static void main(String[] args) {
        new JftApiUserGetUserPayProtocalsTestV1().test_cop();
    }    
}
