package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.SstsQuerySequenceNoRequestV1;
import com.icbc.api.response.SstsQuerySequenceNoResponseV1;
import org.junit.Test;

import java.util.HashMap;

public class SstsQuerySequenceNoTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	protected static final String APP_ID = "10000000000000003402";


	public void test_cop() throws IcbcApiException {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		SstsQuerySequenceNoRequestV1 request = new SstsQuerySequenceNoRequestV1();
		request.setServiceUrl("http://ip:portapi/ssts/envelope/query/V1");

		SstsQuerySequenceNoRequestV1.SstsFhAddTranlogRequestV1Biz bizContent = new SstsQuerySequenceNoRequestV1.SstsFhAddTranlogRequestV1Biz();
		bizContent.setBaseinfo(new HashMap<>());
		bizContent.setChanCommV10(new HashMap<>());
		bizContent.setInfoCommV10(new HashMap<>());
		request.setBizContent(bizContent);

		SstsQuerySequenceNoResponseV1 response = client.execute(request);
		System.out.println(JSONObject.toJSONString(response));
		if (response.isSuccess() ) {
			// 业务成功处理
			System.out.println("处理成功");
		} else {
			// 失败
			System.out.println("处理失败");
		}

	}

}
