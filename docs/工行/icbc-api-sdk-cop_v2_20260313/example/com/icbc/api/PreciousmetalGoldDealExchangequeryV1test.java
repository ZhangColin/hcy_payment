package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.PreciousmetalGoldDealExchangequeryRequestV1;
import com.icbc.api.request.PreciousmetalGoldDealExchangequeryRequestV1.PreciousmetalGoldDealExchangequeryRequestV1Biz;
import com.icbc.api.response.PreciousmetalGoldDealExchangequeryResponseV1;

public class PreciousmetalGoldDealExchangequeryV1test {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002166";

	
	public void test_cop() throws IcbcApiException {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		PreciousmetalGoldDealExchangequeryRequestV1 request = new PreciousmetalGoldDealExchangequeryRequestV1();
		request.setServiceUrl("http://ip:port/api/preciousmetal/gold/deal/V1/exchangequery");

		PreciousmetalGoldDealExchangequeryRequestV1Biz bizContent = new PreciousmetalGoldDealExchangequeryRequestV1Biz();
		bizContent.setTrxSeqNo("123456");
		bizContent.setTrxTs("2017-04-07 17:15:12.762126");
	    
		request.setBizContent(bizContent);

		PreciousmetalGoldDealExchangequeryResponseV1 response = client.execute(request, "msgId");

		System.out.println(JSONObject.toJSONString(response));
		if (response.isSuccess() && response.getSubFlag() == 1) {
			// 业务成功处理
			System.out.println(response.getCustNo());
		} else {
			// 失败
		}

	}
}

