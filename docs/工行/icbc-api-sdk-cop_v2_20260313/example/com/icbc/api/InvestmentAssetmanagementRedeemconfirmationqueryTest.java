package com.icbc.api;

import com.icbc.api.request.InvestmentAssetmanagementRedeemconfirmationqueryRequestV1;
import com.icbc.api.request.InvestmentAssetmanagementRedeemconfirmationqueryRequestV1.InvestmentAssetmanagementRedeemconfirmationqueryRequestV1Biz;
import com.icbc.api.response.InvestmentAssetmanagementRedeemconfirmationqueryResponseV1;

import java.util.logging.Logger;


public class InvestmentAssetmanagementRedeemconfirmationqueryTest {

	//appid
	protected static String APP_ID = "xxxxxxxxxxxxx";
	protected static String BASE_URL = "http://ip:port/api";
	protected static Logger log = Logger.getLogger(InvestmentAssetmanagementRedeemconfirmationqueryTest.class.getName());

	//公钥
	protected static String APIGW_PUBLIC_KEY = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----";
	//私钥
	protected static String PRI_KEY = "-----请使用正确的APP私钥-----";

	public static void main(String[] args) throws Exception {
		String URI = "investment/assetmanagement/redeemconfirmationquery/V1"; // api��ַ
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
		try {

			//
			InvestmentAssetmanagementRedeemconfirmationqueryRequestV1 request = new InvestmentAssetmanagementRedeemconfirmationqueryRequestV1();
			// bizContentʵ��
			InvestmentAssetmanagementRedeemconfirmationqueryRequestV1Biz bizContent = new InvestmentAssetmanagementRedeemconfirmationqueryRequestV1Biz();

			//
			bizContent.settASerialNO("4501");

			// ����ʵ�� ��������
			request.setServiceUrl(BASE_URL + "/" + URI);
			request.setBizContent(bizContent);

			log.info(request.getMethod());
			InvestmentAssetmanagementRedeemconfirmationqueryResponseV1 response = client.execute(request);
			if (response.isSuccess()) {
				log.info("success");//
			} else {
				log.info("error");
			}

		} catch (Exception e) {
			e.printStackTrace();
			log.info(e.getMessage());
		}
	}

}
