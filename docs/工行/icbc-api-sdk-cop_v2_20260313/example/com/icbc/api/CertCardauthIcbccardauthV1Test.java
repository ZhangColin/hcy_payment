...
import com.icbc.api.request.CertCardauthIcbccardauthRequestV1;
import com.icbc.api.request.CertCardauthIcbccardauthRequestV1.CertCardauthIcbccardauthRequestV1Biz;
import com.icbc.api.response.CertCardauthIcbccardauthResponseV1;

import java.util.logging.Logger;

public class CertCardauthIcbccardauthV1Test {
	public static void main(String[] args) {
		Logger log = Logger.getLogger(CertCardauthIcbccardauthV1Test.class.getName());
		// 1、APPID
		String APP_ID = "XXX"; //功能环境
		// 2、网关公钥
		...
		CertCardauthIcbccardauthRequestV1 request = new CertCardauthIcbccardauthRequestV1();
		CertCardauthIcbccardauthRequestV1Biz bizContent = new CertCardauthIcbccardauthRequestV1Biz();

		bizContent.setCertType("0"); // 证件类型
		bizContent.setCertNo("XXX"); // 证件号码
		bizContent.setCustName("张圣"); // 户名
		bizContent.setCardno("XXXX"); // 介质编号
		bizContent.setMobile("18820423459"); // 手机号码
		bizContent.setSendMsgFlag("1");

//		request.setServiceUrl("ip:port/api/cert/cardauth/icbccardauth/V1"); //功能环境-外网
		request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cert/cardauth/icbccardauth/V1"); //功能环境-外网域名
		request.setBizContent(bizContent);
		request.isNeedEncrypt();

		String msg_id = String.valueOf(System.currentTimeMillis());
		try {
			CertCardauthIcbccardauthResponseV1 response = client.execute(request, msg_id);// 第二个参数需要用随机数，APi网关会做防重复检查
