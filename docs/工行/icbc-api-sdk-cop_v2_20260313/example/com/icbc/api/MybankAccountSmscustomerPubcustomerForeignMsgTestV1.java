package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcConstants;
import com.icbc.api.request.MybankAccountSmscustomerPubcustomerForeignMsgRequestV1;
import com.icbc.api.request.MybankAccountSmscustomerPubcustomerForeignMsgRequestV1.MybankAccountSmscustomerPubcustomerForeignMsgRequestV1Biz;
import com.icbc.api.response.MybankAccountSmscustomerPubcustomerForeignMsgResponseV1;

import java.util.logging.Logger;

public class MybankAccountSmscustomerPubcustomerForeignMsgTestV1 {
	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	// 2、appid 通行证第三方测试1 (行外)
	protected static final String APP_ID = "xxxxxxxxxxxxxxxxxxxx";

	// 3、密钥对认证方式，公钥在API平台登记，此处为私钥 行外访问
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	private static Logger logger = Logger.getLogger(MybankAccountSmscustomerPubcustomerForeignMsgTestV1.class.getName());
	
	/**
	 * @param args
	 */
	public static void main(String[] args) {
		// 签名类型为SHA256WithRSA时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		MybankAccountSmscustomerPubcustomerForeignMsgRequestV1 request = new MybankAccountSmscustomerPubcustomerForeignMsgRequestV1();
		StringBuffer serviceUrl = new StringBuffer("http");
		serviceUrl.append("s:")
				  .append("//ip:port")
				  .append("/api/mybank/account/smscustomer/pubcustomer/foreignmsg/V1");
		// 4、根据测试环境和生产环境替换相应ip和端口
		// 行外流程
		request.setServiceUrl(serviceUrl.toString());

		// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		MybankAccountSmscustomerPubcustomerForeignMsgRequestV1Biz bizContent = new MybankAccountSmscustomerPubcustomerForeignMsgRequestV1Biz();

		bizContent.setApiCode("JWFH-API");//api接口调用方编码
		bizContent.setApiId("HWFH");//api接口资产编号
		bizContent.setCountryCode("国家代码");
		bizContent.setAreaCode("地区编码");
		bizContent.setPhoneNo("用户发送短信的时间");
		bizContent.setMsgContent("用户发送短信的时间");
		bizContent.setMsgTime("短信发送时间");
		bizContent.setSessionId("会话编号或事件流水");

		request.setBizContent(bizContent);
		// 建立返回项
		MybankAccountSmscustomerPubcustomerForeignMsgResponseV1 response = new MybankAccountSmscustomerPubcustomerForeignMsgResponseV1();
		try {
			response = client.execute(request);
			logger.info("return_code:" + response.getReturnCode());
			logger.info("return_msg:" + response.getReturnMsg());
			logger.info("tranerrorcode:" + response.getTranerrorcode());
			logger.info("tranerrormsg:" + response.getTranerrormsg());
		} catch (Exception e) {
			//异常处理
		}
	}
}
