package com.icbc.api;

import java.util.UUID;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountCloseacceptRequestV1;
import com.icbc.api.request.SettlementAccountCloseacceptRequestV1.SettlementAccountCloseAcceptRequestV1Biz;
import com.icbc.api.response.SettlementAccountCloseacceptResponseV1;

public class SettlementAccountCloseacceptTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountCloseacceptRequestV1 request = new SettlementAccountCloseacceptRequestV1();
		SettlementAccountCloseAcceptRequestV1Biz bizContent = new SettlementAccountCloseAcceptRequestV1Biz();
		request.setServiceUrl("ip:port/api/settlement/account/V1/closeaccept");
 
		bizContent.setCorpNo("corpInst1234");
		bizContent.setTrxAccDate("2017-03-15");
		bizContent.setTrxAccTime("13:30:01");
		bizContent.setCorpDate("2017-03-15");
		bizContent.setCorpSerno(UUID.randomUUID().toString());
		bizContent.setOutServiceCode("closeaccaccept");
		bizContent.setMediumId("6888888888888888888");
		bizContent.setSecretKey("ASDQWEQDZCSDFAWWQDA");
		bizContent.setMediumIdHash("SDFDFHTEWTGDFWADADAFSDGSESEFD");
		request.setBizContent(bizContent);
		SettlementAccountCloseacceptResponseV1 response = client.execute(request, "msgId");
		
		if (response.isSuccess() ) {
			//成功
		} else {
			// 失败
		}
	}
}
