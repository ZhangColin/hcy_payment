...
import com.icbc.api.request.EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1;
import com.icbc.api.response.EnterpriseSalaryAnnuityEmpmoendamtqryResponseV1;
import org.junit.Assert;
import org.junit.Test;

/**
 * @Author: kfzx-wanghong01
 * @Description: 历史资产月度明细数据查询测试案例
 * @Date: 2021/3/2
 */
public class EnterpriseSalaryAnnuityEmpmoendamtqryV1Test {
    ...
    public void test_cop() {
        ...
        //6、设置请求对象
        EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1 request = new EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1();
        //7、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("http://ip:port/api/enterprise/salary/annuity/empmoendamtqry/V1");
        //8、设置业务参数
        EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1.EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1Biz bizContent = new EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1.EnterpriseSalaryAnnuityEmpmoendamtqryRequestV1Biz();

        //9、组装请求参数
        bizContent.setSerialNo("8303020097321146625"); //流水号
        bizContent.setCustName("小1-100"); //人员姓名
        bizContent.setCertType("1"); //身份证号
        bizContent.setCertNo("412702199209252330"); //证件号码
        bizContent.setMobileNo("18103022030"); //移动电话号码
        bizContent.setQryType("3");//查询周期1-按月查询 2-按季度查询 3-按年查询 另：中石化接口查询类型只支持1和3
        bizContent.setBeginDate("2013-05-01");//开始日期 明细范围在24月以内或5年
        bizContent.setEndDate("2014-06-31");//结束日期 明细范围在24月以内或5年

        request.setBizContent(bizContent);

        EnterpriseSalaryAnnuityEmpmoendamtqryResponseV1 response = null;
        try {
            response = client.execute(request);
        } catch (IcbcApiException e) {
            e.printStackTrace();
            // 测试环境案例经常调不通 给个默认值
            response = new EnterpriseSalaryAnnuityEmpmoendamtqryResponseV1();
        } catch (Throwable e2) {
            e2.printStackTrace();
        }
        ...
        if (response.isSuccess()) {
            // 业务成功处理
            msgStr.append("【 EmpmoendamtqryV1 ReturnCode : ").append(response.getReturnCode()).append("】");
