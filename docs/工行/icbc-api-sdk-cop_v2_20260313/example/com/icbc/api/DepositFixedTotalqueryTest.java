package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.DepositFixedTotalqueryRequestV1;
import com.icbc.api.request.DepositFixedTotalqueryRequestV1.DepositFixedTotalqueryRequestV1Biz;
import com.icbc.api.response.DepositFixedTotalqueryResponseV1;

public class DepositFixedTotalqueryTest{
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		DepositFixedTotalqueryRequestV1 request = new DepositFixedTotalqueryRequestV1();
		DepositFixedTotalqueryRequestV1Biz bizContent = new DepositFixedTotalqueryRequestV1Biz();
		request.setServiceUrl("ip:port/api/deposit/fixed/totalquery/V1");
		bizContent.setCmExternalEventNo("20202020"); //合作方交易单号
		bizContent.setCmAppId("corpJd1234"); //合作方机构编号
		bizContent.setCmOutServiceCode("fixedtotalquery"); //外部服务代码
		bizContent.setCmWorkdate("2017-03-14"); //合作方交易日期
		bizContent.setCmWorktime("10:00:00"); //合作方交易时间
		bizContent.setCmSecretKey("h8ZujhDntpKRohwFmGXcnXygNjJHRQbyuVG3183uOdXI2fRgCXeDED9zOw5dO2JucVy7vZ19od7CSUxI2/sVOn6dPklVoaquVElgKRUPlr6D/cqLu25K7hJ3Pt/uOnx4qF/ykmO/IB2qS0rs/sp0Zw=="); 
		bizContent.setMediumNo("6214760200000022233"); //工行卡号
        request.setBizContent(bizContent);
		DepositFixedTotalqueryResponseV1 response = client.execute(request, "msgId");
		if (response.isSuccess() ) {
			//成功
			//response.getAccountBalance();
		} else {
			//失败
		}
	}
}
