package com.icbc.api;

import com.icbc.api.request.ITmasReqInfoListProRequestV1;
import com.icbc.api.response.ITmasReqInfoListProResponseV1;
import org.junit.Assert;
import org.junit.Test;

import java.util.logging.Logger;

/**
 * @program: icbc-api-sdk-cop
 * @description:人行调缴查询服务(测试类)
 * @author: kfzx-wants
 * @date: 2021/6/9
 **/
public class ITmasReqInfoListProTestV1 {
    private Logger logger = Logger.getLogger(ITmasReqInfoListProTestV1.class.getName());

    //密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    //网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //应用ID，APP申请分配appid
    protected static final String APP_ID = "xxxxxxxxx";


    public void test_cop() throws IcbcApiException {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        ITmasReqInfoListProRequestV1 request = new ITmasReqInfoListProRequestV1();
        //根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://IP:PORT/api/biom/transport/cash/queryTmasReqInfoList/V1");

        ITmasReqInfoListProRequestV1.RequestPubField pubField = new ITmasReqInfoListProRequestV1.RequestPubField();
        pubField.setChancode("702");
        pubField.setOapp("F-BIOM");
        pubField.setOtellerno("80868");
        pubField.setZoneno("200");

        ITmasReqInfoListProRequestV1.RequestPriField priField = new ITmasReqInfoListProRequestV1.RequestPriField();
        priField.setCentbrno("416");
        priField.setCentzoneno("200");
        priField.setAllocDate("2023-05-22");
        priField.setBegNum(1);
        priField.setFetchNum(5);
        priField.setReqType("5");
        priField.setStatus("10");

        ITmasReqInfoListProRequestV1.ITmasReqInfoListProRequestV1Biz bizContent = new ITmasReqInfoListProRequestV1.ITmasReqInfoListProRequestV1Biz();
        bizContent.setPubField(pubField);
        bizContent.setPriField(priField);

        request.setBizContent(bizContent);

        ITmasReqInfoListProResponseV1 response = client.execute(request, System.currentTimeMillis() + "");//请求通讯唯一编号
        boolean testFlag = true;
        if (response.isSuccess() && response.getReturnCode() == 0) {// 业务成功处理
            Assert.assertEquals(true, testFlag);
        } else {// 失败
            Assert.assertEquals(false, testFlag);
        }

    }
}
