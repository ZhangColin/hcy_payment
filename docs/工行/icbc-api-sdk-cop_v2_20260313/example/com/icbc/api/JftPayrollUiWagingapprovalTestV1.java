package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import java.util.logging.Logger;
import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.UiIcbcClient;
import com.icbc.api.request.JftPayrollUiWagingapprovalRequestV1;
import com.icbc.api.request.JftPayrollUiWagingapprovalRequestV1.JftPayrollUiWagingapprovalRequestV1Biz;

public class JftPayrollUiWagingapprovalTestV1 {
	 public static void main(String[] args) throws IcbcApiException  {
        String privateKey = "-----请使用正确的APP私钥-----";
        //  String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
       //  UiIcbcClient client=new UiIcbcClient("10000000000004097477",privateKey,IcbcConstants.CHARSET_UTF8);
    UiIcbcClient client = new UiIcbcClient("youappid",IcbcConstants.SIGN_TYPE_RSA2, privateKey, IcbcConstants.CHARSET_UTF8);
        
    JftPayrollUiWagingapprovalRequestV1 requestV1=new JftPayrollUiWagingapprovalRequestV1();
        requestV1.setServiceUrl("http://ip:port//ui/jft/payroll/wagingapproval/V1");
        Logger log = Logger.getLogger(JftPayrollUiWagingapprovalTestV1.class.getName());
        JftPayrollUiWagingapprovalRequestV1.JftPayrollUiWagingapprovalRequestV1Biz bizContent=new  JftPayrollUiWagingapprovalRequestV1.JftPayrollUiWagingapprovalRequestV1Biz();
        bizContent.setAppId("youappid");
        bizContent.setAppName("dd");
        bizContent.setPlatId("02000002040095");
        bizContent.setCorpId("201003081416290"); 
        requestV1.setBizContent(bizContent);

       
		//System.out.println(client.buildPostForm(requestV1));
	 
		log.info("交易=>" + client.buildPostForm(requestV1));
        
        
        
    // UiIcbcClient clien1t = new UiIcbcClient("10000000000004097477",IcbcConstants.SIGN_TYPE_RSA2, privateKey, IcbcConstants.CHARSET_UTF8);
 
       // System.out.println(clien1t);
  			 
  	 
    }
}
