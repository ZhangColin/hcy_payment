package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.B2cOrderRejectRequestV1;
import com.icbc.api.request.B2cOrderRejectRequestV1.B2cOrderRejectRequestV1Biz;
import com.icbc.api.request.B2cOrderRejectRequestV1.B2cOrderRejectRequestV1CommonInfo;
import com.icbc.api.request.B2cOrderRejectRequestV1.B2cOrderRejectRequestV1TradeInfo;
import com.icbc.api.response.B2cOrderRejectResponseV1;
import com.icbc.api.response.B2cOrderRejectResponseV1.B2cOrderRejectResponseV1CommonRetInfo;
import com.icbc.api.response.B2cOrderRejectResponseV1.B2cOrderRejectResponseV1TradeRetInfo;

public class B2cOrderRejectTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002116";


	public void test_CA() throws IcbcApiException {
		
		String castr = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				"",
				APIGW_PUBLIC_KEY, castr, "12345678");
		
		B2cOrderRejectRequestV1 request = new B2cOrderRejectRequestV1();
		request.setServiceUrl("http://ip:port/api/b2c/order/V1/reject");

		B2cOrderRejectRequestV1Biz bizContent = new B2cOrderRejectRequestV1Biz();
		
		B2cOrderRejectRequestV1CommonInfo commonInfo = new B2cOrderRejectRequestV1CommonInfo();
		commonInfo.setTransCode("EBUSCOM");
		commonInfo.setCis("020090002992836");
		commonInfo.setBankCode("102");
		commonInfo.setId("thy.y.0200");
		commonInfo.setTranDate("20150620");
		commonInfo.setTranTime("204438000");
		commonInfo.setFseqno("98111314511403031941352");
		
		B2cOrderRejectRequestV1TradeInfo tradeInfo = new B2cOrderRejectRequestV1TradeInfo();
		tradeInfo.setTranType("0");
		tradeInfo.setShopType("2");
		tradeInfo.setShopCode("0200EC23497696");
		tradeInfo.setShopAcct("0200080909200100554");
		tradeInfo.setOrderNum("201712021528390");
		tradeInfo.setPayDate("20170227");
		tradeInfo.setPayAmt("1");
		tradeInfo.setSignTime("2017020800000");
		
		bizContent.setCommonInfo(commonInfo);
		bizContent.setTradeInfo(tradeInfo);
		
		request.setBizContent(bizContent);

		B2cOrderRejectResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess()) {
			// 接收成功
			B2cOrderRejectResponseV1CommonRetInfo commonRetInfo = response.getCommonRetInfo();
			B2cOrderRejectResponseV1TradeRetInfo tradeRetInfo = response.getTradeRetInfo();
			if(commonRetInfo.getRetCode().equals("0")){
				if(tradeRetInfo.getResult().equals("1")){
					//指令成功
				}else{
					//根据返回结果继续判断
				}
			} else {
				//查询失败
				System.out.println(commonRetInfo.getRetMsg());
			}
		} else {
			// 失败
			System.out.println(response.getReturnMsg());
		}
	}
}
