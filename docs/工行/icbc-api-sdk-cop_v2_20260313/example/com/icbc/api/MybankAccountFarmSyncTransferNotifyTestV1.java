package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MybankAccountFarmSyncRefundNotifyRequestV1;
import com.icbc.api.request.MybankAccountFarmSyncTransferNotifyRequestV1;
import com.icbc.api.response.MybankAccountFarmSyncRefundNotifyResponseV1;
import com.icbc.api.response.MybankAccountFarmSyncTransferNotifyResponseV1;

public class MybankAccountFarmSyncTransferNotifyTestV1 {
	
protected static String APP_ID = "10000000000004096993";
	

	private  static String BASE_URL = "http://ip:port/api";
	

	protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
     





	public static void main(String[] args) throws Exception {
		String URI = "mybank/account/farm/syncTransferNotify/V1";

		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
		try {
			MybankAccountFarmSyncTransferNotifyRequestV1 request = new MybankAccountFarmSyncTransferNotifyRequestV1();
			MybankAccountFarmSyncTransferNotifyRequestV1.MybankAccountFarmSyncTransferNotifyRequestBizV1 requestBizV1 = new MybankAccountFarmSyncTransferNotifyRequestV1.MybankAccountFarmSyncTransferNotifyRequestBizV1();

			requestBizV1.setfSeqNo("1234567890");
			requestBizV1.setTransDate("20220202");
			requestBizV1.setCorpCode("icbc");
			requestBizV1.setParamType("1");
			requestBizV1.setDetail("{\"busiNo\":\"ZZ20220125000000027113\",\"billId\":\"2202050978878914\",\"propertyId\":\"ZZgovId05093\"," +
                    "\"sellerId\":\"ZZbusiClientId05093\",\"projectId\":\"ZZproductId123\",\"customerId\":\"288739134\",\"transferMode\":\"0\",\"transferAmt\":\"0\"}");

			request.setServiceUrl(BASE_URL +"/"+ URI);
			request.setBizContent(requestBizV1);
			
	        System.out.println(JSON.toJSONString(request));

	        MybankAccountFarmSyncTransferNotifyResponseV1 response = client.execute(request);
			if (response.isSuccess()) {
				System.out.println("success");
				System.out.println(JSONObject.toJSONString(response));
			} else {
				System.out.println("error");
				System.out.println(JSONObject.toJSONString(response));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

	}

}
