package com.icbc.api;

import java.util.Date;
import java.util.logging.Logger;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.CertVoiceprintVoiceprintaddRequestV1;
import com.icbc.api.request.CertVoiceprintVoiceprintaddRequestV1.CertVoiceprintVoiceprintaddRequestV1Biz;
import com.icbc.api.response.CertVoiceprintVoiceprintaddResponseV1;

public class CertVoiceprintVoiceprintaddTest {
	//	调用方自己的AppId
	private static final String APP_ID = new String("10000000s00000409a7529");
	//	调用方私钥
	protected static final String MY_PRIVATE_KEY = new String("-----请使用正确的APP私钥-----");
	//	API网关公钥
	protected static final String APIGW_PUBLIC_KEY = new String("-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB");
	//	API请求地址
	private static final String API_SERVICE_URL = new String("http://ip:port/api");
	
	public static void main(String[] args) {
//		签名类型为SHA256WithRSA时，传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,IcbcConstants.SIGN_TYPE_RSA2,MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		CertVoiceprintVoiceprintaddRequestV1 request = new CertVoiceprintVoiceprintaddRequestV1();
		
//		测试环境替换ip和端口，生成环境已有默认值，此语句可删除
		request.setServiceUrl(API_SERVICE_URL + "/cert/voiceprint/voiceprintadd/V1");
		Logger log = Logger.getLogger(CertVoiceprintVoiceprintaddTest.class.getName());
//		请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		CertVoiceprintVoiceprintaddRequestV1Biz bizContent =  new CertVoiceprintVoiceprintaddRequestV1Biz();
		bizContent.setChannel("EPASS1");
		bizContent.setAppName("F-EPASS");
		bizContent.setAppVersion("1.0");
		bizContent.setAppInfo("F-EPASS");
		bizContent.setTrCode("BAS02sdfgds3");
		bizContent.setTrxZone("1001");
		bizContent.setProgramName("addvoice");
		bizContent.setUserId("0009821sdfsd38090");
		bizContent.setCustomerAgreement("1");
		bizContent.setHack1("1");
		bizContent.setTransportType("2");
		bizContent.setVoiceToken1("vx2E5BDPGf/YEgPCEttYaV9xWxRFr7gg+jhKbz755Nhz+8t8TXpxo/3sMBN/N+WBYf79OIyvBs0+y5KcLiQ+xA==");
		bizContent.setVoiceLabel1("19950608");
		bizContent.setVoiceLabel2("10247596");
		bizContent.setVoiceLabel3("89623157");
		bizContent.setVoice1("Qk02tAQAAAAAADYEAAAoAAAAgAIAAOABAAABAAgAAAAAA");
		bizContent.setVoice2("Wk02tAQAAAAAADYEAAAoAAAAgAIAAOABAAABAAgAAAAAA");
		bizContent.setVoice3("Ek02tAQAAAAAADYEAAAoAAAAgAIAAOABAAABAAgAAAAAA");
		request.setBizContent(bizContent);
		CertVoiceprintVoiceprintaddResponseV1 response;
		
		try {
		    response = client.execute(request, "" + new Date().getTime());
		    if (response.isSuccess()) {
		        // 业务成功处理
		    	log.info("ReturnCode:" + response.getReturnCode());
		    	log.info("response:" + response);
		    } else {
		        // 失败
		    	log.info("ReturnCode:" + response.getReturnCode());
		    	log.info("ReturnMsg:" + response.getReturnMsg());
		    }
		    log.info("response.toJSONString: "+JSON.toJSONString(response));
		    log.info("response.result: " + response.getResult());
		    log.info("response.exMsg: " + response.getExMsg());
		    log.info("response.exCode: " + response.getExCode());
		} catch (IcbcApiException e) {
		    e.printStackTrace();
		}
	}
}
