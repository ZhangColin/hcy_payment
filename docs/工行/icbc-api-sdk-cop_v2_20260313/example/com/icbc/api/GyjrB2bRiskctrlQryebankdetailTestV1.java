package com.icbc.api;

import com.icbc.api.request.GyjrB2bRiskctrlQryebankdetailRequestV1;

import java.util.ArrayList;
import java.util.List;

public class GyjrB2bRiskctrlQryebankdetailTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16" +
            "bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCL" +
            "T7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP" +
            "3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000004097878";
    private static final String CA_PRIVATE_STR="-----请使用正确的APP私钥-----";

    public static void main(String[] args) {

        UiIcbcClient client = new UiIcbcClient(APP_ID, CA_PRIVATE_STR, IcbcConstants.CHARSET_UTF8);

        GyjrB2bRiskctrlQryebankdetailRequestV1 request = new GyjrB2bRiskctrlQryebankdetailRequestV1();

        request.setServiceUrl("http://ip:port/ui/gyjr/b2b/riskctrl/qryebankdetail/V1");

        GyjrB2bRiskctrlQryebankdetailRequestV1.GyjrB2bRiskctrlQryebankdetailRequestV1Biz bizContent = new GyjrB2bRiskctrlQryebankdetailRequestV1.GyjrB2bRiskctrlQryebankdetailRequestV1Biz();

        GyjrB2bRiskctrlQryebankdetailRequestV1.Trans_Info transInfo = new GyjrB2bRiskctrlQryebankdetailRequestV1.Trans_Info();

        List<GyjrB2bRiskctrlQryebankdetailRequestV1.Company> companys = new ArrayList();

        // 公司1
        GyjrB2bRiskctrlQryebankdetailRequestV1.Company company1 = new GyjrB2bRiskctrlQryebankdetailRequestV1.Company();
        company1.setCompanyName("XX公司");
        company1.setOrganizationCode("912300008888123456");

        // 公司2
        GyjrB2bRiskctrlQryebankdetailRequestV1.Company company2 = new GyjrB2bRiskctrlQryebankdetailRequestV1.Company();
        company2.setCompanyName("XX公司");
        company2.setOrganizationCode("912300008888234567");

        companys.add(company1);
        companys.add(company2);

        transInfo.setProtocolCode("GYJR20220705100505000012345");
        transInfo.setCompany(companys);

        bizContent.setTransInfo(transInfo);

        request.setBizContent(bizContent);

        try{
            String s = client.buildPostForm(request);
            System.out.println(s);
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }

    }

}
