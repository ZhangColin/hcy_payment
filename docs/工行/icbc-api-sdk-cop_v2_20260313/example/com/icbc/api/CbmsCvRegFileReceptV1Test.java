package com.icbc.api;

import com.icbc.api.request.CbmsCvRegFileReceptRequestV1;
import com.icbc.api.response.CbmsCvRegFileReceptResponseV1;

/**
 * @Author kfzx-wangz04
 * @Date 2021 - 12 - 09
 * @Deseription
 */
public class CbmsCvRegFileReceptV1Test {
    // 1、网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    // 2、appid
    protected static final String APP_ID = "10000000000000012106";

    // 3、密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    public static void main(String[] args) {

        // 签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY,
                APIGW_PUBLIC_KEY);
        CbmsCvRegFileReceptRequestV1 request = new CbmsCvRegFileReceptRequestV1();
        // 4、根据测试环境和生产环境替换对应的IP和端口
        request.setServiceUrl("http://ip:port/api/saes/servmer/ordersubmit/V1");
        // 5、请对照接口文档用bizContent.setxxx（）的方法对业务上送数据赋值
        CbmsCvRegFileReceptRequestV1.CbmsCvRegFileReceptRequestV1Biz bizContext = new CbmsCvRegFileReceptRequestV1.CbmsCvRegFileReceptRequestV1Biz();
        bizContext.setRegId("00000001");
        bizContext.setSaTrxId("00000001");
        bizContext.setRegTrxType((byte)1);
        bizContext.setRegFile("fsdfa");
        request.setBizContent(bizContext);
        CbmsCvRegFileReceptResponseV1 response;
        try {
            // msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
            response = client.execute(request,"23344544454112");
            System.out.println("response: " + response.getReturnCode());
            if (response.isSuccess()) {
                System.out.println("response: " + response.getReturnCode());
                System.out.println("response: " + response);
            } else {
                System.out.println("response: " + response.getReturnCode());
                System.out.println("response: " + response.getReturnMsg());
            }
        } catch (Exception e) {
           e.printStackTrace();
        }
    }
}
