package example.com.icbc.api;

import com.icbc.api.request.CardbusinessNcpayDwncpayRequestV1;
import com.icbc.api.response.CardbusinessNcpayDwncpayResponseV1;

import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;

public class CardbusinessNcpayDwncpayTestV1 {

	public static void main(String[] args) throws IcbcApiException {
		// 1、APPID
		String APP_ID = "xxxxxxxxxxxxxxxxxxx";
		// 2、网关公钥
		String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
		// 3、密钥对认证方式，公钥在API平台登记，此处为私钥
		String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
		// ENCRYPT_TYPE_AES:通过工具生成
		String ENCRYPT_AES_KEY = "xxxxxxxxxx";

		CardbusinessNcpayDwncpayRequestV1 request = new CardbusinessNcpayDwncpayRequestV1();
		// 是否使用AES加密请求数据
		request.setNeedEncrypt(false);

		DefaultIcbcClient client;
		if (request.isNeedEncrypt()) {
			client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8, IcbcConstants.FORMAT_JSON,
					APIGW_PUBLIC_KEY, IcbcConstants.ENCRYPT_TYPE_AES, ENCRYPT_AES_KEY, "", "");
		} else {
			client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		}

		// 4、API网关地址：根据测试环境和生产环境替换相应IP和端口
		request.setServiceUrl("http://xxxxxxx/api/cardbusiness/ncpay/dwncpay/V1");//功能
		//request.setServiceUrl("http://xxxxxxx/api/cardbusiness/ncpay/ncpay/V1");//功能外网

		// 5、请参照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		CardbusinessNcpayDwncpayRequestV1.CardbusinessNcpayDwncpayRequestV1Biz bizContent = new CardbusinessNcpayDwncpayRequestV1.CardbusinessNcpayDwncpayRequestV1Biz();
		String dataStr = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
		String timeStr = LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
		bizContent.setTrx_serno("");
		bizContent.setTrx_date(dataStr);
		bizContent.setTrx_time(timeStr);
		bizContent.setSub_merchant_id("");
		bizContent.setBuss_type("");
		bizContent.setBuss_code("");
		bizContent.setExpire_minutes(0);
		bizContent.setGoods_tag("");
		bizContent.setSign_medtype(0);
		bizContent.setSign_medid("");
		bizContent.setSign_med_info("");
		bizContent.setTrx_curr_type("");
		bizContent.setTrx_amt("");
		bizContent.setTrx_place("");
		bizContent.setTrx_place_name("");
		bizContent.setDevice_top("");
		bizContent.setDevice_sub("");
		bizContent.setOperator_id("");
		bizContent.setCommodity("");
		bizContent.setVerify_code("");
		bizContent.setMerchant_id("");
		bizContent.setBak1("");
		bizContent.setBak2("");
		request.setBizContent(bizContent);
		// msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
		String msgId = String.valueOf(System.currentTimeMillis());
		CardbusinessNcpayDwncpayResponseV1 response = client.execute(request, msgId);

		if (response.isSuccess()) {
			// 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
		} else {
			// 失败
		}
	}
}
