package com.icbc.api;

import com.icbc.api.request.NfpwChainLoanapplyDetailqryRequestV1;

// 网络融资产业链融资申请详情查询服务（页面）
public class MallNfpChainUiNfploanapplydetailqryTestV1 {

    private static final String APP_ID="10000000000002354554";
    private static final String CA_PRIVATE_STR="-----请使用正确的APP私钥-----";

    public static void main(String[] args){
        UiIcbcClient client = new UiIcbcClient(APP_ID, CA_PRIVATE_STR, IcbcConstants.CHARSET_UTF8);
        NfpwChainLoanapplyDetailqryRequestV1 request=new NfpwChainLoanapplyDetailqryRequestV1();
        request.setServiceUrl("http://ip:port/ui/mall/nfp/chain/ui/nfploanapplydetailqry/V1");
        NfpwChainLoanapplyDetailqryRequestV1.NfpwChainLoanapplyDetailqryRequestV1Biz bizContent = new NfpwChainLoanapplyDetailqryRequestV1.NfpwChainLoanapplyDetailqryRequestV1Biz();
        bizContent.setAppid(APP_ID);
        bizContent.setOutVendorId("19000003");
        request.setBizContent(bizContent);
        System.out.println(request);
        try{
            String s = client.buildPostForm(request);
            System.out.println(s);
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }

}
