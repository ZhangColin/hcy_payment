package com.icbc.api;

import static org.junit.Assert.assertEquals;

import java.util.logging.Logger;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.MedialessProtocolFaceQueryRequestV1;
import com.icbc.api.request.MedialessProtocolFaceQueryRequestV1.MedialessProtocolFaceQueryRequestV1Biz;
import com.icbc.api.response.MedialessProtocolFaceQueryResponseV1;

public class MedialessProtocolFaceQueryTestV1 {
	
	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	// 2、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	// 3、appid
	protected static final String APP_ID = "xxx";
	
	private Logger logger = Logger.getLogger(MedialessProtocolFaceQueryTestV1.class.getName());
	

	public void test_cafm() throws IcbcApiException {
		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		MedialessProtocolFaceQueryRequestV1 request = new MedialessProtocolFaceQueryRequestV1();
		request.setServiceUrl("http://ip:port/api/medialess/protocol/face/query/V1");
		MedialessProtocolFaceQueryRequestV1Biz bizContent = new MedialessProtocolFaceQueryRequestV1Biz();
		bizContent.setProtocolType("");
		bizContent.setInqType("");
		bizContent.setMobileNo("");
		bizContent.setCisNo("");
		// ......
		request.setBizContent(bizContent);
		
		MedialessProtocolFaceQueryResponseV1 response = null;
		response = client.execute(request, "msgId");
		boolean testFlag = true;
		if (response.isSuccess()) {
			// 成功
			logger.info("success response:" + JSON.toJSONString(response));
			logger.info("return_result:" + response.getReturnResult());
		} else {
			// 失败
			testFlag = false;
			logger.info("error response:" + JSON.toJSONString(response));
		}
		assertEquals(true, testFlag);
		
	}

}
