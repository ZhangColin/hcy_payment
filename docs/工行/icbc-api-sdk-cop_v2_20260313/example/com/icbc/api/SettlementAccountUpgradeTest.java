package com.icbc.api;

import java.util.UUID;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountUpgradeRequestV1;
import com.icbc.api.request.SettlementAccountUpgradeRequestV1.SettlementAccountUpgradeRequestV1Biz;
import com.icbc.api.response.SettlementAccountUpgradeResponseV1;

public class SettlementAccountUpgradeTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountUpgradeRequestV1 request = new SettlementAccountUpgradeRequestV1();
		SettlementAccountUpgradeRequestV1Biz bizContent = new SettlementAccountUpgradeRequestV1Biz();
		request.setServiceUrl("http://ip:port/api/settlement/account/upgrade/V1");
		bizContent.setCorpSerno(UUID.randomUUID().toString());//合作方交易单号		
		bizContent.setCorpMediumId("987654321"); //合作方介质号 				
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setCorpDate("2018-10-24");//合作方工作日期
		bizContent.setTrxDate("2018-10-24");//合作方交易日期
		bizContent.setTrxTime("10:38:01");//合作方交易受理时间
		bizContent.setMediumId("6222888888888888888"); //卡号(支持加密，Base64编码)
		bizContent.setCustName("张小二");
		bizContent.setCertType(0);
		bizContent.setCertNo("certNo"); //		
		bizContent.setOutServiceCode("accountleveladjust"); //选输 外部服务代码
		bizContent.setMediumIdHash("SDFDFHTEWTGDFWADADAFSDGSESEFD"); //选输 卡号哈希值
		bizContent.setSecretKey("ASDQWEQDZCSDFAWWQDA"); //选输 SM2公钥加密的sm4对称密钥
		bizContent.setImgSerialNo("");//选输 影像批次号
		
		request.setBizContent(bizContent);
		SettlementAccountUpgradeResponseV1  response = client.execute(request, "msgId");
		
		if (response.isSuccess() ) {
			// 业务成功处理
			response.getEventNo(); //事件编号
			response.getIcbcWorkDate(); //我行工作日期
		} else {
			// 失败
		}
	}
}
