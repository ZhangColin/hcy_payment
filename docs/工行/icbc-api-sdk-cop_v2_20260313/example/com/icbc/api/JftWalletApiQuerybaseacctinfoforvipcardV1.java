package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1;
import com.icbc.api.request.JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1.JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz;
import com.icbc.api.response.JftWalletApiVIPCardquerybaseacctinfoforvipcardResponseV1;
import com.icbc.api.response.JftWalletApiVIPCardquerytranstatusforvipcardResponseV1;

public class JftWalletApiQuerybaseacctinfoforvipcardV1 {

	String MY_PRIVATE_KEY2 = "-----请使用正确的APP私钥-----";
	String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJ h0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000012145";

	// 聚富通钱包会员卡交易明细查询

	public void test_query_deatil() {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA", MY_PRIVATE_KEY2, "UTF-8", "json",
				APIGW_PUBLIC_KEY, "AES", "5xGJdh7qb+B95SUoxDlatg==", "", "");
		// DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA",
		// MY_PRIVATE_KEY2, "UTF-8", "json",APIGW_PUBLIC_KEY, "AES",
		// "zl+5SxQAx6P7bjpqRu693Q==", "", "");
		JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1 request = new JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1();

		request.setServiceUrl("https://ip:port/api/jft/wallet/api/querybaseacctinfoforvipcard/V1");
		JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz = new JftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz();

		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setAppId(APP_ID);
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setBaseAcctNo("1000008880001259417");// 虚户账号1000008880001259417
																										// 6556668880000000026
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setEndDate("20181201");
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setStartDate("20180805");
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setPageCount("10");// 每页显示条数
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setPageNo("1");// 页码

		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setCorpDate("2018-12-01");
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setTrxAccDate("2018-12-04");
		jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz.setTrxAccTime("222");

		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

		System.out.println(jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz);
		request.setBizContent(jftWalletApiVIPCardquerybaseacctinfoforvipcardRequestV1Biz);

		JftWalletApiVIPCardquerybaseacctinfoforvipcardResponseV1 response;

		try { 
			response = (JftWalletApiVIPCardquerybaseacctinfoforvipcardResponseV1) client.execute(request, System.currentTimeMillis() + "");
			System.out.println("~~~~~~~~~");
			System.out.println(JSONObject.toJSONString(response));
			System.out.println("~~~~~~~~~");
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}

		} catch (IcbcApiException e) {

			e.printStackTrace();

		}
	}

}
