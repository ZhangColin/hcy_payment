package com.icbc.api;

import java.util.UUID;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountOpenOrderRequestV1;
import com.icbc.api.request.SettlementAccountOpenOrderRequestV1.SettlementAccountOpenOrderRequestV1Biz;
import com.icbc.api.request.SettlementAccountOpenRequestV1;
import com.icbc.api.request.SettlementAccountOpenRequestV1.SettlementAccountOpenRequestV1Biz;
import com.icbc.api.response.SettlementAccountOpenOrderResponseV1;
import com.icbc.api.response.SettlementAccountOpenResponseV1;

public class SettlementAccountOpenOrderTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountOpenOrderRequestV1 request = new SettlementAccountOpenOrderRequestV1();
		SettlementAccountOpenOrderRequestV1Biz bizContent = new SettlementAccountOpenOrderRequestV1Biz();
		request.setServiceUrl("http://ip:port/api/settlement/account/open/order/V1");
		bizContent.setAddress("");
		bizContent.setBindMedium("6222030200000600407");
		bizContent.setBirthday("");
		bizContent.setCertNo("999999199009099991");
		bizContent.setCertType(0);
		bizContent.setCorpCisNo("1");
		bizContent.setCorpDate("2017-03-15");
		bizContent.setCorpMediumId("987654321");
		bizContent.setCorpNo("corpInst1234");
		bizContent.setCorpSerno(UUID.randomUUID().toString());
		bizContent.setCustName("九九九九九九九九九九九九九九九");
		bizContent.setCustomizationInfo("");
		bizContent.setGender(1);
		bizContent.setIssuingAuthority("");
		bizContent.setMobileNo("13611223388");
		bizContent.setNationality(0);
		bizContent.setNotifyAddr("aaaaabbbbb");
		bizContent.setOccupation(0);
		bizContent.setOutServiceCode("openaccount");
		bizContent.setTaxDeclarationFlag(0);
		bizContent.setTrxAccDate("2017-03-15");
		bizContent.setTrxAccTime("13:30:01");
		bizContent.setValidityPeriod("");
		bizContent.setSecretKey("ASDQWEQDZCSDFAWWQDA");
		bizContent.setBindMediumHash("SDFDFHTEWTGDFWADADAFSDGSESEFD");
		request.setBizContent(bizContent);
		SettlementAccountOpenOrderResponseV1  response = client.execute(request, "msgId");
		
		if (response.isSuccess() ) {
			// 业务成功处理
			//response.getAccountBalance();//
		} else {
			// 失败
		}
	}
}
