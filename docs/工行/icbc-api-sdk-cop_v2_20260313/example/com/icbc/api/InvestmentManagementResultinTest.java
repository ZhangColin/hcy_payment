package com.icbc.api;

import com.icbc.api.request.InvestmentManagementResultinRequestV2;
import com.icbc.api.request.InvestmentManagementResultinRequestV2.InvestmentManagementResultinRequestV2Biz;
import com.icbc.api.request.InvestmentManagementResultinRequestV2.InvestmentManagementResultinRequestV2BizBankbond;
import com.icbc.api.response.InvestmentManagementResultinResponseV2;

/**
 * 成交信息数据接收接口
 */
public class InvestmentManagementResultinTest {

	public static void main(String[] args) throws Exception {

		// 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值

		//1、appid
		String APP_ID = "11000000000000004856";
		//2、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
		String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
		//3、API网关公钥
		String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

		//4、创建与F-APIP外网网关的连接
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
		//5、设置请求对象
		InvestmentManagementResultinRequestV2 request = new InvestmentManagementResultinRequestV2();
		//6、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
		request.setServiceUrl("http://apip-gateway-gw-out-gn-F.month.sdc.cs.icbc/api/investment/management/resultin/V2");
		//7、设置业务参数，按需求自行组装所需业务参数
		InvestmentManagementResultinRequestV2.InvestmentManagementResultinRequestV2Biz bizContent = new InvestmentManagementResultinRequestV2.InvestmentManagementResultinRequestV2Biz();
		InvestmentManagementResultinRequestV2.InvestmentManagementResultinRequestV2BizBankbond bankbondDealconfirm = new InvestmentManagementResultinRequestV2.InvestmentManagementResultinRequestV2BizBankbond();
		//8、组装请求参数
		bankbondDealconfirm.setGuidNo("A20240910");
		bankbondDealconfirm.setFundCode("GS2352");
		bizContent.setService("BondInterbankOrdinaryTransInfoService");
		bizContent.setMethod("receive");
        bizContent.setBankbondDealconfirm(bankbondDealconfirm);
		request.setBizContent(bizContent);

		InvestmentManagementResultinResponseV2 response;
		try {
			System.out.println("request:"+ JSONObject.toJSONString(request));
			response = client.execute(request);
			System.out.println("response:" + JSONObject.toJSONString(response));
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}

	
}


