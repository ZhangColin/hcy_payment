package com.icbc.api;


import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.GyjrB2bBillQueryDischargeBillRequestV1;
import com.icbc.api.response.GyjrB2bBillQueryDischargeBillResponseV1;

import java.util.Random;

public class GyjrB2bBillQueryDischargeBillTestV1 {


    // APPID(变量值需替换)
    protected static final String APP_ID = "10000000000004097876";
    // 应用私钥(变量值需替换)
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    // API网关公钥(变量值需替换)
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";


    public static void main(String[] args) {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        GyjrB2bBillQueryDischargeBillRequestV1 request = new GyjrB2bBillQueryDischargeBillRequestV1();
        request.setServiceUrl("http://IP:PORT/api/gyjr/b2b/bill/querydischargebill/V1");
        GyjrB2bBillQueryDischargeBillRequestV1.BizContentV1Biz bizContent = new GyjrB2bBillQueryDischargeBillRequestV1.BizContentV1Biz();
        GyjrB2bBillQueryDischargeBillRequestV1.TransInfoV1Biz transInfo
                = new GyjrB2bBillQueryDischargeBillRequestV1.TransInfoV1Biz();

        transInfo.setProtocolCode("GYJR20220119160206728375706");
        transInfo.setPlatVendorId("14");
        transInfo.setTransactionRqSeq("9963552352205120520212511");
        transInfo.setCdTp("AC01");
        transInfo.setOrderRule("1");
        transInfo.setTransactionRqSeq("9963552352205120520212511");
        transInfo.setPageIndex("1");
        transInfo.setPageSize("1");
        transInfo.setCdTp("AC01");
        transInfo.setRrcrsAcctId("111111111");
        transInfo.setOrderRule("0");
        transInfo.setPackNoList("");
        transInfo.setUkeyId("dil.oo1.ic");
        bizContent.setTransInfoBiz(transInfo);
        request.setBizContent(bizContent);

        GyjrB2bBillQueryDischargeBillResponseV1 response;
        try {
            response = client.execute(request, new Random().nextInt(99999999) + "msg");
            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
                System.out.println(response.getReturnCode());
                System.out.println(response.getReturnMsg());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
