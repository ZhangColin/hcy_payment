package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.SaleDividendParamMerchantUpdateRequestBizV1;
import com.icbc.api.request.SaleDividendParamMerchantUpdateRequestV1;
import com.icbc.api.response.SaleDividendParamMerchantUpdateResponseV1;

public class SaleDividendParamMerchantUpdateTest {
    //1、网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //2、appid
    protected static final String APP_ID = "10000000000004097413";

    //3、密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    public static void main(String[] args) throws Exception {

        //签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        SaleDividendParamMerchantUpdateRequestV1 request = new SaleDividendParamMerchantUpdateRequestV1();

        //4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://ip:port/api/sale/dividend/param/merchant/update/V1");
        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        SaleDividendParamMerchantUpdateRequestBizV1 bizContent = new SaleDividendParamMerchantUpdateRequestBizV1();
        bizContent.setAppId(APP_ID);
        request.setBizContent(bizContent);

        bizContent.setCorpId("2000000050");
        bizContent.setMerno("20000000500000000047");
        bizContent.setMername("Apple Store");
        bizContent.setSummary("华为商店");
        bizContent.setRebatesrate(55);
        bizContent.setContactPhone("020-82212345");
        bizContent.setContactName("任正非");
        bizContent.setContactMobile("13188888888");
        bizContent.setOperation(1);

        SaleDividendParamMerchantUpdateResponseV1 response;
        String msgId = String.valueOf(System.currentTimeMillis());
        try {
            response = client.execute(request, msgId);//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
            if(response.getReturnCode()==0){
                // 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
                System.out.println("ReturnCode:"+response.getReturnCode());
                System.out.println("response:" + JSON.toJSONString(response));
            } else {
                // 失败
                System.out.println("response:" + JSON.toJSONString(response));
                System.out.println("ReturnCode:"+response.getReturnCode());
                System.out.println("ReturnMsg:"+response.getReturnMsg());
            }

        }catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
