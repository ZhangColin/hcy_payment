package com.icbc.api.test;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.JftApiVendorMemberPlatRegistRequestV1;
import com.icbc.api.request.JftApiVendorMemberPlatRegistRequestV1.JftApiVendorMemberPlatRegistRequestV1Biz;
import com.icbc.api.response.JftApiVendorMemberPlatRegistResponseV1;

import java.util.ArrayList;
import java.util.List;

/**
 * 聚富通平台会员注册接口
 */
public class JftApiVendorMemberPlatRegistTestV1 {

	//1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//2、appid
	protected static final String APP_ID = "10000000000000113005";
	//3、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	public static void main(String[] args) throws Exception {
		
		//签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", 
				"json",APIGW_PUBLIC_KEY,"AES","yuzLR7IMjAOZzlNZ83iXIw==", "", "");
		
		JftApiVendorMemberPlatRegistRequestV1 request = new JftApiVendorMemberPlatRegistRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/jft/api/vendor/member/plat/regist/V1");

		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		JftApiVendorMemberPlatRegistRequestV1Biz bizContent = new JftApiVendorMemberPlatRegistRequestV1Biz();
		
		List<String> ukeyId = new ArrayList<>();
		ukeyId.add("1234567890");
		ukeyId.add("2345678901");
		ukeyId.add("3456789012");
		
		bizContent.setOutVendorId("RegistMemberTest");
		bizContent.setOutUserId("RegistMemberTest");
		bizContent.setVendorName("RegistMemberTest");
		bizContent.setCorporateName("RegistMemberTest");
		bizContent.setCorporateIdNo("RegistMemberTest");
		bizContent.setOperatorName("RegistMemberTest");
		bizContent.setAccountNo("RegistMemberTest");
		bizContent.setCertNo("RegistMemberTest");
		bizContent.setAccountName("RegistMemberTest");
		bizContent.setCertType("RegistMemberTest");
		bizContent.setAppId("RegistMemberTest");
		bizContent.setVendorEmail("RegistMemberTest");
		bizContent.setAccountMobile("RegistMemberTest");
		bizContent.setAccountBankName("RegistMemberTest");
		bizContent.setAccountBankCode("RegistMemberTest");
		bizContent.setCorporateIdType("RegistMemberTest");
		bizContent.setOperatorMobile("RegistMemberTest");
		bizContent.setBusFlag("01");
		bizContent.setBusSwitch("0");
		bizContent.setUkeyId(ukeyId);
		
		
		
		request.setBizContent(bizContent);

		JftApiVendorMemberPlatRegistResponseV1 response;
		try {
			response =  (JftApiVendorMemberPlatRegistResponseV1) client.execute(request, System.currentTimeMillis()+"");
			if (response.isSuccess()) {
				//6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("response:" + response);
			} else {
				//失败
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("ReturnMsg:"+response.getReturnMsg());
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}


	}
	
}

