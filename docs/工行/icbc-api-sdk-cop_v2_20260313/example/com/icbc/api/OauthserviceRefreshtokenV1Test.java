package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.OauthserviceRefreshtokenRequestV1;
import com.icbc.api.request.OauthserviceRefreshtokenRequestV1.OauthserviceRefreshtokenRequestV1Biz;
import com.icbc.api.response.OauthserviceRefreshtokenResponseV1;


public class OauthserviceRefreshtokenV1Test {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000003909";
	
	protected static final String APP_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	

	public void test() throws IcbcApiException {
		//RSA调用
		//DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,APP_PRIVATE_KEY,APIGW_PUBLIC_KEY);
	
		//RSA2调用
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,IcbcConstants.SIGN_TYPE_RSA2,APP_PRIVATE_KEY,APIGW_PUBLIC_KEY);
		
		OauthserviceRefreshtokenRequestV1 request = new OauthserviceRefreshtokenRequestV1();
		request.setServiceUrl("http://ip:port/api/oauthservice/refreshtoken/V1");
		
		OauthserviceRefreshtokenRequestV1Biz bizContent = new OauthserviceRefreshtokenRequestV1Biz();
		bizContent.setRefreshToken("");
		bizContent.setGrantType("refresh_token");
		bizContent.setTimeStamp("2017-12-13 16:03:00");
		
		request.setBizContent(bizContent);
		
		OauthserviceRefreshtokenResponseV1 response = client.execute(request);
		
		if (response.isSuccess()) {
			// 接收成功
			System.out.println("return_code="+response.getReturnCode());
			System.out.println("return_msg="+response.getReturnMsg());
			System.out.println("msg_id="+response.getMsgId());
			System.out.println("access_token="+response.getAccessToken());
			System.out.println("expires_in="+response.getExpiresIn());
			System.out.println("refresh_token="+response.getRefreshToken());
			System.out.println("timestamp="+response.getTimeStamp());
		}else {
			// 失败
			System.out.println("return_code="+response.getReturnCode());
			System.out.println("return_msg="+response.getReturnMsg());
		}
	}
}
