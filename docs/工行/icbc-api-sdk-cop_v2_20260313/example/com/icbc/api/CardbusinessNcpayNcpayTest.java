package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.CardbusinessNcpayNcpayRequestV1;
import com.icbc.api.request.CardbusinessNcpayNcpayRequestV1.CardbusinessNcpayNcpayRequestV1Biz;
import com.icbc.api.response.CardbusinessNcpayNcpayResponseV1;

import java.time.LocalDateTime;
import java.time.format.DateTimeFormatter;

/**
 * 支付接口测试
 */
public class CardbusinessNcpayNcpayTest {

	public static void main(String[] args) throws IcbcApiException {
		// 1、APPID
		String APP_ID = "XXX";
		// 2、网关公钥
		String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
		// 3、密钥对认证方式，公钥在API平台登记，此处为私钥
		String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
		// ENCRYPT_TYPE_AES:通过工具生成
		String ENCRYPT_AES_KEY = "ULgn+LpGG+JMNy+aFbGPZg==";

		CardbusinessNcpayNcpayRequestV1 request = new CardbusinessNcpayNcpayRequestV1();
		// 是否使用AES加密请求数据
		request.setNeedEncrypt(false);

		DefaultIcbcClient client;
		if (request.isNeedEncrypt()) {
			client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8, IcbcConstants.FORMAT_JSON,
					APIGW_PUBLIC_KEY, IcbcConstants.ENCRYPT_TYPE_AES, ENCRYPT_AES_KEY, "", "");
		} else {
			client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		}

		// 4、API网关地址：根据测试环境和生产环境替换相应IP和端口
//		request.setServiceUrl("http://ip:port/api/cardbusiness/ncpay/ncpay/V1");//功能
		request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cardbusiness/ncpay/ncpay/V1");//功能外网

		// 5、请参照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		CardbusinessNcpayNcpayRequestV1Biz bizContent = new CardbusinessNcpayNcpayRequestV1Biz();
		request.setBizContent(bizContent);

		String dataStr = LocalDateTime.now().format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
		String timeStr = LocalDateTime.now().format(DateTimeFormatter.ofPattern("HH:mm:ss"));

		bizContent.setTrxSerno("XXX");
		bizContent.setTrxDate(dataStr);
		bizContent.setTrxTime(timeStr);
		bizContent.setSignMedtype(3);
		bizContent.setSignMedid("粤A1235J");
		bizContent.setSignMedInfo("0");
		bizContent.setMerGroupNo("4600000000001");
		bizContent.setMerchantNo("020001020483");
		bizContent.setSceneAgreement("");
		bizContent.setTrxCurrType("001");
		bizContent.setTrxAmt("10");
		bizContent.setExtendInfo("test-pay");
		bizContent.setCardno("");
		bizContent.setNewMernoFlag("0");
		bizContent.setMerPrtclNo("XXX");
		bizContent.setDiscountFlag("0");
		bizContent.setTrxPlace("6666");
		bizContent.setTrxPlaceName("test");
		bizContent.setDeviceTop("12");
		bizContent.setDeviceSub("34");
		bizContent.setOperatorId("006");
		bizContent.setOrderId("11223114412121");
		bizContent.setVerifyCode("112233");

		// msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
		String msgId = String.valueOf(System.currentTimeMillis());
		CardbusinessNcpayNcpayResponseV1 response = client.execute(request, msgId);

		if (response.isSuccess()) {
			// 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
//			System.out.println(">> Request succeed.");
//			System.out.println(JSON.toJSONString(response, true));
		} else {
			// 失败
//			System.out.println(">> Request failed.");
//			System.out.println(JSON.toJSONString(response, true));
		}
	}
}
