...
import com.icbc.api.request.JftApiVendorAddRevokeCancellationRequestV1;
import com.icbc.api.request.JftApiVendorAddRevokeCancellationRequestV1.JftApiVendorAddRevokeCancellationRequestV1Biz;
import com.icbc.api.response.JftApiVendorAddRevokeCancellationResponseV1;
 ...
 * @Description:
 */
public class JftApiVendorAddRevokeCancellationTestV1 {
    //1、网关公钥
...
//		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","nezfRV32iH9deJq4IrcCpw==", "", "");
        ...
        JftApiVendorAddRevokeCancellationRequestV1 request = new JftApiVendorAddRevokeCancellationRequestV1();

        //4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("https://ip:port/api/jft/api/vendor/add/revoke/cancellation/V1");

        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值

        JftApiVendorAddRevokeCancellationRequestV1Biz bizContent = new JftApiVendorAddRevokeCancellationRequestV1Biz();

        bizContent.setAppId(APP_ID);//appId
        bizContent.setOutVendorId("100000001001");//outVendorId

        request.setBizContent(bizContent);
        System.out.println("request:" + request.toString());

        JftApiVendorAddRevokeCancellationResponseV1 response;

        try {
            response = (JftApiVendorAddRevokeCancellationResponseV1)client.execute(request, System.currentTimeMillis()+"");
