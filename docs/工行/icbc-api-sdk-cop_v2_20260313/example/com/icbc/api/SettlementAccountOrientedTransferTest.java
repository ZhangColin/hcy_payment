package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountOrientedTransferRequestV1;
import com.icbc.api.request.SettlementAccountOrientedTransferRequestV1.SettlementAccountOrientedTransferRequestV1Biz;
import com.icbc.api.response.SettlementAccountOrientedTransferResponseV1;

public class SettlementAccountOrientedTransferTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	protected static final String APP_ID = "10000000000000021587";

	
	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountOrientedTransferRequestV1 request = new SettlementAccountOrientedTransferRequestV1();
		SettlementAccountOrientedTransferRequestV1Biz bizContent = new SettlementAccountOrientedTransferRequestV1Biz();

		request.setServiceUrl("https://ip:port/api/settlement/account/orientedtransfer/V1");
		
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setTrxAccDate("2017-03-14"); //合作方交易日期
		bizContent.setTrxAccTime("00:00:00"); //合作方交易时间
		bizContent.setCorpDate("2017-03-14"); //合作方工作日期
		bizContent.setCorpSerno("20202020"); //合作方交易单号
		bizContent.setOutServiceCode("orientedtransfer"); //外部服务代码
		bizContent.setMediumId("6227003028940029516"); //他行卡号
		bizContent.setCcy(1); //币种
		bizContent.setCashExFlag(0); //钞汇标志
		bizContent.setAmount("300"); //交易金额
		bizContent.setOaccno("0200026009456784219");//对手账户
		bizContent.setDrcrdirection(1);//借贷方向
		bizContent.setSummary("DY"); //摘要
		bizContent.setRemarks("DY"); //注释
		bizContent.setCertNo("999999199009099991"); 
		bizContent.setCertType(0);
        bizContent.setCustName("九九九九九九九九九九九九九九九");

		request.setBizContent(bizContent);

		SettlementAccountOrientedTransferResponseV1 response = client.execute(request, "msgId");
		if (response.isSuccess() ) {
			//成功
			//response.getAccountBalance();
		} else {
			//失败
			System.err.println(response.getReturnCode());//
			System.err.println(response.getReturnMsg());//			
		}
	}

}
