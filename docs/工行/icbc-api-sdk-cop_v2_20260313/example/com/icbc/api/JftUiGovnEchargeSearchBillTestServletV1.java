package me.demo;

import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.UiIcbcClient;
import com.icbc.api.request.JftUiGovnEchargeSearchBillRequestV1;
import com.icbc.api.request.JftUiGovnEchargeSearchBillRequestV1.JftUiGovnEchargeSearchBillRequestV1Biz;


public class JftUiGovnEchargeSearchBillTestServletV1 extends HttpServlet {

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        doPost(req, resp);
    }

    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        try {
            // FIXME: 此处替换合作方 APPID
            String APP_ID = "10000000000000045510";
            // FIXME：此处替换合作方 APPID 对应私钥
            String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
            UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);

            // 组装调用《E缴费账单查询接口V1》上送报文
            JftUiGovnEchargeSearchBillRequestV1 request = new JftUiGovnEchargeSearchBillRequestV1();
            // 指定《E缴费账单查询接口V1》URL
            request.setServiceUrl("https://ip:port/ui/jft/ui/govn/echarge/search/bill/V1");
            JftUiGovnEchargeSearchBillRequestV1Biz bizContent = new JftUiGovnEchargeSearchBillRequestV1Biz();

            bizContent.setAppId(APP_ID);                // 平台商户标识：提交支付请求的平台商户编号
            bizContent.setOutUserId("M201801112326");   // 外系统用户id
            bizContent.setTrxChannel("03");             // 交易渠道：03=H5

            request.setBizContent(bizContent);

            // 调用工行开放平台SDK，生成自动提交表单，将用户跳转到缴费页面
            resp.setHeader("Content-Type", "text/html;charset="
                    + IcbcConstants.CHARSET_UTF8);
            PrintWriter out = resp.getWriter();

            out.write("<html>");
            out.write("<head>");
            out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset="
                    + IcbcConstants.CHARSET_UTF8 + "\">");
            out.write("</head>");
            out.write("<body>");
            out.write(client.buildPostForm(request));
            out.write("</body>");
            out.write("</html>");

        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}


