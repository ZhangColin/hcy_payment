package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BcssMemberCardApplyRequestV3;
import com.icbc.api.response.BcssMemberCardApplyResponseV3;

import java.text.SimpleDateFormat;
import java.util.Date;


public class BcssMemberCardApplyTestV3 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    // APIGW_PUBLIC_KEY
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----\n";

    protected static final String APP_ID = "10000000000004097413";


    /**
     * APIGW（行外网关） 2020年11月 api地址
     */
    protected static final String API_GW_OUT = "http://ip:port/api";


    public static void main(String[] args) {
        try {
            testApply();
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }

    public static void testApply() throws IcbcApiException {
        BcssMemberCardApplyRequestV3.BcssMemberCardApplyRequestBizV1 bizV3 = new BcssMemberCardApplyRequestV3.BcssMemberCardApplyRequestBizV1();
        bizV3.setCorpId("2000000067");
        SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMddHHmmssSSS");
        String format = dateFormat.format(new Date());
        bizV3.setApply_no(format);
        bizV3.setTrx_serno(format);
        bizV3.setProdNo("");
        bizV3.setTel("13444542322");

        BcssMemberCardApplyRequestV3 requestV3 = new BcssMemberCardApplyRequestV3();
        requestV3.setServiceUrl(API_GW_OUT + "/bcss/member/card/apply/V3");
        requestV3.setBizContent(bizV3);
        System.out.println(JSONObject.toJSONString(bizV3));
        DefaultIcbcClient client = getIcbcClient();
        BcssMemberCardApplyResponseV3 response = client.execute(requestV3);
        System.out.println(JSONObject.toJSONString(response));
    }

    private static DefaultIcbcClient getIcbcClient() {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        return client;
    }

    private static UiIcbcClient getUiClinet() throws IcbcApiException {
        UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
        return client;
    }

}
