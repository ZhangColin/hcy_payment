...
import com.icbc.api.request.FinanceTppfinanceSendmessageRequestV1;
import com.icbc.api.response.FinanceTppfinanceSendmessageResponseV1;

public class FinanceTppfinanceSendmessageV1Test {
	...
	public void test() throws IcbcApiException {
		...
		FinanceTppfinanceSendmessageRequestV1 request = new FinanceTppfinanceSendmessageRequestV1();
		request.setServiceUrl("https://ip:port/api/finance/tppfinance/sendmessage/V1");

		// 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值，非必输字段可不上送
		FinanceTppfinanceSendmessageRequestV1.FinanceTppfinanceSendmessageRequestV1Biz bizContent = new FinanceTppfinanceSendmessageRequestV1.FinanceTppfinanceSendmessageRequestV1Biz();
		bizContent.setClient_id("ICBCTEST80001"); // 客户ID
		bizContent.setPhoneno("18888888888"); // 手机号
		request.setBizContent(bizContent);

		FinanceTppfinanceSendmessageResponseV1 response = new FinanceTppfinanceSendmessageResponseV1();
		...
		if (response.isSuccess()) {
			...
			System.out.println("sms_id                  :       " + response.getSms_id()); // 认证短信唯一编号，短信认证时需要和短信编号、短信验证码一同上送
			System.out.println("sms_no                  :       " + response.getSms_no()); // 短信编号
