package com.icbc.api;

import java.io.IOException;
import java.io.PrintWriter;

import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import com.icbc.api.request.JftUiVendorRegisterRequestV1;
import com.icbc.api.request.JftUiVendorRegisterRequestV1.JftUiVendorRegisterRequestV1Biz;

/**
 * 聚富通子商户注册页
 */
public class JftUiVendorRegisterTestV1 extends HttpServlet {

	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		doPost(req, resp);
	}

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {

		try {
			String APP_ID = "10000000000000004473";
			String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

			String AES_Key = "[需替换]应用方加密串，与工行API开放平台一致";
			UiIcbcClient client = new UiIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY,
					IcbcConstants.CHARSET_UTF8, IcbcConstants.ENCRYPT_TYPE_AES, AES_Key);

			JftUiVendorRegisterRequestV1 request = new JftUiVendorRegisterRequestV1();
			request.setServiceUrl("https://ip:port/ui/jft/ui/vendor/register/V1"); 
			
			JftUiVendorRegisterRequestV1Biz bizContent = new JftUiVendorRegisterRequestV1Biz();
			
			bizContent.setAppId(APP_ID);
			bizContent.setAppId("10000000000000004473");
			bizContent.setOutUserId("66666666660055005501");
			bizContent.setOutVendorId("66666666622001234001");
			bizContent.setBizType("26");
			request.setBizContent(bizContent);

			resp.setHeader("Content-Type", "text/html;charset="
					+ IcbcConstants.CHARSET_UTF8);
			PrintWriter out = resp.getWriter();
			System.out.println(client.buildPostForm(request));

			out.write("<html>");
			out.write("<head>");
			out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset="
					+ IcbcConstants.CHARSET_UTF8 + "\">");
			out.write("</head>");
			out.write("<body>");
			out.write(client.buildPostForm(request));
			out.write("</body>");
			out.write("</html>");

		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
}

