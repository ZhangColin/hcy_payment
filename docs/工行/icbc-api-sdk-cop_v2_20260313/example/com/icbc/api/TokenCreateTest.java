...
import com.icbc.api.request.TokenRequestV1.TokenRequestV1Biz;
...
public class TokenCreateTest {
	...
	public void test_cop() {
		...
		request.setServiceUrl("http://ip:port/api/jft/api/vacct/token/get/V1");
		TokenRequestV1Biz bizContent = new TokenRequestV1Biz();
		bizContent.setAppId("123455");//appId外部商户编号
		bizContent.setVersion("1.0.0");//版本号
		bizContent.setValidTime("1800");//有效时间。不超过30分钟（秒）
		request.setBizContent(bizContent);
		TokenResponseV1 response;
		try {
			response = (TokenResponseV1) client.execute(request, System.currentTimeMillis()+"");
