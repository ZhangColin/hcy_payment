package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.CertCardauthBranchIcbccardauthRequestV1;
import com.icbc.api.request.CertCardauthBranchIcbccardauthRequestV1.CertCardauthBranchIcbccardauthRequestV1Biz;
import com.icbc.api.response.CertCardauthBranchIcbccardauthResponseV1;

import java.util.logging.Logger;


public class CertCardauthBranchIcbccardauthTestV1 {
	public static void main(String[] args) {
		Logger log = Logger.getLogger(CertCardauthBranchIcbccardauthTestV1.class.getName());
		// 1、APPID
		String APP_ID = "XXX"; //功能环境
		// 2、网关公钥
		String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
		// 3、私钥
		String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		CertCardauthBranchIcbccardauthRequestV1 request = new CertCardauthBranchIcbccardauthRequestV1();
		CertCardauthBranchIcbccardauthRequestV1Biz bizContent = new CertCardauthBranchIcbccardauthRequestV1Biz();

		bizContent.setCertType("0"); // 证件类型
		bizContent.setCertNo("XXX"); // 证件号码
		bizContent.setCustName("张圣"); // 户名
		bizContent.setCardno("XXX"); // 介质编号
		bizContent.setMobile("18820423459"); // 手机号码
		bizContent.setSendMsgFlag("1");

//		request.setServiceUrl("http://ip:port/api/cert/cardauth/icbccardauth/V1"); //功能环境-外网
		request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cert/cardauth/icbccardauth/V1"); //功能环境-外网域名
		request.setBizContent(bizContent);
		request.isNeedEncrypt();

		String msg_id = String.valueOf(System.currentTimeMillis());
		try {
			CertCardauthBranchIcbccardauthResponseV1 response = client.execute(request, msg_id);// 第二个参数需要用随机数，APi网关会做防重复检查
			if (response.isSuccess()) {
				// 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				log.info(">> Request succeed.");
				log.info(JSON.toJSONString(response, true));
			} else {
				// 失败
				log.info(">> Request failed.");
				log.info(JSON.toJSONString(response, true));
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
