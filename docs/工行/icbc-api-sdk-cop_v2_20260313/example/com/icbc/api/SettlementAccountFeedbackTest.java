package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountFeedbackRequestV1;
import com.icbc.api.request.SettlementAccountFeedbackRequestV1.SettlementAccountFeedbackRequestV1Biz;
import com.icbc.api.response.SettlementAccountFeedbackResponseV1;



public class SettlementAccountFeedbackTest{
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	//protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	//protected static final String APP_ID = "10000000000000001531";
	
	
	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		
		SettlementAccountFeedbackRequestV1 request = new SettlementAccountFeedbackRequestV1();
		SettlementAccountFeedbackRequestV1Biz bizContent = new SettlementAccountFeedbackRequestV1Biz();
		
		request.setServiceUrl("http://ip:port/api/settlement/account/V1/feedback");
		
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setTrxAccDate("2017-03-23");
		bizContent.setTrxAccTime("12:30:21");
		bizContent.setCorpSerno("123456789"); //合作方交易单号
		bizContent.setCorpDate("2017-03-23"); //合作方工作日期
		bizContent.setOutServiceCode("feedback"); //外部服务代码
		bizContent.setMediumId("1myGlhZWe8ZVG7DKe/7pAr9ew9Yd08XzaX+L2juTb7Q="); //工行联名卡号
		bizContent.setCcy(1); //币种
		bizContent.setCashExFlag(0); //钞汇标志
		bizContent.setAmount("10000"); //交易额100元
		bizContent.setSummary("回馈"); //摘要
		bizContent.setRemarks("回馈"); //注释
		bizContent.setNotifyAddr("http://seas.icbc/seas/index.jsp"); //联机通知地址
		bizContent.setSecretKey("h8ZujhDntpKRohwFmGXcnXygNjJHRQbyuVG3183uOdXI2fRgCXeDED9zOw5dO2JucVy7vZ19od7CSUxI2/sVOn6dPklVoaquVElgKRUPlr6D/cqLu25K7hJ3Pt/uOnx4qF/ykmO/IB2qS0rs/sp0Zw==");
		request.setBizContent(bizContent);

		SettlementAccountFeedbackResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess() && response.getReturnCode() == 0) {
			// 业务成功处理
			System.out.println(response.getAccountBalance());//
		} else {
			// 失败
			System.err.println(response.getReturnCode());//
			System.err.println(response.getReturnMsg());//
		}
	}
}
