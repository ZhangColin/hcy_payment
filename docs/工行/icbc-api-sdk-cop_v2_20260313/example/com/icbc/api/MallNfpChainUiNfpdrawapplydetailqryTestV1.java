package com.icbc.api;

import com.icbc.api.request.NfpwChainDrawapplyDetailqryRequestV1;
import com.icbc.api.request.NfpwChainDrawapplyDetailqryRequestV1.NfpwChainDrawapplyDetailqryRequestV1Biz;

// 网络融资产业链提款申请详情查询服务（页面）
public class MallNfpChainUiNfpdrawapplydetailqryTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APP_ID = "10000000000002354554";
	protected static final String SERVICE_URL = "http://ip:port/ui/mall/nfp/chain/ui/nfpdrawapplydetailqry/V1";

	public static void main(String[] args) {
		String buildPostForm = "";
		UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
		NfpwChainDrawapplyDetailqryRequestV1 request = new NfpwChainDrawapplyDetailqryRequestV1();
		request.setServiceUrl(SERVICE_URL);
		NfpwChainDrawapplyDetailqryRequestV1Biz bizContent = new NfpwChainDrawapplyDetailqryRequestV1Biz();
		bizContent.setAppid(APP_ID);
		bizContent.setOutVendorId("190002");
		bizContent.setQueryType("0");
		bizContent.setDrawapplyId("123456");
		request.setBizContent(bizContent);
		try {
			buildPostForm = client.buildPostForm(request);
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
		System.out.println(buildPostForm);
	}
	
}

