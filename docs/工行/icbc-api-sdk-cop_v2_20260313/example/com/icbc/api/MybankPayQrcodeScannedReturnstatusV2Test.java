...
import com.icbc.api.request.MybankPayQrcodeScannedReturnstatusRequestV2.MybankPayQrcodeScannedReturnstatusRequestV2Biz;
import com.icbc.api.response.MybankPayQrcodeScannedReturnstatusResponseV2;

public class MybankPayQrcodeScannedReturnstatusV2Test {
	// 1、网关公钥
	...
	public static void main(String[] args) throws Exception {
		...
		MybankPayQrcodeScannedReturnstatusRequestV2 request = new MybankPayQrcodeScannedReturnstatusRequestV2();
				
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://*****/api/mybank/pay/qrcode/scanned/returnstatus/V2");
		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		MybankPayQrcodeScannedReturnstatusRequestV2Biz bizContent = new MybankPayQrcodeScannedReturnstatusRequestV2Biz();
		bizContent.setMerId("0200****460001");
		bizContent.setCustId("123");
		//该字段非必输项
		bizContent.setOutTradeNo("");//该字段非必输项,out_trade_no和order_id选一项上送即可
		bizContent.setOrderId("0200010204*****5001");//该字段非必输项,out_trade_no和order_id选一项上送即可
		bizContent.setRejectNo("20190507****0493144989");

		request.setBizContent(bizContent);
		MybankPayQrcodeScannedReturnstatusResponseV2 response;
		try {
			response = client.execute(request, System.currentTimeMillis()+"");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
			/*System.out.println(response.getCardNo());
