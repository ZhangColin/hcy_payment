package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiVacctNovelAcctTransactionDetailListQueryRequestV1;
import com.icbc.api.request.JftApiVacctNovelAcctTransactionDetailListQueryRequestV1.JftApiVacctNovelAcctTransactionDetailListQueryRequestV1Biz;
import com.icbc.api.response.JftApiVacctNovelAcctTransactionDetailListQueryResponseV1;
import org.junit.Test;
/**
 * 新型登记簿明细列表查询
 *
 * @author Jxg
 * @date 2021/5/26 10:52
 */
public class JftApiVacctNovelAcctTransactionDetailListQueryTestV1 {
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    protected static final String APP_ID = "10000000000000004964";

    @SuppressWarnings("unchecked")

    public void test_cop() {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");
        JftApiVacctNovelAcctTransactionDetailListQueryRequestV1 request = new JftApiVacctNovelAcctTransactionDetailListQueryRequestV1();
        request.setServiceUrl("http://ip:port/api/jft/api/vacct/novel/acct/transaction/detail/list/query/V1");
        JftApiVacctNovelAcctTransactionDetailListQueryRequestV1Biz bizContent = new JftApiVacctNovelAcctTransactionDetailListQueryRequestV1.JftApiVacctNovelAcctTransactionDetailListQueryRequestV1Biz();
        bizContent.setAppId(APP_ID);
        bizContent.setAcctNo("12345678901234567890");
        bizContent.setCreateDate("2020-05-13");
        bizContent.setEndDate("2020-05-14");
        bizContent.setPage("");
        bizContent.setPageSize("");
        bizContent.setVersion("1.0.0");//设置接口访问版本号
        request.setBizContent(bizContent);
        JftApiVacctNovelAcctTransactionDetailListQueryResponseV1 response;
        try {
            response = (JftApiVacctNovelAcctTransactionDetailListQueryResponseV1) client.execute(request, System.currentTimeMillis()+"");

            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
