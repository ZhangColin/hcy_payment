package com.icbc.api;


import com.icbc.api.request.CertAtmElockElockpasswordrequestRequestV1;
import com.icbc.api.response.CertAtmElockElockpasswordrequestResponseV1;
import org.junit.Assert;
import org.junit.Test;

import java.text.SimpleDateFormat;
import java.util.Date;
import java.util.logging.Logger;

/**
 * @program: icbc-api-sdk-cop
 * @description 测试类
 * @author: kfzx-xiewx01
 * @create: 2020-07-13 19:41
 **/
public class CertAtmElockElockpasswordrequestTestV1 {
    private Logger logger = Logger.getLogger(CertAtmElockElockpasswordrequestTestV1.class.getName());

    //密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    //网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //应用ID，APP申请分配appid
    protected static final String APP_ID = "xxxxxxxxx";


    public void test_cop() throws IcbcApiException {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CertAtmElockElockpasswordrequestRequestV1 request = new CertAtmElockElockpasswordrequestRequestV1();
        //根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://ip:port/api/cert/atm/elock/elockpasswordrequest/V1");

        CertAtmElockElockpasswordrequestRequestV1.CertAtmElockElockpasswordrequestRequestV1Biz bizContent = new CertAtmElockElockpasswordrequestRequestV1.CertAtmElockElockpasswordrequestRequestV1Biz();
        CertAtmElockElockpasswordrequestRequestV1.ReqPubField pubField = new CertAtmElockElockpasswordrequestRequestV1.ReqPubField();
        pubField.setChancode("702");
        pubField.setOapp("F-BIOM");
        pubField.setOtellerno("49");
        bizContent.setPubField(pubField);

        CertAtmElockElockpasswordrequestRequestV1.ReqPriField priField = new CertAtmElockElockpasswordrequestRequestV1.ReqPriField();
        priField.setCloseCode("12345673");
        priField.setRandomNumber("12345678");
        priField.setElockDevid("DW1234567890112");
        priField.setAuthUserA("001100845");
        priField.setAuthUserB("001100885");
        priField.setBranchId("0200001242");
        priField.setDevid("000000002000012");
        priField.setOperUser("002200835");

        bizContent.setPriField(priField);

        request.setBizContent(bizContent);
        CertAtmElockElockpasswordrequestResponseV1 response = client.execute(request,System.currentTimeMillis()+"");//请求通讯唯一编号
        boolean testFlag = true;
        logger.info(String.valueOf(response.getReturnCode()));
        logger.info(response.getReturnMsg());
        if (response.isSuccess() && response.getReturnCode() == 0) {// 业务成功处理
            Assert.assertEquals(true,testFlag);
        } else {// 失败
            Assert.assertEquals(false, testFlag);
        }

    }
}
