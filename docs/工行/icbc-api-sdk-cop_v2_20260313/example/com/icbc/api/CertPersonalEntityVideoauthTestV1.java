/*
package com.icbc.api;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcConstants;
import com.icbc.api.IcbcApiException;
import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.CertPersonalEntityVideoauthRequestV1;
import com.icbc.api.response.CertPersonalEntityVideoauthResponseV1;

public class CertPersonalEntityVideoauthTestV1 {
    public static void main(String[] args) {
        // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值

        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "xxxx";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //4、创建与F-APIP外网网关的连接
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, (String)null, (String)null, (String)null, (String)null);
        //5、设置请求对象
        CertPersonalEntityVideoauthRequestV1 request = new CertPersonalEntityVideoauthRequestV1();
        //6、设置请求路径；联测时，请联系我行告知并替换成测试环境地址。
        request.setServiceUrl("http://xxxxx:xxx/api/cert/personal/entity/videoauth/V1");
        //7、设置业务参数
        CertPersonalEntityVideoauthRequestV1.CertPersonalEntityVideoauthRequestV1Biz bizContent = new CertPersonalEntityVideoauthRequestV1.CertPersonalEntityVideoauthRequestV1Biz();

        //8、组装请求参数
        bizContent.setChannel("4");
        bizContent.setIdCode("141002199501230097");
        bizContent.setName("孟磊");
        bizContent.setReqId("123456");
        bizContent.setTrxZone("1001");
        bizContent.setVideo1("");
        bizContent.setIsUseLabel("1");
        bizContent.setLabelByVideo("1345");
        bizContent.setTokenByVideo("234545234");
        bizContent.setValidStart("20170808");
        bizContent.setValidEnd("20280303");
        request.setBizContent(bizContent);
        CertPersonalEntityVideoauthResponseV1 response;
        try {
            System.out.println("request:"+ JSON.toJSONString(request));
            response = client.execute(request,"111");
            System.out.println("response:" + JSON.toJSONString(response));
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
*/
