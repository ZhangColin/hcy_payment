package com.icbc.api;


import com.icbc.api.request.JftApiPaySplitAccountRequestV1;
import com.icbc.api.response.JftApiPaySplitAccountResponseV1;

import java.util.ArrayList;
import java.util.List;


public class JftApiPaySplitAccountTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000011524";
											
	
	public static void main(String[] args) {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8", 
				"json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");

		JftApiPaySplitAccountRequestV1 request = new JftApiPaySplitAccountRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/api/pay/split/V1");
		JftApiPaySplitAccountRequestV1.JftApiPaySplitAccountRequestV1Biz bizContent = new JftApiPaySplitAccountRequestV1.JftApiPaySplitAccountRequestV1Biz();
		bizContent.setAppId(APP_ID);//平台商户标识
		bizContent.setOutVendorId("201809121");//平台子商户id
		bizContent.setNotifyUrl("http://shanghu.com/index/JumpUrl.html");//分账通知url
		bizContent.setOutOrderId("12018092700003540815");//平台订单ID
		bizContent.setOutSplitId("1111021");
		bizContent.setSplitType("00");

		List<JftApiPaySplitAccountRequestV1.SplitAccountDetail> details = new ArrayList<JftApiPaySplitAccountRequestV1.SplitAccountDetail>();
		JftApiPaySplitAccountRequestV1.SplitAccountDetail detail1 = new JftApiPaySplitAccountRequestV1.SplitAccountDetail();
		detail1.setOutDevId("201809121");//原子商户
		detail1.setDevSerPercent("50");//分润比例
		detail1.setOutSplitDetailId("1111022");//分润明细编号
		details.add(detail1);
		JftApiPaySplitAccountRequestV1.SplitAccountDetail detail2 = new JftApiPaySplitAccountRequestV1.SplitAccountDetail();
		detail2.setOutDevId("201809122");//分润方
		detail2.setDevSerPercent("50");//分润比例
		detail2.setOutSplitDetailId("1111023");//分润明细编号
		details.add(detail2);
		bizContent.setSplitAccountDetail(details);
		request.setBizContent(bizContent);

		JftApiPaySplitAccountResponseV1 response;
		try {
			response = client.execute(request, System.currentTimeMillis()+"");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
			if (response.isSuccess()) {
				//6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("response:" + response);
			} else {
				//失败
				System.out.println("ReturnCode:"+response.getReturnCode());
				System.out.println("ReturnMsg:"+response.getReturnMsg());
			}		
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}

