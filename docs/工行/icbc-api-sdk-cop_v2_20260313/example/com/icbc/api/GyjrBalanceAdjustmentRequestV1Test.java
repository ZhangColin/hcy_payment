package com.icbc.api;

import com.alibaba.fastjson.JSON;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcConstants;
import com.icbc.api.IcbcResponse;
import com.icbc.api.request.GyjrBalanceAdjustmentRequestV1;
import com.icbc.api.response.GyjrBalanceAdjustmentResponseV1;


/**
 * @author kfzx-wujw
 * @version 1.0
 * @description TODO
 * @date 2022/11/4 13:48
 */
public class GyjrBalanceAdjustmentRequestV1Test {
    protected static final String APP_ID = "10000000000004098168";
    //protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    // API网关公钥(变量值需替换)
    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    protected static String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    public static void main(String[] args) {
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        try {
            GyjrBalanceAdjustmentRequestV1 request = new GyjrBalanceAdjustmentRequestV1();
            request.setServiceUrl("http://ip:port/api/gyjr/b2b/bankEnterpriseRecon/balanceAdjustment/V1");
            GyjrBalanceAdjustmentRequestV1.GygrBalanceAdjustmentRequestV1Biz bizContent = new GyjrBalanceAdjustmentRequestV1.GygrBalanceAdjustmentRequestV1Biz();
            GyjrBalanceAdjustmentRequestV1.GygrBalanceAdjustmentRequestV1Biz.TransInfo transInfo = new  GyjrBalanceAdjustmentRequestV1.GygrBalanceAdjustmentRequestV1Biz.TransInfo();
            transInfo.setAccno("02000002092133528");
            transInfo.setAppId("987987");
            transInfo.setProtocolCode("GYJR20221031170517175329288");
            transInfo.setCurrtype("001");
            transInfo.setFseqno("020000002023123139001113");
            transInfo.setBalance("99999");
            bizContent.setTransInfo(transInfo);
            request.setBizContent(bizContent);
            System.out.println("入参：" + JSON.toJSONString(request));
            IcbcResponse response =  client.execute(request);
            System.out.println("出参：" + JSON.toJSONString(response));
            if (response.isSuccess()) {
                System.out.println("success");// 成功
            } else {
                System.out.println("error");// 失败
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}
