package com.icbc.api;

import java.util.UUID;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountOpenRealtimeimgUploadRequestV1;
import com.icbc.api.request.SettlementAccountOpenRealtimeimgUploadRequestV1.SettlementAccountOpenRealtimeimgUploadRequestV1Biz;
import com.icbc.api.response.SettlementAccountOpenRealtimeimgUploadResponseV1;


public class SettlementAccountOpenRealtimeimgUploadTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	

	public void test_cop() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		SettlementAccountOpenRealtimeimgUploadRequestV1 request = new SettlementAccountOpenRealtimeimgUploadRequestV1();
		SettlementAccountOpenRealtimeimgUploadRequestV1Biz bizContent = new SettlementAccountOpenRealtimeimgUploadRequestV1Biz();
		request.setServiceUrl("http://ip:port/api/settlement/account/open/realtimeimg/upload/V1");
		bizContent.setOutServiceCode("uploadimg"); //外部服务代码
		bizContent.setCorpSerno(UUID.randomUUID().toString());//合作方交易单号					
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setTrxDate("2018-10-24");//合作方交易日期
		bizContent.setTrxTime("10:38:01");//合作方交易时间		
		bizContent.setCustName("张小二");//姓名
		bizContent.setCertNo("441402198010010004"); //证件号码
		bizContent.setValidityPeriod("2019-05-23");//证件有效期截止日
		bizContent.setFrontImgType("01");//正面图片类型
		bizContent.setFrontImg("");//正面图片
		bizContent.setBackImgType("01");//背面图片类型
		bizContent.setBackImg("");//背面图片
		bizContent.setMediumId("6222888888888888888"); //选输 卡号(支持加密，Base64编码)
		bizContent.setSecretKey("ASDQWEQDZCSDFAWWQDA"); //选输 SM2公钥加密的sm4对称密钥
		bizContent.setClearFlag(0);//是否补上传标志
		bizContent.setFaceImgType("01");//现场图片类型
		bizContent.setFaceImg("");//现场图片
		
		request.setBizContent(bizContent);
		SettlementAccountOpenRealtimeimgUploadResponseV1  response = client.execute(request, "msgId");
		
		if (response.isSuccess() ) {
			// 业务成功处理
			response.getCorpSerno(); //合作方交易单号
		} else {
			// 失败
		}
	}
}
