package com.icbc.api;

import com.icbc.api.request.EcspNftCustomerRegisterRequestV1;


public class EcspNftCustomerRegisterRequestV1Test {
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000002156";

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    public static void main(String[] args) throws Exception {
        String priKey = "TKhSiK9Blwr6+aCq+O0MFg==";
        UiIcbcClient client = new UiIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, "UTF-8", "AES", priKey);

        EcspNftCustomerRegisterRequestV1 request = new EcspNftCustomerRegisterRequestV1();

        request.setServiceUrl("https://gw.icloud.icbc.com.cn/ui/ecsp/nft/customer/register/V1");

        EcspNftCustomerRegisterRequestV1.EcspNftCustomerRegisterRequestV1Biz bizContent = new EcspNftCustomerRegisterRequestV1.EcspNftCustomerRegisterRequestV1Biz();
        bizContent.setCorpId("0000000001");
        bizContent.setPhoneNo("15919191616");
        bizContent.setCallbackUrl("https://www.xxx.com/call");
        request.setBizContent(bizContent);
        System.out.println(client.buildPostForm(request));
    }
}
