package com.icbc.api;

import java.util.List;

import com.icbc.api.request.Queryca29126RequestV1;
import com.icbc.api.request.Queryca29126RequestV1.Queryca29126RequestV1Biz;
import com.icbc.api.request.Queryca29126RequestV1.Queryca29126RequestV1Pub;
import com.icbc.api.request.Queryca29126RequestV1.Queryca29126RequestV1In;
import com.icbc.api.response.Queryca29126ResponseV1;
import com.icbc.api.response.Queryca29126ResponseV1.Queryca29126ResponseV1Pub;
import com.icbc.api.response.Queryca29126ResponseV1.Queryca29126ResponseV1Out;
import com.icbc.api.response.Queryca29126ResponseV1.Queryca29126ResponseV1Detail;

public class Queryca29126Test {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002116";

	public void test_CA() throws IcbcApiException {
		
		String castr = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				"",
				APIGW_PUBLIC_KEY, castr, "12345678");
		
		Queryca29126RequestV1 request = new Queryca29126RequestV1();
		request.setServiceUrl("http://ip:port/api/queryca29126/query/V1");

		Queryca29126RequestV1Biz bizContent = new Queryca29126RequestV1Biz();
		
		Queryca29126RequestV1Pub pub = new Queryca29126RequestV1Pub();
		pub.setCode("29126");
		pub.setZone("0200");
		pub.setFr("0");
		pub.setDt("2018-12-26");
		
		Queryca29126RequestV1In in = new Queryca29126RequestV1In();
		in.setGrpmerno("123412413131");
		in.setSadlindate("2017-07-06");
		in.setQuerytype("1");
		in.setPage("1");
		in.setBeginrow("1");
		in.setEndrow("70");
		
		bizContent.setPub(pub);
		bizContent.setIn(in);
		
		request.setBizContent(bizContent);

		Queryca29126ResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess()) {
			// 接收成功
			Queryca29126ResponseV1Pub RetPub = response.getPub();
			Queryca29126ResponseV1Out RetOut = response.getOut();
			if(RetOut.getResult().equals("00000000")){
				//查询成功 获取返回结果
				List<Queryca29126ResponseV1Detail> resultList = RetOut.getDetail();
				//获取结果集中字段
				for(Queryca29126ResponseV1Detail detail:resultList){
					//获取信息.....
					detail.getGRPMERNO();
				}
			} else {
				//查询失败
				System.out.println(RetOut.getResult());
			}
		} else {
			// 失败
			System.out.println(response.getReturnMsg());
		}
	}
}
