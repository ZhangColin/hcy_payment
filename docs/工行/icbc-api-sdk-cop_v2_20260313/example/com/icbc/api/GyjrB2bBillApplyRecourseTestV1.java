package com.icbc.api;

import com.icbc.api.request.GyjrB2bBillApplyRecourseRequestV1;
import org.junit.Test;
import java.util.ArrayList;
import java.util.List;

public class GyjrB2bBillApplyRecourseTestV1 {
    protected static final String APP_ID = "10000000000004097933";
    // 应用私钥(变量值需替换)
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
    // API网关公钥(变量值需替换)
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    
    public void test_request() {

        try {
            UiIcbcClient client = new UiIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY,"UTF-8");
            GyjrB2bBillApplyRecourseRequestV1 request = new GyjrB2bBillApplyRecourseRequestV1();
            // 根据实际环境替换相应的域名
            request.setServiceUrl("https://ip:port/ui/gyjr/b2b/bill/applyrecourse/V1");
            GyjrB2bBillApplyRecourseRequestV1.GyjrB2bBillApplyRecourseRequestV1Biz  biz = new GyjrB2bBillApplyRecourseRequestV1.GyjrB2bBillApplyRecourseRequestV1Biz();
            GyjrB2bBillApplyRecourseRequestV1.GyjrB2bBillApplyRecourseRequestV1Biz.Bill  bill = new GyjrB2bBillApplyRecourseRequestV1.GyjrB2bBillApplyRecourseRequestV1Biz.Bill();
            List<GyjrB2bBillApplyRecourseRequestV1.GyjrB2bBillApplyRecourseRequestV1Biz.Bill> billList = new ArrayList<>();
            billList.add(bill);
            biz.setList(billList);
            biz.setProtocol_code("1");
            biz.setPlat_vendor_id("1");
            biz.setPlat_batSerial_no("1");
            biz.setTrade_name("applyRecourse");
            biz.setTrade_version("V1");
            biz.setPackNo("11");
            biz.setCdTp("AC01");
            biz.setAccptrName("11");
            biz.setIsRefuse("1");
            biz.setRangeBgn("11");
            biz.setRangeEnd("12");
            biz.setHolderAcctId("11");
            biz.setRcrsrName("11");
            biz.setRcrsType("RT01");
            biz.setRangeAmt("11");
            biz.setDueDate("20230202");
            biz.setTotalAmt("11");
            biz.setTotalCount("11");
            biz.setCallBackUrl("11");
            request.setBizContent(biz);
            String s =  client.buildPostForm(request);
            System.out.println(s);
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
