package com.icbc.api;

import java.util.ArrayList;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.request.InvestmentFundoutsourceTrailbalancereportqueryRequestV1;
import com.icbc.api.request.InvestmentFundoutsourceTrailbalancereportqueryRequestV1.InvestmentFundoutsourceTrailbalancereportqueryRequestV1Biz;
import com.icbc.api.request.InvestmentFundoutsourceTrailbalancereportqueryRequestV1.InvestmentFundoutsourceTrailbalancereportqueryRequestV1In;
import com.icbc.api.request.InvestmentFundoutsourceTrailbalancereportqueryRequestV1.InvestmentFundoutsourceTrailbalancereportqueryRequestV1InRecord;
import com.icbc.api.response.InvestmentFundoutsourceTrailbalancereportqueryResponseV1;


public class InvestmentFundoutsourceTrailbalancereportqueryTest {

	//科目余额表
	protected static String APP_ID = "10000000000000013551";
	protected static String BASE_URL = "http://ip:port/api"; // 行外

	//网关公钥
	protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//应用私钥（与内管平台注册app时的“签名公钥”配对）
	protected static String PRI_KEY = "-----请使用正确的APP私钥-----";

	public static void main(String[] args) throws Exception {
		String URI = "investment/fundoutsource/trailbalancereportquery/V1"; // api地址
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
		try {

			// 请求实体
			InvestmentFundoutsourceTrailbalancereportqueryRequestV1 request = new InvestmentFundoutsourceTrailbalancereportqueryRequestV1();
			// bizContent实体
			InvestmentFundoutsourceTrailbalancereportqueryRequestV1Biz bizContent = new InvestmentFundoutsourceTrailbalancereportqueryRequestV1Biz();
			// 请求报文IN节点实体
			InvestmentFundoutsourceTrailbalancereportqueryRequestV1In requestIn = new InvestmentFundoutsourceTrailbalancereportqueryRequestV1In();
			// Record实体
			InvestmentFundoutsourceTrailbalancereportqueryRequestV1InRecord instBean = new InvestmentFundoutsourceTrailbalancereportqueryRequestV1InRecord();

			// Record实体 设置属性
//			instBean.setReportKind("03");

			ArrayList<InvestmentFundoutsourceTrailbalancereportqueryRequestV1InRecord> list = new ArrayList<>();
			list.add(instBean);

			// 请求报文IN节点实体 设置属性
			requestIn.setFileType("4501");
			requestIn.setFundId("333020");
			requestIn.setReportType("01");
			requestIn.setBeginDate("20080930");
			requestIn.setEndDate("20080930");
			requestIn.setDeptCode("G001");
			requestIn.setCertId("");
			requestIn.setSerialNo("20181024000001066406");
			requestIn.setInRecords(list);

			// bizContent实体 设置属性
			bizContent.setIn(requestIn);

			// 请求实体 设置属性
			request.setServiceUrl(BASE_URL + "/" + URI);
			request.setBizContent(bizContent);

			System.out.println(request.getMethod());
			// 输出支付指令提交响应对象，需根据实际调用服务更换（InvestmentFundoutsourceTrailbalancereportqueryResponseV1）
			InvestmentFundoutsourceTrailbalancereportqueryResponseV1 response = client.execute(request);
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println("success");//
			} else {
				// 失败
				System.out.println("error");
			}

		} catch (Exception e) {
			e.printStackTrace();
		}
	}

}
