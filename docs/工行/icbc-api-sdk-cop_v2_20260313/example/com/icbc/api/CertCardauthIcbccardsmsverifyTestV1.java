package com.icbc.api;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.CertCardauthIcbccardsmsverifyRequestV1;
import com.icbc.api.request.CertCardauthIcbccardsmsverifyRequestV1.CertCardauthIcbccardsmsverifyBiz;
import com.icbc.api.response.CertCardauthIcbccardsmsverifyResponseV1;

import java.text.SimpleDateFormat;
import java.util.logging.Logger;


public class CertCardauthIcbccardsmsverifyTestV1 {
    public static void main(String[] args) throws IcbcApiException {
        Logger log = Logger.getLogger(CertCardauthIcbccardsmsverifyTestV1.class.getName());

        //1、API平台申请的APPID
        String APP_ID = "XXX";

        //2、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        //3、密钥对认证方式，公钥在API平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CertCardauthIcbccardsmsverifyRequestV1 request = new CertCardauthIcbccardsmsverifyRequestV1();

        //4、API网关地址：根据测试环境和生产环境替换相应IP和端口
//        request.setServiceUrl("http://ip:port/api/cert/cardauth/icbccardsmsverify/V1");//功能
        request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cert/cardauth/icbccardsmsverify/V1");//功能外网


        //5、请参照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        CertCardauthIcbccardsmsverifyBiz bizContent = new CertCardauthIcbccardsmsverifyBiz();

        SimpleDateFormat sdfDate = new SimpleDateFormat("yyyy-MM-dd");
        SimpleDateFormat sdfTime = new SimpleDateFormat("HH:mm:ss");
        long dataNum = System.currentTimeMillis();

        bizContent.setTrxSerno("XXX");
        bizContent.setTrxDate(sdfDate.format(dataNum));
        bizContent.setTrxTime(sdfTime.format(dataNum));
        bizContent.setPhoneNo("18820423459");//手机号
        bizContent.setVerifyCode("352484");//动态码
        bizContent.setVerifyCodeNo("574935");//动态码编号

        request.setBizContent(bizContent);
        CertCardauthIcbccardsmsverifyResponseV1 response;
        try {
            String msg_id = String.valueOf(System.currentTimeMillis());
            response = client.execute(request, msg_id);//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
            if (response.isSuccess()) {
                // 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
                log.info(">> Request succeed.");
                log.info(JSON.toJSONString(response, true));
            } else {
                // 失败
                log.info(">> Request failed.");
                log.info(JSON.toJSONString(response, true));
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }

}
