package com.icbc.api;
import org.junit.Assert;
import org.junit.Test;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.request.CertCustomerRisklevelqryRequestV2;
import com.icbc.api.request.CertCustomerRisklevelqryRequestV2.CertCustomerRisklevelqryRequestV2Biz;
import com.icbc.api.response.CertCustomerRisklevelqryResponseV2;

/**
 * 反洗钱客户风险分类等级查询
 * 2021年2月版：新的分类标准查询接口
 * @author kfzx-hanrj01
 *
 */
public class CertCustomerRisklevelqryV2Test {


	public void test_cop() {
		String APP_ID = "*****";		//appid
		String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
		String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		CertCustomerRisklevelqryRequestV2 request = new CertCustomerRisklevelqryRequestV2();
		request.setServiceUrl("https://ip:port/api/cert/customer/risklevelqry/V2");
		CertCustomerRisklevelqryRequestV2Biz bizContent = new CertCustomerRisklevelqryRequestV2Biz();
		bizContent.setSerialNo("*****");		//请求序列号
		bizContent.setAppNo("F-AML");
		bizContent.setAreaCode("0020000067");
		bizContent.setTeller("020083999");
		bizContent.setLanguage("zh_CN");
		bizContent.setVerify("acst4aml");
		bizContent.setVer("2.0");
		bizContent.setQueryType("1");
		bizContent.setCustNo("020090001957324");
		bizContent.setBranchId("");
		bizContent.setCustType("");
		bizContent.setCustName("");
		bizContent.setRegType("");
		bizContent.setRegNo("");

		request.setBizContent(bizContent);

		CertCustomerRisklevelqryResponseV2 response;
		try {
			response = client.execute(request, "msgId");
			if (response.isSuccess() ) {
				// 业务成功处理
				Assert.assertEquals(response.getStatus(), 0);
			} else {
				// 失败
			}		
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
