...
import com.icbc.api.request.CardbusinessMerTripCheckedticketqueryRequestV1;
import com.icbc.api.response.CardbusinessMerTripCheckedticketqueryResponseV1;
import org.apache.commons.httpclient.util.DateUtil;

import java.util.Date;

public class CardbusinessMerTripCheckedTicketQueryV1Test {
    ...
    public static void main(String[] args) throws Exception {
        ...
        CardbusinessMerTripCheckedticketqueryRequestV1 request = new CardbusinessMerTripCheckedticketqueryRequestV1();
        // 4、根据测试环境和生产环境替换相应ip和端口 http://ip:port/api
        request.setServiceUrl("http://ip:port/api/cardbusiness/mer/trip/checkedticketquery/V1");
        // 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        CardbusinessMerTripCheckedticketqueryRequestV1.CardbusinessMerTripCheckedticketqueryRequestV1Biz biz = new CardbusinessMerTripCheckedticketqueryRequestV1.CardbusinessMerTripCheckedticketqueryRequestV1Biz();
        request.setBizContent(biz);
        // 核销信息
        biz.setCorpId("2000000136");
        biz.setOperType("001");
        biz.setTrxDate(DateUtil.formatDate(new Date(),"yyyy-MM-dd HH:mm:ss"));
        biz.setSpotPointId("1");
        biz.setTerminalId("Bcss1234");
        // msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
        String msgId = String.valueOf(System.currentTimeMillis());
        CardbusinessMerTripCheckedticketqueryResponseV1 response = client.execute(request, msgId);
