package com.icbc.api;

import com.icbc.api.request.CacaReceiptTipsposreceiptUpdatecountRequestV1;
import com.icbc.api.response.CacaReceiptTipsposreceiptUpdatecountResponseV1;

import java.util.ArrayList;

public class CacaReceiptTipsposreceiptUpdatecountTestV1 {
    // 示例总体说明：请使用F-APIP最新的行外网关SDK进行开发，签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
    public static void main(String[] args) {
        //1、密钥对认证方式，公钥在F-APIP平台登记，此处为私钥
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        //2、appid
        String APP_ID = "1";
        //3、网关公钥
        String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
        CacaReceiptTipsposreceiptUpdatecountRequestV1 request = new CacaReceiptTipsposreceiptUpdatecountRequestV1();
        request.setServiceUrl("https://ip:port/api/caca/receipt/tipsposreceipt/updatecount/V1");
        CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesRequestV1Biz bizContent = new CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesRequestV1Biz();
        // public 通讯区定义赋值
        CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesRequestV1Pub pubObj = new CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesRequestV1Pub();
        pubObj.setCode("29009");
        pubObj.setZone("0200");
        pubObj.setFr("0");
        pubObj.setDt("2017-07-06");
        // private 通讯区定义赋值
        CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesRequestV1In inObj = new CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesRequestV1In();

        inObj.setZone("0200");
        inObj.setBrno("5555");
        inObj.setTelno("");
        inObj.setChannel("203");
        inObj.setTelno("");

        CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesCondRequestV1In condIn = new CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesCondRequestV1In();
        condIn.setOpertype("");
        condIn.setAccno("02000241000011111");
        condIn.setMainaccno("02000241000011111");
        condIn.setSeqno("000000001");
        condIn.setCurrtype("001");
        condIn.setPrint_flag("1");
        condIn.setBustypno("");
        condIn.setCardno("4580651200001111111");
        condIn.setDeviceno("");
        condIn.setHelp_custmoer("");
        condIn.setPartitioncolm("");
        condIn.setAmount("");
        condIn.setHostdate("");
        ArrayList<CacaReceiptTipsposreceiptUpdatecountRequestV1.UpdatePrintTimesCondRequestV1In> updatePrintTimesRequestCondV1Ins = new ArrayList<>();
        updatePrintTimesRequestCondV1Ins.add(condIn);
        inObj.setCond(updatePrintTimesRequestCondV1Ins);
        bizContent.setIn(inObj);
        bizContent.setPub(pubObj);
        request.setBizContent(bizContent);
        try {
            CacaReceiptTipsposreceiptUpdatecountResponseV1 response = client.execute(request);
            if (response.isSuccess()) {
                // 业务成功处理
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}

