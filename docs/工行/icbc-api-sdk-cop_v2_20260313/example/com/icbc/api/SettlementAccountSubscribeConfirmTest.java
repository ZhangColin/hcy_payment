package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.SettlementAccountSubscribeConfirmRequestV1;
import com.icbc.api.request.SettlementAccountSubscribeConfirmRequestV1.SettlementAccountSubscribeConfirmRequestV1Biz;
import com.icbc.api.response.SettlementAccountSubscribeConfirmResponseV1;



public class SettlementAccountSubscribeConfirmTest{
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//Appid：通过内管查看详情，可获取对应的组织机构、接入方式、公钥等信息
	//APP管理中查看
	protected static final String APP_ID = "10000000000000021587";
	
	
	public void test_cop() throws IcbcApiException{
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		
		SettlementAccountSubscribeConfirmRequestV1 request = new SettlementAccountSubscribeConfirmRequestV1();
		request.setServiceUrl("http://ip:port/api/settlement/account/subscribeconfirm/V1");
		SettlementAccountSubscribeConfirmRequestV1Biz bizContent = new SettlementAccountSubscribeConfirmRequestV1Biz();
		
		bizContent.setCorpNo("corpInst1234"); //合作方机构编号
		bizContent.setCorpSerno("20202020"); //合作方交易单号
		bizContent.setOutServiceCode("subscribeconfirm");
		bizContent.setMediumId("6214760200000022233");
		bizContent.setSummary("summary");
		bizContent.setRemarks("remarks");
		bizContent.setSecretKey("");
		bizContent.setMediumIdHash("");
		bizContent.setResult("1"); //交易结果 
		bizContent.setErrNo("0"); //交易结果
		bizContent.setMsgNo("0"); //交易结果
		bizContent.setErrMsg(""); //交易结果
		bizContent.setWorkdate("2017-03-02"); //我行工作日期
		bizContent.setAccountUseamt("5987");
		bizContent.setAccountBalance("5987");


		request.setBizContent(bizContent);
		
		SettlementAccountSubscribeConfirmResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess() && response.getReturnCode() == 0) {
			// 业务成功处理
		} else {
			// 失败
			System.err.println(response.getReturnCode());//
			System.err.println(response.getReturnMsg());//
		}
	}
}
