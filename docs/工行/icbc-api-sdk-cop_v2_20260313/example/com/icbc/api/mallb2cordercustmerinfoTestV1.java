package com.icbc.api;

import org.junit.Test;

import com.icbc.api.request.mallb2cordercustmerinfoRequestV1;
import com.icbc.api.request.mallb2cordercustmerinfoRequestV1.mallb2CordercustmerinfoRequestV1Biz;
import com.icbc.api.response.mallb2CordercustmerinfoResponseV1;

public class mallb2cordercustmerinfoTestV1 {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000005530";


	public void test_cop() throws IcbcApiException {
		//http://ip:port:8181/marketOrder/toMarketOrderPage.jhtml
		//DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		//String appId, String signType, String privateKey, String charset, String format,		String icbcPulicKey, 
		//String encryptType, String encryptKey, String ca, String password
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", 
				"json",APIGW_PUBLIC_KEY,"AES","G242nUlMmiT1tcToX29xkg==", "", "");
		mallb2cordercustmerinfoRequestV1 request = new mallb2cordercustmerinfoRequestV1();
		request.setServiceUrl("http://ip:port/api/MalluserInfo/ForOuter/Service/query/V1");

		mallb2CordercustmerinfoRequestV1Biz bizContent = new mallb2CordercustmerinfoRequestV1Biz();
		bizContent.setUserInfoKey("201901150000007609");
		bizContent.setAppId("10000000000000005530");
		request.setBizContent(bizContent);
		
		mallb2CordercustmerinfoResponseV1 response;
		try {
			response = client.execute(request, System.currentTimeMillis()+"");
//			String a = client.execute(request, System.currentTimeMillis()+"");
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}

	}
	
	
}
