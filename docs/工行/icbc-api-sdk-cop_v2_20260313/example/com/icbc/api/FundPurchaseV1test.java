package com.icbc.api;

import org.junit.Test;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.FundPurchaseRequestV1;
import com.icbc.api.request.FundPurchaseRequestV1.FundPurchaseRequestV1Biz;
import com.icbc.api.response.FundPurchaseResponseV1;

public class FundPurchaseV1test {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002166";


	public void test_cop() throws IcbcApiException {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

		FundPurchaseRequestV1 request = new FundPurchaseRequestV1();
		request.setServiceUrl("http://ip:port/api/fund/V1/purchase");

		FundPurchaseRequestV1Biz bizContent = new FundPurchaseRequestV1Biz();
		bizContent.setCorpNo("12345678");
		bizContent.setTrxSeqNo("99999");//合作方交易流水号，所有交易唯一，数字型
		bizContent.setCustNo("ABC123");//合作方客户唯一编号（例如：京东号）
		bizContent.setCardNo("6222080200000021603");//客户卡号 
		bizContent.setFundId("000848");//基金代码  
		bizContent.setAmount(1000);//金额
		bizContent.setTrxReqTs("2016-10-28 17:15:12.762126 ");//合作方交易请求时间戳
	
		request.setBizContent(bizContent);

		FundPurchaseResponseV1 response = client.execute(request);

		System.out.println(JSONObject.toJSONString(response));
		if (response.isSuccess()) {
			// 业务成功处理
			System.out.println(response.getReturnCode());
		} else {
			// 失败
		}
	}
}

