package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.MybankLoanHousefundsupHfsagreeconfirmRequestV1;
import com.icbc.api.request.MybankLoanHousefundsupHfsagreeconfirmRequestV1.MybankLoanHousefundsupHfsagreeconfirm;
import com.icbc.api.request.MybankLoanHousefundsupHfsagreeconfirmRequestV1.MybankLoanHousefundsupHfsagreeconfirmRequestV1Biz;
import com.icbc.api.response.MybankLoanHousefundsupHfsagreeconfirmResponseV1;
import java.util.logging.Logger;

public class MybankLoanHousefundsupHfsagreeconfirmV1Test {
    static Logger log = Logger.getLogger(MybankLoanHousefundsupHfsagreeconfirmV1Test.class.getName());
    //估值表
    protected static String APP_ID = "appid";
    //protected static String APP_ID = "10000000000000289005";
    protected static String BASE_URL = "http:/pi:port/api"; // 行外
   // protected static String BASE_URL = "http://ip:port/api"; // 行外

    //网关公钥
    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //应用私钥（与内管平台注册app时的“签名公钥”配对）
    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
    public static void main(String[] args) throws Exception {
        String URI = "/mybank/loan/housefundsup/hfsagreeconfirm/V1"; // api地址
       // DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2",PRI_KEY, APIGW_PUBLIC_KEY);
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", PRI_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, null, (String)null, (String)null, (String)null);               
        
        try {

            // 请求实体
            MybankLoanHousefundsupHfsagreeconfirmRequestV1 request = new MybankLoanHousefundsupHfsagreeconfirmRequestV1();
            // bizContent实体
            MybankLoanHousefundsupHfsagreeconfirmRequestV1Biz bizContent  = new MybankLoanHousefundsupHfsagreeconfirmRequestV1Biz();           
            MybankLoanHousefundsupHfsagreeconfirm param= new  MybankLoanHousefundsupHfsagreeconfirm();
            //param.setAppId("10000000000000289005");
            param.setAppId("1233");
            param.setAppNo("F-APIP");
            param.setAreaCode("0020000019");           
            param.setBankNo("0020000019");
            param.setBuyerIdType("0");
            param.setSerialNo("00000001");
            param.setCooperSerial("ooperSeria");
            param.setCooperPK("00000001");
            //param.setOrderNo("ZJ202011251016460000000000022038");
            param.setOrderNo("ZJ2020112510");
            param.setBuyerName("音傻帽");
            param.setBuyerIdNo("0");           
            bizContent.setParam(param);                                                        
            request.setServiceUrl(BASE_URL  + URI);  
            request.setBizContent(bizContent);
            MybankLoanHousefundsupHfsagreeconfirmResponseV1 response = client.execute(request);
            log.info("response:" + JSON.toJSONString(response));
            //System.out.println("response:" + JSON.toJSONString(response));
            boolean flag=response.isSuccess();
            if (flag) {
                // 业务成功处理           	
                log.info("success");//
            } else {
                // 失败
                log.info("error");
            }

        } catch (Exception e) {
            e.printStackTrace();          
           // System.out.println(e.getMessage());
        }
    }
}
