package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiVendorEnterpriseQueryRonganRequestV1;
import com.icbc.api.response.JftApiVendorEnterpriseQueryRonganResponseV1;

public class JftApiVendorEnterpriseQueryRonganTestV1 {

    //1、网关公钥
    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //2、appid
    protected static final String APP_ID = "10000000000000014048";
    //3、密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    public static void main(String[] args) throws Exception {

        //签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", MY_PRIVATE_KEY,"UTF-8",
                "json",APIGW_PUBLIC_KEY,"AES","5xGJdh7qb+B95SUoxDlatg==", "", "");

        JftApiVendorEnterpriseQueryRonganRequestV1 request
                = new JftApiVendorEnterpriseQueryRonganRequestV1();

        //4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("https://ip:port/api/jft/api/vendor/enterprise/info/query/rongan/V1");

        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        JftApiVendorEnterpriseQueryRonganRequestV1.JftApiVendorEnterpriseQueryRonganBizContent bizContent
                = new JftApiVendorEnterpriseQueryRonganRequestV1.JftApiVendorEnterpriseQueryRonganBizContent();
        bizContent.setAppId(APP_ID);
        bizContent.setVendorName("xxx");
        bizContent.setOutVendorId("1234345");
        bizContent.setCertType("100");
        bizContent.setCertNo("912101117507527734");
        request.setBizContent(bizContent);

        JftApiVendorEnterpriseQueryRonganResponseV1 response;
        try {
            response = (JftApiVendorEnterpriseQueryRonganResponseV1) client.execute(request, System.currentTimeMillis()+"");
            if (response.isSuccess()) {
                //6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
                System.out.println("ReturnCode:"+response.getReturnCode());
                System.out.println("response:" + JSONObject.toJSONString(response));
            } else {
                //失败
                System.out.println("ReturnCode:"+response.getReturnCode());
                System.out.println("ReturnMsg:"+response.getReturnMsg());
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
