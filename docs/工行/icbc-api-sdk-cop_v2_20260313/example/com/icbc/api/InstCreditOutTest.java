package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.TotalAmountQueryRequestV1;
import com.icbc.api.request.TotalAmountQueryRequestV1.TotalAmountQueryRequestV1Biz;
import com.icbc.api.response.TotalAmountQueryResponseV1;

public class InstCreditOutTest {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	protected static final String APP_ID = "10000000000000004964";


	public void test_cop() {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","eyiSiHvFqYFLu/AZDCt+4w==", "", "");
		TotalAmountQueryRequestV1 request = new TotalAmountQueryRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/api/vacct/inst/total/amount/query/V1");
		TotalAmountQueryRequestV1Biz bizContent = new TotalAmountQueryRequestV1Biz();
		bizContent.setAppId("10000000000000003952");//appId:外部商户编号
		bizContent.setVersion("1.0.0");//版本号
		request.setBizContent(bizContent);
		TotalAmountQueryResponseV1 response;
		try {
			response = (TotalAmountQueryResponseV1) client.execute(request, System.currentTimeMillis()+"");
		
		System.out.println(JSONObject.toJSONString(response));
			if (response.isSuccess()) {
				System.out.println(response.getReturnCode());
			} else {
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
}
