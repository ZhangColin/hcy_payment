package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.BusinessopElinkOrderBgstatusupdateRequestV1;
import com.icbc.api.request.BusinessopElinkOrderBgstatusupdateRequestV1.NoiceRequestV1;
import com.icbc.api.response.BusinessopElinkOrderBgstatusupdateResponseV1;

// 报关单状态更新
public class BusinessopElinkOrderBgstatusupdateTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
//	protected static final String APP_ID = "10000000000004114199";
	protected static final String APP_ID = "10000000000000155509";

	public static void main(String[] args) throws Exception {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8",
					"json",APIGW_PUBLIC_KEY,"AES","mQbJILokBccRHUkS+XBk7A==", "", "");
		BusinessopElinkOrderBgstatusupdateRequestV1 request = new BusinessopElinkOrderBgstatusupdateRequestV1();
//		request.setServiceUrl("http://ip:port/api/businessop/elink/order/bgstatusupdate/V1");
		request.setServiceUrl("http://ip:port/api/businessop/elink/order/bgstatusupdate/V1");
		NoiceRequestV1 biz = new NoiceRequestV1();
		biz.setOrderSn("OD201911068985193154");
		biz.setCustomsStatus("4");
		biz.setCustomsReform("退回原因");
		request.setBizContent(biz);
		BusinessopElinkOrderBgstatusupdateResponseV1 response;
		try {
			System.out.println("Request: " + JSONObject.toJSONString(biz));
			response =client.execute(request, System.currentTimeMillis() + "");
			System.out.println("Response: " + JSONObject.toJSONString(response));
			if (response.isSuccess()) {
				System.out.println("execute success");
			} else {
				System.out.println("execute failue");
			}
		} catch (IcbcApiException e) {
			System.out.println("execute exception" + e);
			e.printStackTrace();
		}

	}
}
