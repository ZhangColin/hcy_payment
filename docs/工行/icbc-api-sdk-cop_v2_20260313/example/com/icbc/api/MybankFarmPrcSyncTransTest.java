package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.MybankFarmPrcSyncProjectRequestV1;
import com.icbc.api.request.MybankFarmPrcSyncTransRequestV1;
import com.icbc.api.response.MybankFarmPrcSyncProjectResponseV1;
import com.icbc.api.response.MybankFarmPrcSyncTransResponseV1;

/**
 * 三农APP同步产权中心数据接口
 * 
 * 请商户配置SDK中的lib文件夹中的jar包后，根据备注1-6提示进行数据替换
 */
public class MybankFarmPrcSyncTransTest {
	         

	protected static String APP_ID = "10000000000004096993";
	

	private  static String BASE_URL = "http://ip:port/api";
	

	protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
     



	public static void main(String[] args) throws Exception {
		String URI = "mybank/account/farm/syncTrans/V1";
		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA2", PRI_KEY, APIGW_PUBLIC_KEY);
		try {
			MybankFarmPrcSyncTransRequestV1 request = new MybankFarmPrcSyncTransRequestV1();
			MybankFarmPrcSyncTransRequestV1.MybankFarmPrcSyncTransRequestBizV1 requestBizV1 = new MybankFarmPrcSyncTransRequestV1.MybankFarmPrcSyncTransRequestBizV1();
			requestBizV1.setMsgId("123456");
			requestBizV1.setfSeqNo("1234567890");
			requestBizV1.setCorpCode("icbc");
			requestBizV1.setParamType("1");
			requestBizV1.setTransDate("20220202");
			requestBizV1.setInstId("110110");
			requestBizV1.setDetail("{\"orderNo\":\"lzl1001667\",\"orderTitle\":\"订单标题测试30\",\"customerId\":\"user00101667\",\"orderAmt\":\"100\",\"orderStatus\":\"1\","
					+ "\"projectId\":\"026\",\"propertyId\":\"2024030200010403\",\"orderType\":\"1\",\"billNo\":\"lzl1221667\",\"billAmt\":\"50\",\"billType\":\"1\","
					+ "\"billStatus\":\"1\",\"transSummary\":\"摘要测试\",\"transBankMark\":\"测试交易\",\"transferDate\":\"20220202\",\"transferTime\":\"020123\"}"
					   );

			request.setServiceUrl(BASE_URL + "/" + URI);
			request.setBizContent(requestBizV1);
			
	        System.out.println(JSON.toJSONString(request));

			MybankFarmPrcSyncTransResponseV1 response = client.execute(request);
			if (response.isSuccess()) {
				System.out.println("success");
				System.out.println(JSONObject.toJSONString(response));
			} else {
				System.out.println("error");
				System.out.println(JSONObject.toJSONString(response));
			}
		} catch (Exception e) {
			e.printStackTrace();
		}

	}

	
}


