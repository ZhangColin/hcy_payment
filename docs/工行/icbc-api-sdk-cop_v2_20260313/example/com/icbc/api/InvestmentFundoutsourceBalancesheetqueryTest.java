...
import com.icbc.api.request.InvestmentFundoutsourceBalancesheetqueryRequestV1.InvestmentFundoutsourceBalancesheetqueryRequestV1Biz;
import com.icbc.api.request.InvestmentFundoutsourceBalancesheetqueryRequestV1.InvestmentFundoutsourceBalancesheetqueryRequestV1In;
import com.icbc.api.request.InvestmentFundoutsourceBalancesheetqueryRequestV1.InvestmentFundoutsourceBalancesheetqueryRequestV1InRecord;
import com.icbc.api.response.InvestmentFundoutsourceBalancesheetqueryResponseV1;


public class InvestmentFundoutsourceBalancesheetqueryTest {
	...
	public static void main(String[] args) throws Exception {
		String URI = "investment/fundoutsource/balancesheetquery/V1"; // api地址
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
		try {

			// 请求实体
			InvestmentFundoutsourceBalancesheetqueryRequestV1 request = new InvestmentFundoutsourceBalancesheetqueryRequestV1();
			// bizContent实体
			InvestmentFundoutsourceBalancesheetqueryRequestV1Biz bizContent = new InvestmentFundoutsourceBalancesheetqueryRequestV1Biz();
			// 请求报文IN节点实体
			InvestmentFundoutsourceBalancesheetqueryRequestV1In requestIn = new InvestmentFundoutsourceBalancesheetqueryRequestV1In();
			// Record实体
			InvestmentFundoutsourceBalancesheetqueryRequestV1InRecord instBean = new InvestmentFundoutsourceBalancesheetqueryRequestV1InRecord();

			// Record实体 设置属性
//			instBean.setReportKind("03");

			ArrayList<InvestmentFundoutsourceBalancesheetqueryRequestV1InRecord> list = new ArrayList<>();
			list.add(instBean);

			// 请求报文IN节点实体 设置属性
			requestIn.setFileType("4501");
			requestIn.setFundId("0101010");
			requestIn.setReportType("03");//
			requestIn.setBeginDate("20160101");
			requestIn.setEndDate("99999999");
			requestIn.setDeptCode("0213");
			requestIn.setCertId("");
			requestIn.setSerialNo("20181024000001067884");
			...
			// 输出支付指令提交响应对象，需根据实际调用服务更换（InvestmentFundoutsourceBalancesheetqueryResponseV1）
			InvestmentFundoutsourceBalancesheetqueryResponseV1 response = client.execute(request);
