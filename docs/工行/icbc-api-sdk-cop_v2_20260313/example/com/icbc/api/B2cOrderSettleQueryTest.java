package com.icbc.api;

import java.util.List;

import org.junit.Test;

import com.icbc.api.request.B2cOrderSettleQueryRequestV1;
import com.icbc.api.request.B2cOrderSettleQueryRequestV1.B2cOrderSettleQueryRequestV1Biz;
import com.icbc.api.request.B2cOrderSettleQueryRequestV1.B2cOrderSettleQueryRequestV1CommonInfo;
import com.icbc.api.request.B2cOrderSettleQueryRequestV1.B2cOrderSettleQueryRequestV1TradeInfo;
import com.icbc.api.response.B2cOrderSettleQueryResponseV1;
import com.icbc.api.response.B2cOrderSettleQueryResponseV1.B2cOrderSettleQueryResponseV1CommonRetInfo;
import com.icbc.api.response.B2cOrderSettleQueryResponseV1.B2cOrderSettleQueryResponseV1DetailRetInfo;
import com.icbc.api.response.B2cOrderSettleQueryResponseV1.B2cOrderSettleQueryResponseV1TradeRetInfo;

public class B2cOrderSettleQueryTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000002116";


	public void test_CA() throws IcbcApiException {
		
		String castr = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				"",
				APIGW_PUBLIC_KEY, castr, "12345678");
		
		B2cOrderSettleQueryRequestV1 request = new B2cOrderSettleQueryRequestV1();
		request.setServiceUrl("http://ip:port/api/b2c/order/V1/settle/query");

		B2cOrderSettleQueryRequestV1Biz bizContent = new B2cOrderSettleQueryRequestV1Biz();
		
		B2cOrderSettleQueryRequestV1CommonInfo commonInfo = new B2cOrderSettleQueryRequestV1CommonInfo();
		commonInfo.setTransCode("QEBUS");
		commonInfo.setCis("020090002992836");
		commonInfo.setBankCode("102");
		commonInfo.setId("thy.y.0200");
		commonInfo.setTranDate("20171231");
		commonInfo.setTranTime("204438000");
		commonInfo.setFseqno("98111314511403031941352");
		
		B2cOrderSettleQueryRequestV1TradeInfo tradeInfo = new B2cOrderSettleQueryRequestV1TradeInfo();
		tradeInfo.setShopType("2");
		tradeInfo.setShopCode("0200EC23497696");
		tradeInfo.setShopAcct("0200080909200100554");
		tradeInfo.setOrderType("1");
		tradeInfo.setQueryDate("20190201");
		
		bizContent.setCommonInfo(commonInfo);
		bizContent.setTradeInfo(tradeInfo);
		
		request.setBizContent(bizContent);

		B2cOrderSettleQueryResponseV1 response = client.execute(request, "msgId");

		if (response.isSuccess()) {
			// 接收成功
			B2cOrderSettleQueryResponseV1CommonRetInfo commonRetInfo = response.getCommonRetInfo();
			B2cOrderSettleQueryResponseV1TradeRetInfo tradeRetInfo = response.getTradeRetInfo();
			if(commonRetInfo.getRetCode().equals("0")){
				//查询成功 获取返回结果
				List<B2cOrderSettleQueryResponseV1DetailRetInfo> resultList = tradeRetInfo.getDetailRetInfo();
				//获取结果集中字段
				for(B2cOrderSettleQueryResponseV1DetailRetInfo detail:resultList){
					//获取信息.....
					detail.getTranAmt();
				}
			} else {
				//查询失败
				System.out.println(commonRetInfo.getRetMsg());
			}
		} else {
			// 失败
			System.out.println(response.getReturnMsg());
		}
	}
}
