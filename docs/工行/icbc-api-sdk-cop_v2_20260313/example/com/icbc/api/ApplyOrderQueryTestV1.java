package com.icbc.api;

import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.ApplyOrderQueryRequestV1;
import com.icbc.api.request.ApplyOrderQueryRequestV1.ApplyOrderQueryRequestV1Biz;
import com.icbc.api.response.ApplyOrderQueryResponseV1;

public class ApplyOrderQueryRequestTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000213532";

	public void test_cop() {

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", MY_PRIVATE_KEY, "UTF-8", "json",
				APIGW_PUBLIC_KEY, "AES", "G242nUlMmiT1tcToX29xkg==", "", "");

		ApplyOrderQueryRequestV1 request = new ApplyOrderQueryRequestV1();
		request.setServiceUrl("https://ip:port/api/jft/api/pay/customs/apply/V1");
		
		ApplyOrderQueryRequestV1Biz bizContent = new ApplyOrderQueryRequestV1Biz();
		bizContent.setAppId(APP_ID);//平台商户标识
        bizContent.setOutApplyNo("202004041003");//外部报关订单编号
		
		request.setBizContent(bizContent);

		ApplyOrderQueryResponseV1 response;

		try {
			response = client.execute(request, String.valueOf(System.currentTimeMillis()));//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一

			System.out.println(JSONObject.toJSONString(response));
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}

	}

	public static void main(String[] args) {
		new ApplyOrderQueryRequestTestV1().test_cop();
	}
}
