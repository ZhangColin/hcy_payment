package com.icbc.api;

import java.util.logging.Logger;

import com.icbc.api.request.MybankAccountWbscustomerPubaccountUseridentityqryRequestV1;
import com.icbc.api.request.MybankAccountWbscustomerPubaccountUseridentityqryRequestV1.MybankAccountWbscustomerPubaccountUseridentityqryRequestV1Biz;
import com.icbc.api.response.MybankAccountWbscustomerPubaccountUseridentityqryResponseV1;

public class MybankAccountWbscustomerPubaccountUseridentityqryTestV1 {
	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	// 2、appid 通行证第三方测试1 (行外)
	protected static final String APP_ID = "xxxxxxxxxxxxxxxxxxxx";

	// 3、密钥对认证方式，公钥在API平台登记，此处为私钥 行外访问
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	private static Logger logger = Logger.getLogger(MybankAccountWbscustomerPubaccountUseridentityqryTestV1.class.getName());

	/**
	 * @param args
	 */
	public static void main(String[] args) {
		// 签名类型为SHA256WithRSA时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA, MY_PRIVATE_KEY,
				APIGW_PUBLIC_KEY);

		MybankAccountWbscustomerPubaccountUseridentityqryRequestV1 request = new MybankAccountWbscustomerPubaccountUseridentityqryRequestV1();

		// 4、根据测试环境和生产环境替换相应ip和端口
		// 行外流程
		request.setServiceUrl("http://ip:port/api/wbs/customer/pubaccount/useridentityqry/V1");

		// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		MybankAccountWbscustomerPubaccountUseridentityqryRequestV1Biz bizContent = new MybankAccountWbscustomerPubaccountUseridentityqryRequestV1Biz();

		bizContent.setAppid("xxxxxxxxxxxxxxxxxxxx");
		bizContent.setParabcode("QDB");
		bizContent.setCode("0612JfUF1mQ4b60kB7VF14cXTF12JfUJ");

		request.setBizContent(bizContent);
		// 建立返回项
		MybankAccountWbscustomerPubaccountUseridentityqryResponseV1 response = new MybankAccountWbscustomerPubaccountUseridentityqryResponseV1();
		try {
			response = client.execute(request);
			logger.info("return_code:" + response.getReturn_code());
			logger.info("return_msg:" + response.getReturn_msg());
			logger.info("tranerrorcode:" + response.getTranerrorcode());
			logger.info("tranerrormsg:" + response.getTranerrormsg());
			logger.info("result:" + response.getResult());
		} catch (Exception e) {
			logger.info(e.getMessage());
		}
	}
}
