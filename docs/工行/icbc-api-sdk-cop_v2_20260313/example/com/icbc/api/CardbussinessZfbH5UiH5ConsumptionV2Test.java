package com.icbc.api;

import static org.junit.Assert.assertEquals;

import java.io.UnsupportedEncodingException;
import java.util.logging.Logger;

import com.alibaba.fastjson.JSON;
import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;

import org.junit.Test;

import com.icbc.api.request.CardbussinessZfbH5UiH5ConsumptionRequestV2;
import com.icbc.api.request.CardbussinessZfbH5UiH5ConsumptionRequestV2.CardbussinessZfbH5UiH5ConsumptionRequestV2Biz;

public class CardbussinessZfbH5UiH5ConsumptionV2Test {
	// 1、网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	// 2、密钥对认证方式，公钥在API平台登记，此处为私钥
	protected final static String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	// 3、appid
	protected static final String APP_ID = "xxx";
	
	private Logger logger = Logger.getLogger(CardbussinessZfbH5UiH5ConsumptionV2Test.class.getName());
	

	public void test_cop() throws IcbcApiException, UnsupportedEncodingException {
		
		//4、需传入appid，私钥，字符集
		UiIcbcClient client = new UiIcbcClient(APP_ID,MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
		CardbussinessZfbH5UiH5ConsumptionRequestV2 request = new CardbussinessZfbH5UiH5ConsumptionRequestV2();
		//5、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/ui/cardbussiness/zfbh5/ui/h5consumption/V2");
		CardbussinessZfbH5UiH5ConsumptionRequestV2Biz bizContent = new CardbussinessZfbH5UiH5ConsumptionRequestV2Biz();
		request.setBizContent(bizContent);
		//6、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		bizContent.setMer_id("xxx");
		bizContent.setMer_prtcl_no("xxx");
		bizContent.setOrder_id("xxx");
		bizContent.setOrder_date_time("xxx");
		bizContent.setAmount("xxx");
		bizContent.setCur_type("xx");
		bizContent.setBody("xxx");
		bizContent.setNotify_url("xxx");
		bizContent.setIcbc_appid("xxx");
		bizContent.setExpire_time("xxx");
		bizContent.setNotify_type("xxx");
		bizContent.setResult_type("xxx");
		bizContent.setPay_limit("xxx");
		bizContent.setReturn_url("xxx");
		bizContent.setQuit_url("xxx");
		bizContent.setOrder_apd_inf("xxx");
		bizContent.setDetail("xxx");
		bizContent.setAttach("xxxx");
		bizContent.setCustName("xxxx");
		bizContent.setCustCertType("xxxx");
		bizContent.setCustCertNo("xxxx");
		bizContent.setIsSuborder("1");
		bizContent.setSer_id("xxxx");
		bizContent.setOrder_num("x");
		JSONObject subOrder01 = JSON.parseObject("{\"xxx\":\"xx\",\"xxx\":\"xxx\"}");
		JSONArray subOrderArray = new JSONArray();
		subOrderArray.add(subOrder01);
		bizContent.setorder_submit_act_input(subOrderArray);
		request.setBizContent(bizContent);
		try {
			boolean testFlag = true;
			logger.info(client.buildPostForm(request));
			assertEquals(true, testFlag);
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
