package com.icbc.api;

import com.icbc.api.request.NfpwChainTradeInfoUploadRequestV1;

// 网络融资产业链贸易信息上传服务（页面）
public class MallNfpChainUiNfptradeinfouploadTestV1 {

	protected static final String APP_ID = "10000000000002354554";
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	public static void main(String[] args) throws Exception {
		MallNfpChainUiNfptradeinfouploadTestV1.upload();
	}

	public static String  upload() throws Exception{
		UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
		NfpwChainTradeInfoUploadRequestV1 request=new NfpwChainTradeInfoUploadRequestV1();
		// 设置请求路径
		request.setServiceUrl("http://ip:port/ui/mall/nfp/chain/ui/nfptradeinfoupload/V1");
		NfpwChainTradeInfoUploadRequestV1.NfpwChainTradeInfoUploadRequestV1Biz biz = new NfpwChainTradeInfoUploadRequestV1.NfpwChainTradeInfoUploadRequestV1Biz();
		biz.setChainId("100000000000000");
		biz.setConnectUserCid("100000000000000");
		biz.setCoreCid("100000000000000");
		biz.setCustomCid("100000000000000");
		biz.setLoginUserCid("100000000000000");
		biz.setProdType("01");
		//...
		request.setBizContent(biz);
		// 生成自提交的表单返回客户浏览器，该表单会自动提交完成调用
		String s = client.buildPostForm(request);
		System.out.println(s);
		return s;


	}
	
}

