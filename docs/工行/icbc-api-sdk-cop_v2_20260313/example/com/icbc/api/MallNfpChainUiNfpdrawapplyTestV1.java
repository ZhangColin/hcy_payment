package com.icbc.api;

import com.icbc.api.request.NfpwChainDrawapplyRequestV1;
import com.icbc.api.request.NfpwChainDrawapplyRequestV1.NfpwChainDrawapplyRequestV1Biz;

// 网络融资产业链提款申请服务（页面）
public class MallNfpChainUiNfpdrawapplyTestV1 {
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	protected static final String APP_ID = "10000000000000082009"; 
	
	protected static final String SERVICE_URL = "http://ip:port/ui/mall/nfp/chain/ui/nfpdrawapply/V1";
	
	public static void main(String[] args) {
		String buildPostForm = "";
		UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8);
		NfpwChainDrawapplyRequestV1 request = new NfpwChainDrawapplyRequestV1();
		request.setServiceUrl(SERVICE_URL);
		NfpwChainDrawapplyRequestV1Biz bizContent = new NfpwChainDrawapplyRequestV1Biz();
		bizContent.setAppid(APP_ID);
		bizContent.setOutVendorId("10001068");
		bizContent.setOutCoreVendorId("10001006");
		bizContent.setOutCustomVendorId("10001068");
		bizContent.setTradeId("201908190846003");
		bizContent.setDrawAmount("20000");
		bizContent.setDrawAccount("020022790902017938");
		bizContent.setDrawDueDate("20191030");
		request.setBizContent(bizContent);
		try {
			buildPostForm = client.buildPostForm(request);
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
		System.out.println(buildPostForm);
	}
}
