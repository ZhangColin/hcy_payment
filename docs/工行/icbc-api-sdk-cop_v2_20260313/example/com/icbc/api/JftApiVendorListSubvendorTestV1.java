...
import com.icbc.api.request.JftApiVendorListSubvendorRequestV1;
import com.icbc.api.request.JftApiVendorListSubvendorRequestV1.JftApiVendorListSubvendorRequestV1Biz;
import com.icbc.api.response.JftApiVendorInfoQueryResponseV1;
import com.icbc.api.response.JftApiVendorListSubvendorResponseV1;
 ...
 */
public class JftApiVendorListSubvendorTestV1 {
	...
	public static void main(String[] args) throws Exception {
		...
		JftApiVendorListSubvendorRequestV1 request = new JftApiVendorListSubvendorRequestV1();
		
		//4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("https://ip:port/api/jft/api/vendor/list/subvendor/V1");
		
		//5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		JftApiVendorListSubvendorRequestV1Biz bizContent = new JftApiVendorListSubvendorRequestV1Biz();
		bizContent.setAppId(APP_ID);
		bizContent.setPageNo("1");
		bizContent.setPageCount("10");
		bizContent.setVendorStatus("2");
		bizContent.setAuditStatus("04");
		request.setBizContent(bizContent);

		JftApiVendorListSubvendorResponseV1 response;
		try {
			response = (JftApiVendorListSubvendorResponseV1) client.execute(request, System.currentTimeMillis()+"");
