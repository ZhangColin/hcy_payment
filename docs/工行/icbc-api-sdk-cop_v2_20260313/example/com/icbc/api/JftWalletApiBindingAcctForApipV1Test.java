package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftWalletApiBindingAcctForApipRequestV1;
import com.icbc.api.request.JftWalletApiBindingAcctForApipRequestV1.JftWalletApiBindingAcctForApipRequestV1Biz;
import com.icbc.api.response.JftWalletApiBindingAcctForApipReponseV1;


/**
 * 二类户绑定发送短信
 *
 * @author kfzx-changcx
 *
 */
public class JftWalletApiBindingAcctForApipV1Test {

	// 公钥（根据环境变化）
String MY_PRIVATE_KEY2 = "-----请使用正确的APP私钥-----";
//私钥（根据环境变化）
String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";


	protected static final String APP_ID = "10000000000000008062";// (根据实际情况确定appid)



	// 绑定发送短信

	public void test_cop() {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA",
				MY_PRIVATE_KEY2, "UTF-8", "json", APIGW_PUBLIC_KEY, "AES",
				"i5N/TQHG9hq00OpdNzpUUw==", "", "");
		JftWalletApiBindingAcctForApipRequestV1 request = new JftWalletApiBindingAcctForApipRequestV1();

		request.setServiceUrl("http://ip:port/api/jft/wallet/api/bindingacctforapip/V1");//根据实际情况确定url和端口

		JftWalletApiBindingAcctForApipRequestV1Biz biz = new JftWalletApiBindingAcctForApipRequestV1Biz();

		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				biz.setAppId("10000000000000012090");
				biz.setTrxAccDate("2018-10-09");
				biz.setTrxAccTime("15:47:00");
				biz.setCorpDate("2017-03-15");
				biz.setCorpSerno("321654987");
				biz.setOutServiceCode("binding");
				biz.setCropCisNo("77777");
				biz.setCorpMediumId("6222020200123701928");
				biz.setBindMedium("6222020200123701928");
				biz.setMediumId("120180712000538311");
				biz.setCertType(0);
				biz.setCertNo("110105198101021118");
				biz.setCustName("武则天");
				biz.setMobileNo("18813312345");
				biz.setNotifyAddr("");
				biz.setUserProFlag("0");

				//风险特征值
				biz.setCustIp("82.32.02.33");
				biz.setCustMac("1121212");
				biz.setDeviceId("82.39.02.33");
				biz.setBrowserLang("");
				biz.setBrowserVersion("");
				biz.setChannelType("1");//1-app,CustIp、CustMac和DeviceId必输；2-非app
				biz.setCvn2("");
				biz.setExpired("");
				biz.setFactoryInfo("");
				biz.setMemoryCapacity("");
				biz.setScreenResolution("");
				biz.setSystemVersion("");
				biz.setCpuId("");

				// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
				System.out.println(biz);
				request.setBizContent(biz);

				JftWalletApiBindingAcctForApipReponseV1 response;

				try {

					response = (JftWalletApiBindingAcctForApipReponseV1) client.execute(request, System.currentTimeMillis() + "");
					System.out.println("~~~~~~~~~");
					System.out.println(JSONObject.toJSONString(response));
					System.out.println("~~~~~~~~~");
					if (response.isSuccess()) {
						// 业务成功处理
						System.out.println(response.getReturnCode());
					} else {
						// 失败
					}

				} catch (IcbcApiException e) {

					e.printStackTrace();

				}
			}

			public static void main(String[] args) {
				new JftWalletApiBindingAcctForApipV1Test().test_cop();
			}
		}
