package com.icbc.api.test;


import com.alibaba.fastjson.JSONObject;
import com.icbc.api.DefaultIcbcClient;
import com.icbc.api.IcbcApiException;
import com.icbc.api.request.MybankCreditcardCardapplyCardapplylistqryRequestV1;
import com.icbc.api.request.MybankCreditcardCardapplyCardapplylistqryRequestV1.MybankCreditcardCardapplyCardapplylistqryRequestBizV1;
import com.icbc.api.response.MybankCreditcardCardapplyCardapplylistqryReponseV1;



public  class MybankCreditcardCardapplyCardapplylistqryTestV1 {


	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	//protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //重新生成
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----" ;
	//protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的APP私钥-----BIjANBgkqhkiG9w0BAQEFAAOCAQ8A-----请使用正确的APP私钥-----";

	//APP_ID：工行API接口的ID
	protected static final String APP_ID = "xxxxxxxxxxxx"; 
	
	public static void main(String[] args) throws Exception{
		MybankCreditcardCardapplyCardapplylistqryTestV1 a=new MybankCreditcardCardapplyCardapplylistqryTestV1();
		a.test_cop();
	}

	
	
	public void test_cop() throws IcbcApiException {
		MybankCreditcardCardapplyCardapplylistqryRequestV1 request = new MybankCreditcardCardapplyCardapplylistqryRequestV1();
		request.setServiceUrl("http://ip:port/api/mybank/creditcard/cardapply/cardapplylistqry/V1");
		MybankCreditcardCardapplyCardapplylistqryRequestBizV1 bizContent = new MybankCreditcardCardapplyCardapplylistqryRequestBizV1();
		bizContent.setName("xxxx");  //中文名
		bizContent.setCity_num("xxxx");  //开户地区号
		bizContent.setId_card_no("xxxxx");  //证件号码
		bizContent.setId_card_type("0");  //证件类型
		bizContent.setApply_id("XXXXXXX");  //流水号
		bizContent.setFirm_num("XXXX");  //公司代码
		bizContent.setChannel("XXXX");  //渠道代码
		bizContent.setPage_num("XXXX");  //页面位置代码
		bizContent.setCard_logo("XXXX"); //营销档案编号
		request.setBizContent(bizContent);
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		MybankCreditcardCardapplyCardapplylistqryReponseV1 response ;
		try{
			response = client.execute(request);
			if (response.isSuccess()) {// 业务成功处理
				//住宅地址
				response.getHomeAddr();
				//。。。。
			} else {// 失败
				;
			}
		}catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}

																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																																														
	
}	
