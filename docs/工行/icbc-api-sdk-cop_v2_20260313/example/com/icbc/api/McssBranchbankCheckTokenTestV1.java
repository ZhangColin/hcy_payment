...
import com.icbc.api.request.McssBranchbankCheckTokenRequestV1;
import com.icbc.api.request.McssBranchbankCheckTokenRequestV1.McssBranchbankCheckTokenRequestV1Biz;
import com.icbc.api.response.McssBranchbankCheckTokenResponseV1;

import java.util.logging.Logger;

public class McssBranchbankCheckTokenTestV1 {
    static Logger log = Logger.getLogger(McssBranchbankCheckTokenTestV1.class.getName());
    ...
    public static void main(String[] args) throws Exception {
        ...
        McssBranchbankCheckTokenRequestV1 request = new McssBranchbankCheckTokenRequestV1();

        //4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://ip:port/api/saes/servmer/orderthaw/V1");
        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        McssBranchbankCheckTokenRequestV1Biz bizContent = new McssBranchbankCheckTokenRequestV1Biz();

        bizContent.setUserId("020057018890");//用户编号
        bizContent.setToken("72385888c61142678fa894d1028889cc");//token

        request.setBizContent(bizContent);
        McssBranchbankCheckTokenResponseV1 response;
        try {
            response = client.execute(request, "23344544454440");//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一

            log.info("McssBranchbankCheckToken response:" + response);
            if (response.isSuccess()) {
                // 6、业务成功处理，请根据接口文档用response.getxxx()获取同步返回的业务数据
                log.info("McssBranchbankCheckToken returnCode:" + Integer.toString(response.getReturnCode()));
            } else {
                // 失败
                log.info("McssBranchbankCheckToken returnCode:" + Integer.toString(response.getReturnCode()));
                log.info("McssBranchbankCheckToken returnMsg:" + response.getReturnMsg());
            }
        } catch (IcbcApiException e) {
            log.info("McssBranchbankCheckToken IcbcApiException:" + e);
        }
    }
}
