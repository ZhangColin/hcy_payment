...
import com.icbc.api.request.CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1;
import com.icbc.api.request.CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1.CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1Biz;
import com.icbc.api.response.CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesResponseV1;

public class CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesV1Test {
	// 1、网关公钥
	...
	private Logger logger = Logger.getLogger(CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesV1Test.class.getName());
	
	
	public void test_cop() throws IcbcApiException {
		
		DefaultIcbcClient client = null;
		CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1 request = new CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1();
		...
		if(request.isNeedEncrypt()){//若需要进行AES解密
			...
			client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY,
					IcbcConstants.CHARSET_UTF8, IcbcConstants.FORMAT_JSON, APIGW_PUBLIC_KEY,
					IcbcConstants.ENCRYPT_TYPE_AES, encryptKey, null, null);
		}else{
			//4、签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
			client = new DefaultIcbcClient(APP_ID,IcbcConstants.SIGN_TYPE_RSA2,MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		}
		//5、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://ip:port/api/cardbusiness/aggregatepay/b2c/online/foreignpaywithoutpages/V1");
		CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1Biz bizContent = new CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesRequestV1Biz();
		...
		CardbusinessAggregatepayB2cOnlineForeignpaywithoutpagesResponseV1 response;
		try {
