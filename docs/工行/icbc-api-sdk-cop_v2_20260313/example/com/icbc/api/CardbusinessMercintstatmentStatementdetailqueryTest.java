package com.icbc.api;

import java.util.List;

import com.icbc.api.request.CardbusinessMercintstatmentStatementdetailqueryRequestV1;
import com.icbc.api.request.CardbusinessMercintstatmentStatementdetailqueryRequestV1.CardbusinessMercintstatmentStatementdetailqueryRequestV1Biz;
import com.icbc.api.request.CardbusinessMercintstatmentStatementdetailqueryRequestV1.CardbusinessMercintstatmentStatementdetailqueryRequestV1Pub;
import com.icbc.api.request.CardbusinessMercintstatmentStatementdetailqueryRequestV1.CardbusinessMercintstatmentStatementdetailqueryRequestV1In;
import com.icbc.api.response.CardbusinessMercintstatmentStatementdetailqueryResponseV1;
import com.icbc.api.response.CardbusinessMercintstatmentStatementdetailqueryResponseV1.CardbusinessMercintstatmentStatementdetailqueryResponseV1Pub;
import com.icbc.api.response.CardbusinessMercintstatmentStatementdetailqueryResponseV1.CardbusinessMercintstatmentStatementdetailqueryResponseV1AppStatV10;
import com.icbc.api.response.CardbusinessMercintstatmentStatementdetailqueryResponseV1.CardbusinessMercintstatmentStatementdetailqueryResponseV1Out;
import com.icbc.api.response.CardbusinessMercintstatmentStatementdetailqueryResponseV1.CardbusinessMercintstatmentStatementdetailqueryResponseV1Total;
import com.icbc.api.response.CardbusinessMercintstatmentStatementdetailqueryResponseV1.CardbusinessMercintstatmentStatementdetailqueryResponseV1Detail;

import java.util.logging.Logger;

public class CardbusinessMercintstatmentStatementdetailqueryTest {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

//	protected static final String APP_ID = "10000000000000002116";

	public static void main(String[] args) throws IcbcApiException {
//		String APP_ID = "10000000000000002116";
		String APP_ID = args[0];

		String castr = "-----请使用正确的APP私钥-----";

		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,
				"",
				APIGW_PUBLIC_KEY, castr, "12345678");
		
		CardbusinessMercintstatmentStatementdetailqueryRequestV1 request = new CardbusinessMercintstatmentStatementdetailqueryRequestV1();
		request.setServiceUrl("http://gw.dccnet.com.cn:8088/api/cardbusiness/mercintstatment/statementdetailquery/V1");

		CardbusinessMercintstatmentStatementdetailqueryRequestV1Biz bizContent = new CardbusinessMercintstatmentStatementdetailqueryRequestV1Biz();
		
		CardbusinessMercintstatmentStatementdetailqueryRequestV1Pub pub = new CardbusinessMercintstatmentStatementdetailqueryRequestV1Pub();
		pub.setCode("29133");
		pub.setZone("0200");
		pub.setFr("0");
		pub.setDt("2018-12-26");
		
		CardbusinessMercintstatmentStatementdetailqueryRequestV1In in = new CardbusinessMercintstatmentStatementdetailqueryRequestV1In();
		in.setZoneno("0200");
		in.setMerno("123412413131000");
		in.setAgreeno("");
		in.setDateb("2017-07-06");
		in.setDatee("2017-07-06");
		in.setQuerytype("1");
		in.setQuerycount("1");
		in.setPage("1");
		in.setSeqno("");
		in.setSadlindate("");
		
		bizContent.setPub(pub);
		bizContent.setIn(in);
		
		request.setBizContent(bizContent);

		CardbusinessMercintstatmentStatementdetailqueryResponseV1 response = client.execute(request, "msgId");
		Logger log = Logger.getLogger(CardbusinessMercintstatmentStatementdetailqueryTest.class.getName());

		if (response.isSuccess()) {
			// 接收成功
//			CardbusinessMercintstatmentStatementdetailqueryResponseV1Pub RetPub = response.getPub();
			CardbusinessMercintstatmentStatementdetailqueryResponseV1AppStatV10 RetAppStatV10 = response.getAppStatV10();
			CardbusinessMercintstatmentStatementdetailqueryResponseV1Out RetOut = response.getOut();
			if(RetAppStatV10.getReturnCode().equals("00000000")){
				//查询成功 获取返回结果
				List<CardbusinessMercintstatmentStatementdetailqueryResponseV1Detail> resultList = RetOut.getDetail();
				//获取结果集中字段
				for(CardbusinessMercintstatmentStatementdetailqueryResponseV1Detail detail:resultList){
					//获取信息.....
					detail.getSadlindate();
				}
			} else {
				//查询失败
				log.info(RetAppStatV10.getReturnCode());

			}
		} else {
			// 失败
			log.info(response.getReturnMsg());
		}
	}
}
