package com.icbc.api;


import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.CardbusinessEparkOrdercreateRequestV1;
import com.icbc.api.request.CardbusinessEparkOrdercreateRequestV1.CardbusinessEparkOrdercreateRequestV1Biz;
import com.icbc.api.request.CardbusinessEparkOrdercreateRequestV1.CardbusinessEparkOrdercreateRequestV1PrivateInfo;
import com.icbc.api.response.CardbusinessEparkOrdercreateResponseV1;
import com.icbc.api.response.CardbusinessEparkOrdercreateResponseV1.CardbusinessEparkOrdercreateResponseV1Data;

/**
 * @author kfzx-gucy
 *
 */
public class CardbusinessEparkOrdercreateTestV1 {

	public  static void main(String[] args) throws IcbcApiException{
		
		//1、网关公钥
//		String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
		String APIGW_PUBLIC_KEY = "XXXXXX";
		
		//2、appid
//		String APP_ID = "10000000000004096931";//10000000000004097784
		String APP_ID = "XXXXXX";
		

		//3、密钥对认证方式，公钥在API平台登记，此处为私钥
//        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        String MY_PRIVATE_KEY = "XXXXXX";
        //签名类型为RSA2时，需传入appid，私钥和网关公钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);

        CardbusinessEparkOrdercreateRequestV1 request = new CardbusinessEparkOrdercreateRequestV1();
       
        //4、根据测试环境和生产环境替换相应ip和端口 http://ip:port/api
//        request.setServiceUrl("http://ip:port/api/cardbusiness/epark/ordercreate/V1");
        request.setServiceUrl("http://XXXXXX:XX/api/cardbusiness/epark/ordercreate/V1");
        
        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        CardbusinessEparkOrdercreateRequestV1Biz bizContent = new CardbusinessEparkOrdercreateRequestV1Biz();
        CardbusinessEparkOrdercreateRequestV1PrivateInfo privateInfo=new CardbusinessEparkOrdercreateRequestV1PrivateInfo();

        String depotId = "znpark0000000016";
        String startTime = "2020-05-29 13:43:44";
        String plateNumber = "粤A00239";
        String carType = "1";
//        String actualPay = "1.50";
        privateInfo.setDepotId(depotId);
        privateInfo.setStartTime(startTime);
        privateInfo.setCarType(carType);
        privateInfo.setPlateNumber(plateNumber);
        bizContent.setPrivateInfo(privateInfo);
        bizContent.setPrivateInfo( privateInfo);
        
        request.setBizContent(bizContent);
        CardbusinessEparkOrdercreateResponseV1 response;
        
        String msgId = String.valueOf(System.currentTimeMillis());
        try {
			response = client.execute(request, msgId);//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一
//			if(response.isSuccess()) {
//				CardbusinessEparkOrdercreateResponseV1Data data=response.getData();
//				System.out.println("data:"+ JSON.toJSONString(response.getData()));
//
//			}
//			System.out.println("ReturnCode:"+response.getReturnCode());
//			System.out.println("ReturnMsg:"+response.getReturnMsg());
			
		    }catch (IcbcApiException e) {
			       e.printStackTrace();
		    }
		
	}
}
