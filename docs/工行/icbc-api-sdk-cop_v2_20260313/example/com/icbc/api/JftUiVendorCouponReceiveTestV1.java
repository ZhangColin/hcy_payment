package com.icbc.api.test;

import com.icbc.api.IcbcApiException;
import com.icbc.api.IcbcConstants;
import com.icbc.api.UiIcbcClient;
import com.icbc.api.request.JftUiVendorCouponReceiveRequestV1;
import com.icbc.api.request.JftUiVendorCouponReceiveRequestV1.JftUiVendorCouponReceiveRequestV1Biz;
import javax.servlet.ServletException;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.io.PrintWriter;


public class JftUiVendorCouponReceiveTestV1 extends HttpServlet {


	@Override
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		doPost(req, resp);
	}

	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
			throws ServletException, IOException {
		try {
			//商户编号
			String APP_ID = "10000000000000003923";
			String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

			UiIcbcClient client = new UiIcbcClient(APP_ID, MY_PRIVATE_KEY,
					IcbcConstants.CHARSET_UTF8);

			JftUiVendorCouponReceiveRequestV1 request = new JftUiVendorCouponReceiveRequestV1();
			
			request.setServiceUrl("https://ip:port/ui/jft/ui/vendor/coupon/receive/V1");

			JftUiVendorCouponReceiveRequestV1Biz bizContent = new JftUiVendorCouponReceiveRequestV1Biz();
			bizContent.setAppId("Oikeclo001");
			bizContent.setOutUserId("USER021008000001");
			bizContent.setMobile("13455556666");
			request.setBizContent(bizContent);

			resp.setHeader("Content-Type", "text/html;charset="
					+ IcbcConstants.CHARSET_UTF8);
			PrintWriter out = resp.getWriter();
			System.out.println(client.buildPostForm(request));

			out.write("<html>");
			out.write("<head>");
			out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset="
					+ IcbcConstants.CHARSET_UTF8 + "\">");
			out.write("</head>");
			out.write("<body>");
			out.write(client.buildPostForm(request));
			out.write("</body>");
			out.write("</html>");

		} catch (IcbcApiException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}

	}

}
