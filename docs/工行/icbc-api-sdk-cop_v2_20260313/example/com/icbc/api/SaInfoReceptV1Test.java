...
import com.icbc.api.request.SaInfoReceptRequestV1;
import com.icbc.api.request.SaInfoReceptRequestV1.SaInfoReceptRequestV1Biz;
import com.icbc.api.response.SaInfoReceptResponseV1;

/**
 * @author kfzx-linfeng
 */
public class SaInfoReceptV1Test {
	// 1、网关公钥
	...
	public static void main(String[] args) throws Exception {
		...
		SaInfoReceptRequestV1 request = new SaInfoReceptRequestV1();

		// 4、根据测试环境和生产环境替换相应ip和端口
		request.setServiceUrl("http://ip:port/api/saes/servmer/ordersubmit/V1");
		// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		SaInfoReceptRequestV1Biz bizContent = new SaInfoReceptRequestV1Biz();
		bizContent.setCorpId("0000000001");
		bizContent.setSaTrxId("0000000001");
		bizContent.setSaId("0000000001");
		bizContent.setSaName("xx基金会");
		bizContent.setSaType((byte)1);
		bizContent.setCvBusType((byte)0);
		bizContent.setLegalPersonName("张三");
		bizContent.setContactor("李四");
		bizContent.setContactNum("159xxxx6540");
		bizContent.setRegAmount(10000L);//此值为100元，以分为单位
		bizContent.setBrName("贵州分行");
		bizContent.setZoneId(2401);
		bizContent.setBrchId(1);
		bizContent.setCallbackUrl("www.xxxx.com");
		bizContent.setAfId("0000000001");
		bizContent.setDeptId("0000000001");

		request.setBizContent(bizContent);

		SaInfoReceptResponseV1 response;
		try {
