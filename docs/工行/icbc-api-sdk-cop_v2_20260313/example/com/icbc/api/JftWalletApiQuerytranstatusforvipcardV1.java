package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftWalletApiVIPCardquerytranstatusforvipcardRequestV1;
import com.icbc.api.request.JftWalletApiVIPCardquerytranstatusforvipcardRequestV1.JftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz;
import com.icbc.api.response.JftWalletApiVIPCardquerytranstatusforvipcardResponseV1;

public class JftWalletApiQuerytranstatusforvipcardV1 {

	String MY_PRIVATE_KEY2 = "-----请使用正确的APP私钥-----";
	String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJ h0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

	protected static final String APP_ID = "10000000000000012145";

	// 聚富通钱包会员卡交易状态查询
	
	public void test_query_status() {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA", MY_PRIVATE_KEY2, "UTF-8", "json",
				APIGW_PUBLIC_KEY, "AES", "5xGJdh7qb+B95SUoxDlatg==", "", "");
		// DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA",
		// MY_PRIVATE_KEY2, "UTF-8", "json",APIGW_PUBLIC_KEY, "AES",
		// "zl+5SxQAx6P7bjpqRu693Q==", "", "");
		JftWalletApiVIPCardquerytranstatusforvipcardRequestV1 request = new JftWalletApiVIPCardquerytranstatusforvipcardRequestV1();

		request.setServiceUrl("https://ip:port/api/jft/wallet/api/querytranstatusforvipcard/V1");

		JftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz = new JftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz();

		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

		jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz.setCorpDate("2018-11-16");
		jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz.setTrxAccDate("2018-11-16");
		jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz.setTrxAccTime("15:47:00");

		jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz.setAppId(APP_ID);
		jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz.setOrigCorpSerno("1JFT2018082200asd000001");// 流水号
		jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz.setTranType("2");// 类型
		// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

		System.out.println(jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz);
		request.setBizContent(jftWalletApiVIPCardquerytranstatusforvipcardRequestV1Biz);

		JftWalletApiVIPCardquerytranstatusforvipcardResponseV1 response;

		try {

			response = (JftWalletApiVIPCardquerytranstatusforvipcardResponseV1) client.execute(request,
					System.currentTimeMillis() + "");
			System.out.println("~~~~~~~~~");
			System.out.println(JSONObject.toJSONString(response));
			System.out.println("~~~~~~~~~");
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}

		} catch (IcbcApiException e) {

			e.printStackTrace();

		}
	}
	/************************************************************* 会员卡交易状态查询结束 **********************************************************************/

}
