package com.icbc.api;

import com.icbc.api.request.ThirdpartyOrderPayRequestV5;
import com.icbc.api.request.ThirdpartyOrderPayRequestV5.ThirdpartyOrderPayRequestV5Biz;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.PrintWriter;
import java.util.logging.Logger;

/**
 * 第三方买单权益星点值对换支付接口
 *
 * 请商户配置SDK中的lib文件夹中的jar包后，根据备注1-5提示进行数据替换
 */
public class ThirdpartyStarOrderPayTestV1 {

    static Logger LOGGER = Logger.getLogger(ThirdpartyStarOrderPayTestV1.class.getName());

    //1、appid
    protected static final String APP_ID = "10000000000000002156";

    //2、密钥对认证方式，公钥在API平台登记，此处为私钥
    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    //3、AES密钥
    protected static final String MY_AES_KEY = "TKhSiK9Blwr6+aCq+O0MFg==";

    public void order(HttpServletRequest request1, HttpServletResponse response) throws Exception {

        //签名类型为RSA2时，需传入appid，私钥和AES密钥，签名类型使用定值IcbcConstants.SIGN_TYPE_RSA2，其他参数使用缺省值
        UiIcbcClient client = new UiIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8, IcbcConstants.ENCRYPT_TYPE_AES, MY_AES_KEY);

        ThirdpartyStarOrderPayRequestV1 request = new ThirdpartyStarOrderPayRequestV1();

        //4、根据测试环境和生产环境替换相应ip和端口
        request.setServiceUrl("http://ip:port/ui/thirdparty/star/order/pay/V1");

        //5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
        ThirdpartyStarOrderPayRequestV1Biz bizContent = new ThirdpartyStarOrderPayRequestV1Biz();
        bizContent.setMerId("020002040095");
        bizContent.setStoreCode("02000015087");
        bizContent.setCustId("9hvyM+yzA9Mbx+PrvkRNGUxFF9nT2IWJ");
        bizContent.setOutTradeNo("ZHL777O15002039");
        bizContent.setOrderAmt("7370");
        bizContent.setTradeDate("20170112");
        bizContent.setTradeTime("160346");
        bizContent.setAttach("接口测试20170601");// 该字段非必输项
        bizContent.setPayExpire("1200");
        bizContent.setNotifyUrl("ip:port");// 该字段非必输项
        bizContent.setNotifyFlag("1"); //  NotifyFlag = 1  NotifyUrl 必输
        bizContent.setAutoSubmitFlag("1");//V1 固定上送 1 自动提交
        bizContent.setGoodsName("商品001");// 该字段非必输项
        bizContent.setAllPointsFlag("1");// V1 固定上送1
        bizContent.setGoodPoints("23");
        bizContent.setStarType("1");
        bizContent.setStarPoints("3");//StarType=2 当前字段必输
        bizContent.setPbOrderId("pbms1234567890");
        bizContent.setPbIssueTime("2024-01-01 12:00:00");//PBMS权益发放时间，三方调PBMS的专属权益发放API服务生成
        bizContent.setPbInfo("{\"param\":\"其他pbms信息\",\"chargePhone\":\"13891919191\"}");// 该字段非必输项

        request.setBizContent(bizContent);

        LOGGER.info(client.buildPostForm(request,"msgId"));//msgId消息通讯唯一编号，要求每次调用独立生成，APP级唯一

        response.setHeader("Content-Type", "text/html; charset=utf-8");
        PrintWriter out = response.getWriter();
        try {
            out.write("<html>");
            out.write("<head>");
            out.write("<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8 \">");
            out.write("</head>");
            out.write("<body>");
            out.write(client.buildPostForm(request,"msgId"));
            out.write("</body>");
            out.write("</html>");
        } catch (IcbcApiException e) {
            LOGGER.info("IcbcApiException:" + e);
        } finally {
            out.flush();
            out.close();
        }
    }

}
