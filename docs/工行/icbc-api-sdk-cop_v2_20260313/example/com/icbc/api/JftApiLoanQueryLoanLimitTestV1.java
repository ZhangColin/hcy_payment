package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiLoanQueryLoanLimitRequestV1;
import com.icbc.api.request.JftApiLoanQueryLoanLimitRequestV1.JftApiLoanQueryLoanLimitRequestV1Biz;
import com.icbc.api.response.JftApiLoanQueryLoanLimitResponseV1;
import org.junit.Test;

public class JftApiLoanQueryLoanLimitTestV1 {

    protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

    protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

    protected static final String APP_ID = "10000000000000108514";

    @SuppressWarnings("unchecked")

    public void test_cop() {

        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","eyiSiHvFqYFLu/AZDCt+4w==", "", "");
        JftApiLoanQueryLoanLimitRequestV1 request = new JftApiLoanQueryLoanLimitRequestV1();
        request.setServiceUrl("http://gateway-cfjs-app.sdc.cs.icbc:9080/api/jft/api/loan/qryloanlimit/V1");
        JftApiLoanQueryLoanLimitRequestV1Biz bizContent = new JftApiLoanQueryLoanLimitRequestV1Biz();
        bizContent.setAppId("10000000000000108514");
        bizContent.setOutSubVendorId("21543653456546");
        bizContent.setOutUserId("523465436546");
        bizContent.setLoanType("0");
        bizContent.setVersion("1.0.0");//设置接口访问版本号

        request.setBizContent(bizContent);
        JftApiLoanQueryLoanLimitResponseV1 response;
        try {
            response = (JftApiLoanQueryLoanLimitResponseV1) client.execute(request, System.currentTimeMillis()+"");

            System.out.println(JSONObject.toJSONString(response));
            if (response.isSuccess()) {
                // 业务成功处理
                System.out.println(response.getReturnCode());
            } else {
                // 失败
            }
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
    }
}
