package com.icbc.api;

import java.util.logging.Logger;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request. MybankLoanHousefundsupHfsrestamountRequestV1;
import com.icbc.api.request. MybankLoanHousefundsupHfsrestamountRequestV1. MybankLoanHousefundsupHfsrestamountRequestV1Biz;
import com.icbc.api.response. MybankLoanHousefundsupHfsrestamountResponseV1;

public class MybankLoanHousefundsupHfsrestamountV1Test {
    static Logger log = Logger.getLogger(MybankLoanHousefundsupHfsrestamountV1Test.class.getName());
    //估值表
    protected static String APP_ID = "1000"+"000000"+"00002"+"89005";

    //网关公钥
    protected static String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
    //应用私钥（与内管平台注册app时的“签名公钥”配对）
    protected static String PRI_KEY = "-----请使用正确的APP私钥-----";
    public static void main(String[] args) throws Exception {
        //  String URI = "investment/insurance/prodapply/applycheck/V1"; // api地址
       // DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2",PRI_KEY, APIGW_PUBLIC_KEY);
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, "RSA2", PRI_KEY, "UTF-8", "json", APIGW_PUBLIC_KEY, null, (String)null, (String)null, (String)null);               
        
        try {

            // 请求实体
        	 MybankLoanHousefundsupHfsrestamountRequestV1 request = new  MybankLoanHousefundsupHfsrestamountRequestV1();
            // bizContent实体
        	 MybankLoanHousefundsupHfsrestamountRequestV1Biz bizContent  = new  MybankLoanHousefundsupHfsrestamountRequestV1Biz();

    		bizContent.setSerialNo("xxxxxxxxxx");
        	bizContent.setAppNo("AppNo");  
        	bizContent.setVer("2.0");
        	bizContent.setLanguage("zh_CN");
        	bizContent.setAreaCode("002000072");
        	bizContent.setEmployeeCode("888800135");
        	bizContent.setTransNo("123456");
        	bizContent.setTurnPageFlag("");
        	bizContent.setBeginRow("");
        	bizContent.setRowCount("");
            bizContent.setNotify_url("http://baidu.com");
            bizContent.setCooperSerial("31245666");
            bizContent.setCooperCode("666666");
            bizContent.setAlterReason("测试类");
            bizContent.setCnlType("405");
            bizContent.setOrderNo("11111111");
            bizContent.setBankNo("325555");
            bizContent.setCooperPK("123456");
            
            //bizContent.setParam(param);                                                        
            //request.setServiceUrl(BASE_URL  + URI);           
            String url = "http"+"://8"+"3.33"+".222."+"62:80"+"81/api/mybank/loa"+"n/housefundsup/hfsrestamount/V1"; 
            
            request.setServiceUrl(url);
            request.setBizContent(bizContent);
             MybankLoanHousefundsupHfsrestamountResponseV1 response = client.execute(request);
            log.info("response:" + JSON.toJSONString(response));
            boolean flag=response.isSuccess();
            if (flag) {
                // 业务成功处理
                log.info("success");//
            } else {
                // 失败
                log.info("error");
            }

        } catch (Exception e) {
            e.printStackTrace();
        }
    }
}

