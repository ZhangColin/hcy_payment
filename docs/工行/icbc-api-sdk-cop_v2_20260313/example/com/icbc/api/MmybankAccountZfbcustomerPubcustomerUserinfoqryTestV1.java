...
import com.icbc.api.request.MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1;
import com.icbc.api.request.MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1.MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1Biz;
import com.icbc.api.response.MybankAccountZfbcustomerPubcustomerUserinfoqryResponseV1;



public class MmybankAccountZfbcustomerPubcustomerUserinfoqryTestV1 {
	...
	public static void main(String[] args) {
		...
		MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1 request = new MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1();

		// 4、根据测试环境和生产环境替换相应ip和端口
		// 行外流程
		request.setServiceUrl("https://ip:port/api/mybank/account/zfbcustomer/pubcustomer/userinfoqry/V1");

		// 5、请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
		MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1Biz bizContent = new MybankAccountZfbcustomerPubcustomerUserinfoqryRequestV1Biz();
		bizContent.setChannel("YXT");
		bizContent.setApiCode("YXT");
		bizContent.setApiId("xxxxxxxxxxxxxxxxxxxxxx");
		bizContent.setAuthCode("123rty1234tdfghjvbn");
		bizContent.setAppId("YXT");
		request.setBizContent(bizContent);
		// 建立返回项
		MybankAccountZfbcustomerPubcustomerUserinfoqryResponseV1 response = new MybankAccountZfbcustomerPubcustomerUserinfoqryResponseV1();
		
		try {
			response = client.execute(request);
			logger.info("code:" + response.getReturnCode());
			logger.info("msg:" + response.getReturnMsg());
			logger.info("transErrorCode"+response.getTransErrorCode());
			logger.info("transErrorMsg:" + response.getTransErrorMsg());
