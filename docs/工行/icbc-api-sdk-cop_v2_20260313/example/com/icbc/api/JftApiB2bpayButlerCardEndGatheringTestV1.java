package com.icbc.api;

import com.icbc.api.request.JftApiB2bpayButlerCardEndGatheringRequestV1;
import com.icbc.api.response.JftApiB2bpayButlerCardEndGatheringResponseV1;

public class JftApiB2bpayButlerCardEndGatheringTestV1 {

    public static void main(String[] args) {

        String APP_ID = "10000000000000212516";
        String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
        String icbcPulicKey = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";

        String AES_Key = "5xGJdh7qb+B95SUoxDlatg==";
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, IcbcConstants.SIGN_TYPE_RSA2, MY_PRIVATE_KEY, IcbcConstants.CHARSET_UTF8, IcbcConstants.FORMAT_JSON,
                icbcPulicKey, IcbcConstants.ENCRYPT_TYPE_AES, AES_Key, null, null);

        JftApiB2bpayButlerCardEndGatheringRequestV1 request = new JftApiB2bpayButlerCardEndGatheringRequestV1();
        request.setServiceUrl("https://apipcs4.dccnet.com.cn/api/jft/api/b2bpay/butlercardendgathering/V1");

        JftApiB2bpayButlerCardEndGatheringRequestV1.JftApiB2bpayButlerCardEndGatheringRequestV1Biz bizContent = new JftApiB2bpayButlerCardEndGatheringRequestV1.JftApiB2bpayButlerCardEndGatheringRequestV1Biz();

        bizContent.setAppId("10000000000000205515");
        bizContent.setOutUserId("04221125");
        bizContent.setOrderId("20181128531212026");
        bizContent.setOriOrderId("20181128531212026");
        bizContent.setGoodsId("123456789");
        bizContent.setOperateType("02");
        request.setBizContent(bizContent);

        JftApiB2bpayButlerCardEndGatheringResponseV1 responseV1 = null;
        try {
            responseV1 = client.execute(request);
        } catch (IcbcApiException e) {
            e.printStackTrace();
        }
        System.out.println(responseV1);

    }
}
