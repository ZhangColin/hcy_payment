...
import com.icbc.api.request.BbcacpmTransPrequaryRequestV2;
import com.icbc.api.request.BbcacpmTransPrequaryRequestV2.BbcacpmTransPrequaryRequestBizV2;
import com.icbc.api.response.BbcacpmTransPrequaryResponseV2;


public class BbcacpmTransPrequaryTest {
    ...
    public static void main(String[] args) throws Exception {
        String URI = "bbcacpm/trans/prequary/V2"; // api地址
        DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, PRI_KEY, APIGW_PUBLIC_KEY);
        try {

            BbcacpmTransPrequaryRequestBizV2 bizContent = new BbcacpmTransPrequaryRequestBizV2();
            BbcacpmTransPrequaryRequestV2 request = new BbcacpmTransPrequaryRequestV2();

            // 请对照接口文档用bizContent.setxxx()方法对业务上送数据进行赋值
            bizContent.getGrpHdr().setRefrence("0118011800003042");
            bizContent.getGrpHdr().setBusinessCode("10080");
            ...
            bizContent.getAcpmTransPrequaryInBusitext().setCurrtype("RMB");
            bizContent.getAcpmTransPrequaryInBusitext().setOgcode("402100000018");
            bizContent.getAcpmTransPrequaryInBusitext().setProdacc("2000361660000001");
            bizContent.getAcpmTransPrequaryInBusitext().setTrxserno("789456123");
            bizContent.getAcpmTransPrequaryInBusitext().setCashamt("2000");

            // 通讯区参数设置
            request.setServiceUrl(BASE_URL + "/" + URI);
            request.setBizContent(bizContent);
            // 输出支付指令提交响应对象，需根据实际调用服务更换（BbcacpmTransPrequaryResponseV2）
            BbcacpmTransPrequaryResponseV2 response = client.execute(request);
