package com.icbc.api;

import com.icbc.api.internal.util.internal.util.fastjson.JSON;
import com.icbc.api.request.DepositAccountCorpRequestQryRequestV1;
import com.icbc.api.request.DepositAccountCorpRequestQryRequestV1.DepositAccountCorpRequestQryRequestV1Biz;
import com.icbc.api.response.DepositAccountCorpRequestQryResponseV1;

public class DepositAccountCorpRequestQryTestV1 {
	// 网关公钥
	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
		
	// 密钥对认证方式，公钥在API平台登记，此处为私钥
	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";
	
	// APPID
	protected static final String APP_ID = "xxxxxxxxxxxxxxxx";

	public static void main(String[] args) throws Exception {
		DepositAccountCorpRequestQryTestV1 test = new DepositAccountCorpRequestQryTestV1();
		test.corpTest();
	}

	public void corpTest() throws IcbcApiException {
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID, MY_PRIVATE_KEY, APIGW_PUBLIC_KEY);
		DepositAccountCorpRequestQryRequestV1 request = new DepositAccountCorpRequestQryRequestV1();
		DepositAccountCorpRequestQryRequestV1Biz bizContent = new DepositAccountCorpRequestQryRequestV1Biz();
		request.setServiceUrl("http://ip:port/api/deposit/account/corprequestqry/V1");

		// 根据API文档送入
		bizContent.setCorpAppId("xxxxxxxxxxxxxxxx"); // 合作方机构编号
		bizContent.setCorpSerno("corpJd1234"); // 合作方交易单号  
		bizContent.setCorpDate("2017-03-14"); // 合作方交易日期
		bizContent.setCorpTime("00:00:00"); // 合作方交易时间
		bizContent.setQryCorpSerno("corpJd1233"); // 查询的交易单号

		request.setBizContent(bizContent);		
		DepositAccountCorpRequestQryResponseV1 response;
		try {
			response = client.execute(request, "msgId");
			if (response.isSuccess() ) {
				// 成功
			} else {
				// 失败
			}			
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
}
