package com.icbc.api;

import org.junit.Test;

import com.icbc.api.internal.util.internal.util.fastjson.JSONObject;
import com.icbc.api.request.JftApiGetAcctDetailByAcctNoOutRequestV1;
import com.icbc.api.request.JftApiGetAcctDetailByAcctNoOutRequestV1.JftApiGetAcctDetailByAcctNoOutRequestV1Biz;
import com.icbc.api.response.JftApiGetAcctDetailByAcctNoOutResponseV1;

public class JftApiGetAcctDetailByAcctNoOutTestV1 {

	protected static final String MY_PRIVATE_KEY = "-----请使用正确的APP私钥-----";

	protected static final String APIGW_PUBLIC_KEY = "-----请使用正确的网关公钥-----D4kzEVPdOj03ctKM7KV+16bWZ5BMNgvEeuEQwfQYkRVwI9HFOGkwNTMn5hiJXHnlXYCX+zp5r6R52MY0O7BsTCLT7aHaxsANsvI9ABGx3OaTVlPB59M6GPbJh0uXvio0m1r/lTW3Z60RU6Q3oid/rNhP3CiNgg0W6O3AGqwIDAQAB";
	
	protected static final String APP_ID = "10000000000000004964";

	@SuppressWarnings("unchecked")

	public void test_cop() {
		
		DefaultIcbcClient client = new DefaultIcbcClient(APP_ID,"RSA", MY_PRIVATE_KEY,"UTF-8", "json",APIGW_PUBLIC_KEY,"AES","eyiSiHvFqYFLu/AZDCt+4w==", "", "");
		JftApiGetAcctDetailByAcctNoOutRequestV1 request = new JftApiGetAcctDetailByAcctNoOutRequestV1();
		request.setServiceUrl("http://ip:port/api/jft/api/vacct/detail/query/V1");
		JftApiGetAcctDetailByAcctNoOutRequestV1Biz bizContent = new JftApiGetAcctDetailByAcctNoOutRequestV1Biz();
		bizContent.setAcctNo("0001400902018020501552");
		bizContent.setAppId("0000000049");
		bizContent.setOutVendorId("");
		bizContent.setAcctType("00");
		bizContent.setQueryType("00");
		bizContent.setVersion("1.0.0");//设置接口访问版本号

		request.setBizContent(bizContent);
		JftApiGetAcctDetailByAcctNoOutResponseV1 response;
		try {
			response = (JftApiGetAcctDetailByAcctNoOutResponseV1) client.execute(request, System.currentTimeMillis()+"");
		
		System.out.println(JSONObject.toJSONString(response));
			if (response.isSuccess()) {
				// 业务成功处理
				System.out.println(response.getReturnCode());
			} else {
				// 失败
			}
		} catch (IcbcApiException e) {
			e.printStackTrace();
		}
	}
	
	
}

